# Blueballs API conventions — FROZEN CONTRACT (Wave 0)
...
| Rule | Value |
|---|---|
| Base | `https://blueballs.tech/v2` · self-hosted `http://localhost:5290/v2` |
...
A rail that settles in seconds walks the whole spine inside the create request; there is nothing
to wait for. A batch rail — one with a cutoff and a scheduled window — stops at `funds_received`
and waits, because that wait is real: the money has left the account and has not been handed to
the rail. `POST /v2/transfers/:id/settle` runs that window on demand, since a reference
implementation has no clock of its own.
...
| `canceled` | yes | any state before submission: `created`, `awaiting_funds`, `funds_received` |
