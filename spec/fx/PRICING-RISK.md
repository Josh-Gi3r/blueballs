...
The pricing and risk engine receives only sources that the institution's policy layer has authorized for the specific transaction context.
...
Fiat settlement is handled by the settlement layer.
...
Reference combination order is an optimization problem over firm executable slices, not a fixed marketing ladder.
...
Therefore:
- private-market route creation uses `fx-market` transactional reservations;
...
The test suite proves at least:
...
The deterministic pricing and risk core does not own:
- KYC vendor adapters;
- sanctions providers;
...
Keeping the core deterministic, auditable and executable is what lets those layers be added without re-deriving the pricing model.
