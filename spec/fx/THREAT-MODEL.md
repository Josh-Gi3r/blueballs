...
**On-chain policy revocation:** the kernel requires the taker intent's
`policyAuthorizationHash` to be live in `PolicyAuthorizationRegistry` at
execution time. The institution can revoke an individual authorization or
advance a minimum epoch to invalidate older authorizations. A compromised
executor therefore cannot settle a route merely because its maker and taker
...
Repository tests are not an independent security audit. The contracts and
cross-layer operational design require independent external review before a
deployment can be described as production-audited financial infrastructure.
