...
- the complete local release gate is green;
...
Run the complete JavaScript, API, Worker, contract-generation and publication
gate:
...
Blueballs deliberately runs no CI service. The release steps below are a manual
runbook: run them locally with `pnpm verify` as the gate, and record the output
with the release. Nothing in this repository is verified by a hosted pipeline,
so nothing here should be read as having been checked automatically.
...
- stores one release bundle in maintainer-controlled release storage.
