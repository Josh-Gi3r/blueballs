...
Blueballs v0.2 turns the first public release into a much more complete open financial operating system: a harder banking core, durable provider orchestration, institution-owned FX, stronger settlement contracts, reusable product Blueprints, infrastructure decision tooling and source-linked public proof.
...
## 7. One public product, built to distribute

The visible product is now converged around one canonical public information architecture:

**Home → Products → Stablecoin FX → Developers → Cards → Providers → Blueprints → Proof → Build with us**

with Sandbox as the primary build CTA.

Full public product shells use the same navigation order and responsive behavior, while Blueprint and Sandbox keep contextual controls for sharing, forking and Builder stages.

Blueballs also ships dedicated 1200×630 social launch cards for Home, FX, Cards, Providers, Blueprints, Sandbox and Proof. The edge selects the correct Open Graph and X image by route, serves the assets from stable `/social/*.png` endpoints and verifies image type, dimensions, cache policy and route mapping in the repository test path.

## 8. Proof is a product surface
...
## 9. Release proof stays with the code
