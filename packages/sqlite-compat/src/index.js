...
export class DatabaseSync {
  ...
  exec(sql) {
    ...
    if (normalized.startsWith("PRAGMA JOURNAL_MODE") || normalized.startsWith("PRAGMA BUSY_TIMEOUT")) {
      return undefined;
    }
    if (["BEGIN", "BEGIN IMMEDIATE", "COMMIT", "ROLLBACK"].includes(normalized)) {
      throw new Error("Durable Object transaction-control SQL is unsupported; use transactionSync(callback)");
    }
  ...
  transactionSync(callback) {
    if (typeof callback !== "function") throw new TypeError("transactionSync requires a callback");
    if (this.native) {
