import { applyBps, quoteInputAtomicExactOutput, rational, toDecimalString } from './rational.js';
...
function referenceRational(reference) {
  ...
  if (numerator == null || denominator == null) throw new TypeError('reference midRational required');
...
export class PrincipalQuoteEngine {
  constructor({
    ...
    if (!riskBook || typeof riskBook.reserve !== 'function') throw new TypeError('riskBook required');
  ...
  previewExactOutput({
    ...
    if (typeof inputAsset !== 'string' || inputAsset.length === 0) throw new TypeError('inputAsset required');
    if (typeof outputAsset !== 'string' || outputAsset.length === 0) throw new TypeError('outputAsset required');
    ...
    const components = {
      ...
      inventoryBps: integerBps('inventoryBps', inventoryBps, { allowNegative: true }),
    };
    const rawSpreadBps = Object.values(components).reduce((sum, value) => sum + value, 0);
    const totalSpreadBps = Math.max(this.minimumSpreadBps, rawSpreadBps);
    if (totalSpreadBps <= -10_000) throw new RangeError('total spread makes price non-positive');
