...
export function parseDecimal(value) {
  ...
  const numerator =
    BigInt(`${whole}${fraction}` || "0") * (negative ? -1n : 1n);
...
export function median(values) {
  if (!Array.isArray(values) || values.length === 0)
    throw new RangeError("median requires values");
...
export function applyBps(value, bps) {
  ...
  if (basisPoints <= -10_000n)
    throw new RangeError("bps would make value non-positive");
...
export function ceilDiv(numerator, denominator) {
  ...
  if (n < 0n || d <= 0n)
    throw new RangeError(
      "ceilDiv requires non-negative numerator and positive denominator",
    );
  return (n + d - 1n) / d;
}
