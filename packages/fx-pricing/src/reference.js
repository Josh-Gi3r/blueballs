...
export class ReferencePriceEngine {
  constructor({
    ...
    if (!Number.isSafeInteger(maxAgeMs) || maxAgeMs <= 0)
      throw new RangeError("maxAgeMs invalid");
    if (!Number.isSafeInteger(minSources) || minSources <= 0)
      throw new RangeError("minSources invalid");
  ...
  consensus({ base, quote, observations }) {
    if (typeof base !== "string" || base.length === 0)
      throw new TypeError("base required");
    if (typeof quote !== "string" || quote.length === 0)
      throw new TypeError("quote required");
    ...
    for (const observation of observations) {
      ...
      if (
        ...
        now - observation.observedAt > this.maxAgeMs
      ) {
      ...
      if (
        observation.status &&
        !["OK", "DEGRADED"].includes(observation.status)
      ) {
    ...
    if (accepted.length === 0)
      return this.#unavailable(base, quote, rejected, "NO_VALID_SOURCES");
    ...
    return {
      ...
      midRational: {
        numerator: finalMedian.n.toString(),
        denominator: finalMedian.d.toString(),
      },
      sourceIds,
      rejected,
      observedAt,
