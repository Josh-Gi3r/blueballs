...
import { FxPolicyEngine, createMarketAuthorizationVerifier } from '../src/index.js';
...
function setup() {
  ...
  const market = new FxMarketService({
    ...
    policyAuthorizer: async (admission) => {
      ...
      return result.valid ? { eligible: true } : { eligible: false, reason: result.reason };
    },
...
test('authorization context is bound to action corridor amount and maker account', async () => {
  ...
  await assert.rejects(() => market.admitOrder(wrongMaker), /ACCOUNT_ATTRIBUTION_MISMATCH/);
...
test('credential change after admission removes order before reservation', async () => {
  ...
  assert.deepEqual(market.aggregateDepth({ inputToken: INPUT, outputToken: OUTPUT }), []);
