...
export class FxPolicyEngine {
  ...
  configurePolicy(policy) {
    ...
    if (typeof policy.policyId !== "string" || !policy.policyId)
      throw new TypeError("policyId required");
    if (!Number.isSafeInteger(policy.version) || policy.version <= 0)
      throw new RangeError("policy version invalid");
    if (
      !Number.isSafeInteger(policy.authorizationTtlMs) ||
      policy.authorizationTtlMs <= 0
    ) {
    ...
    const normalized = {
      ...
      enabledParticipantTypes: [
        ...new Set(policy.enabledParticipantTypes ?? []),
      ].sort(),
      ...
      blockedJurisdictions: [
        ...new Set(policy.blockedJurisdictions ?? []),
      ].sort(),
  ...
  upsertParticipant({
    ...
    if (typeof participantId !== "string" || !participantId)
      throw new TypeError("participantId required");
    if (!PARTICIPANT_TYPES.has(participantType))
      throw new RangeError("participantType invalid");
    if (!["ACTIVE", "SUSPENDED", "CLOSED"].includes(status))
      throw new RangeError("participant status invalid");
    if (typeof jurisdiction !== "string" || !jurisdiction)
      throw new TypeError("jurisdiction required");

    return this.#transaction(() => {
      const existing = this.db
        .prepare("SELECT * FROM fx_participants WHERE participant_id = ?")
        .get(participantId);
      const changed =
        existing &&
        (existing.participant_type !== participantType ||
          existing.status !== status ||
          existing.jurisdiction !== jurisdiction ||
          existing.risk_tier !== riskTier);
  ...
  upsertCredential({
    ...
    if (typeof credentialType !== "string" || !credentialType)
      throw new TypeError("credentialType required");
    if (!["PASSED", "FAILED", "PENDING", "EXPIRED"].includes(status))
      throw new RangeError("credential status invalid");
    ...
    return this.#transaction(() => {
      ...
      const changed =
        ...
        previous.provider_ref !== providerRef ||
        previous.issued_at !== issuedAt ||
  ...
  mapAccount({ participantId, accountRef, accountType = "WALLET" }) {
    ...
    if (typeof accountRef !== "string" || !accountRef)
      throw new TypeError("accountRef required");
    return this.#transaction(() => {
      const previous = this.db
  ...
  #bumpParticipantEpoch(participantId) {
    this.db
      .prepare(
        "UPDATE fx_participants SET epoch = epoch + 1 WHERE participant_id = ?",
      )
      .run(participantId);
  }
  ...
  authorize({
    ...
    if (participant?.status !== "ACTIVE")
      reasons.push("PARTICIPANT_NOT_ACTIVE");
    if (
      participant &&
      !policy.enabledParticipantTypes.includes(participant.participantType)
    )
      reasons.push("PARTICIPANT_TYPE_DISABLED");
    if (
      participant &&
      policy.blockedJurisdictions.includes(participant.jurisdiction)
    )
      reasons.push("JURISDICTION_BLOCKED");
    if (
      !policy.allowedAssets.includes(inputAsset) ||
      !policy.allowedAssets.includes(outputAsset)
    )
      reasons.push("ASSET_NOT_ALLOWED");
    if (!policy.allowedCorridors.includes(`${inputAsset}/${outputAsset}`))
      reasons.push("CORRIDOR_NOT_ALLOWED");
    ...
    if (participant) {
      for (const type of policy.requiredCredentials[
        ...
        credentialsUsed.push({
          type,
          providerRef: credential.provider_ref,
    ...
    if (accountRef !== null) {
      const mapping = this.db
        .prepare("SELECT participant_id FROM fx_accounts WHERE account_ref = ?")
        .get(accountRef);
      if (!mapping || mapping.participant_id !== participantId)
        reasons.push("ACCOUNT_NOT_ATTRIBUTED");
    }
    ...
    const expiresAt = Math.min(
      now + policy.authorizationTtlMs,
      credentialExpiry ?? Number.MAX_SAFE_INTEGER,
    );
    const snapshot = {
      participantId,
  ...
  verifyAuthorization(authorizationId) {
    ...
    if (!participant || participant.status !== "ACTIVE")
      return { valid: false, reason: "PARTICIPANT_NOT_ACTIVE" };
    if (participant.epoch !== row.participant_epoch)
      return { valid: false, reason: "PARTICIPANT_CHANGED" };
    const policy = this.currentPolicy();
    if (
  ...
  revokeAuthorization(authorizationId) {
    const result = this.db
      .prepare(
        "UPDATE fx_authorizations SET revoked = 1 WHERE authorization_id = ? AND revoked = 0",
      )
      .run(authorizationId);
