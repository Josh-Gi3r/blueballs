...
export class FxMarketService {
  constructor(options = {}) {
    ...
    if (
      this.authorizationVerifier !== null &&
      typeof this.authorizationVerifier !== "function"
    ) {
  ...
  #verifyAuthorization(order) {
    ...
    if (!result || result.valid !== true) {
      return {
        valid: false,
        reason: result?.reason ?? "AUTHORIZATION_INVALID",
      };
    }
    return result;
  }
  ...
  #revalidateReservedRoute(routeId) {
    ...
    for (const order of rows) {
      const verification = this.#verifyAuthorization(order);
      if (!verification.valid) {
        this.blockOrderPolicy(
          order.order_hash,
          `authorization invalid: ${verification.reason}`,
        );
        const error = new Error(
          `route authorization invalid: ${verification.reason}`,
        );
  ...
  getRoute(routeId) {
    return rowToRoute(
      this.db.prepare("SELECT * FROM routes WHERE route_id = ?").get(routeId),
    );
  }
  ...
  markRouteSubmitted(routeId, submissionRef) {
    ...
    if (
      ...
      current.submissionRef === submissionRef
    ) {
      return current;
    }
    if (current.state !== "RESERVED")
      throw new Error(`route cannot be submitted from ${current.state}`);
    if (current.expiresAt <= this.now())
      throw new Error("route reservation expired");
    ...
    return this.#transaction(() => {
      ...
      if (route.state !== "RESERVED")
        throw new Error(`route cannot be submitted from ${route.state}`);
      if (route.expires_at <= this.now())
        throw new Error("route reservation expired");

      this.db
        .prepare(
          "UPDATE routes SET state = 'SUBMITTED', submission_ref = ? WHERE route_id = ?",
        )
  ...
  releaseRoute(routeId, reason = "RELEASED") {
    ...
    if (route.state === "SUBMITTED")
      throw new Error("submitted route cannot be released; reconcile it");
    if (["CONFIRMED", "FAILED", "RELEASED", "EXPIRED"].includes(route.state))
      return 0;

    const released = this.market.releaseRoute(routeId, reason);
    this.db
      .prepare(
        "UPDATE routes SET state = 'RELEASED', failure_reason = ? WHERE route_id = ?",
      )
      .run(reason, routeId);
    return released;
  }
  ...
  expireRoutes(now = this.now()) {
    const rows = this.db
      .prepare(
        "SELECT route_id FROM routes WHERE state = 'RESERVED' AND expires_at <= ?",
      )
      .all(now);
    ...
    for (const row of rows) {
      ...
      this.db
        .prepare("UPDATE routes SET state = 'EXPIRED' WHERE route_id = ?")
        .run(row.route_id);
    }
    return released;
  }
  ...
  confirmSubmittedRoute({ routeId, eventId, fills }) {
    ...
    if (route.state === "CONFIRMED") {
      if (route.confirmedEventId !== eventId)
        throw new Error("route already confirmed by another event");
    ...
    this.db
      .prepare(
        "UPDATE routes SET state = 'CONFIRMED', confirmed_event_id = ? WHERE route_id = ?",
      )
      .run(eventId, routeId);
  ...
  failSubmittedRoute({
    ...
    this.db
      .prepare(
        "UPDATE routes SET state = 'FAILED', failure_reason = ? WHERE route_id = ?",
      )
      .run(reason, routeId);
    return result;
  }
  ...
  blockOrderPolicy(orderHash, reason = "policy revoked") {
    return this.#transaction(() => {
      ...
      if (activeReservation?.route_state === "SUBMITTED") {
        this.db
          .prepare(
            "UPDATE orders SET policy_blocked = 1, policy_reason = ? WHERE order_hash = ?",
          )
      ...
      if (activeReservation) {
        ...
        this.db
          .prepare(
            "UPDATE routes SET state = 'RELEASED', failure_reason = 'POLICY_BLOCKED' WHERE route_id = ? AND state = 'RESERVED'",
          )
  ...
  cancelOrderOffchain(orderHash, { onChainInvalidated = false } = {}) {
    return this.#transaction(() => {
      ...
      if (activeReservation?.route_state === "SUBMITTED") {
        this.db
          .prepare(
            "UPDATE orders SET offchain_hidden = 1, onchain_invalidated = ? WHERE order_hash = ?",
          )
      ...
      if (activeReservation) {
        ...
        this.db
          .prepare(
            "UPDATE routes SET state = 'RELEASED', failure_reason = 'ORDER_CANCELLED' WHERE route_id = ? AND state = 'RESERVED'",
          )
          .run(activeReservation.route_id);
      }
  ...
  #normalizePostSettlementOrderStates(routeId) {
    ...
    for (const orderHash of orderHashes) {
      ...
      if (row.onchain_invalidated || row.offchain_hidden) {
        this.db
          .prepare("UPDATE orders SET state = 'CANCELLED' WHERE order_hash = ?")
          .run(orderHash);
      } else if (row.policy_blocked) {
        this.db
          .prepare(
            "UPDATE orders SET state = 'POLICY_BLOCKED' WHERE order_hash = ?",
          )
          .run(orderHash);
      }
    }
