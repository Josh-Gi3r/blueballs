...
export class SqliteFxMarket {
  ...
  async admitOrder(payload) {
    ...
    if (!policy || policy.eligible !== true) {
      throw new Error(
        `policy rejected order${policy?.reason ? `: ${policy.reason}` : ""}`,
      );
    }
  ...
  reserveExactOutput({
    ...
    if (typeof routeId !== "string" || routeId.length === 0)
      throw new TypeError("routeId is required");
    const input = normalizeAddress(inputToken, "inputToken");
    const output = normalizeAddress(outputToken, "outputToken");
    if (input === output)
      throw new RangeError("input and output token must differ");

    const desired = BigInt(String(desiredOutput));
    if (desired <= 0n)
      throw new RangeError("desiredOutput must be greater than zero");
    ...
    return this.#transaction(() => {
      const existing = this.db
        .prepare(
          "SELECT * FROM reservations WHERE route_id = ? AND state = 'ACTIVE'",
        )
        .all(routeId);
      if (existing.length > 0)
        throw new Error("route already has active reservations");
      ...
      for (const row of candidates) {
        ...
        this.db
          .prepare(
            "UPDATE orders SET reserved_sell = ?, state = 'RESERVED' WHERE order_hash = ?",
          )
  ...
  #restoreOrderAfterReservation(orderHash) {
    ...
    let state =
      filled === 0n ? "OPEN" : filled < signed ? "PARTIALLY_FILLED" : "FILLED";
    if (row.policy_blocked) state = "POLICY_BLOCKED";
    if (row.offchain_hidden || row.onchain_invalidated) state = "CANCELLED";

    this.db
      .prepare(
        "UPDATE orders SET reserved_sell = ?, state = ? WHERE order_hash = ?",
      )
    ...
    return this.#transaction(() => {
      ...
      for (const routeId of expired) {
        ...
        for (const reservation of reservations) {
          this.db
            .prepare(
              "UPDATE reservations SET state = 'EXPIRED' WHERE reservation_id = ?",
            )
            .run(reservation.reservation_id);
          this.#restoreOrderAfterReservation(reservation.order_hash);
          count += 1;
        }
        this.#audit("ROUTE_EXPIRED", {
          routeId,
  ...
  confirmRoute({ routeId, eventId, fills }) {
    ...
    return this.#transaction(() => {
      ...
      const reservations = this.db
        ...
        .all(routeId);
      if (reservations.length === 0)
        throw new Error("no active reservations for route");
      ...
      for (let i = 0; i < reservations.length; i += 1) {
        ...
        const newSell =
          BigInt(order.confirmed_filled_sell) +
          BigInt(reservation.maker_sell_amount);
        const newBuy =
          BigInt(order.confirmed_filled_buy) +
          BigInt(reservation.taker_pay_amount);
        if (
          newSell > BigInt(order.sell_amount) ||
          newBuy > BigInt(order.buy_amount)
        ) {
          throw new Error("confirmed fill exceeds signed maker order");
        }

        const nextState =
          newSell === BigInt(order.sell_amount) ? "FILLED" : "PARTIALLY_FILLED";
        this.db
          .prepare(
            `
            UPDATE orders
        ...
        this.db
          .prepare(
            "UPDATE reservations SET state = 'CONSUMED' WHERE reservation_id = ?",
          )
          .run(reservation.reservation_id);
      }
  ...
  blockPolicy(orderHash, reason = "policy revoked") {
    return this.#transaction(() => {
      ...
      const active = this.db
        .prepare(
          "SELECT * FROM reservations WHERE order_hash = ? AND state = 'ACTIVE'",
        )
        .all(orderHash);
