...
function market({ now = () => 1_000_000, signatureVerifier, policyAuthorizer, path } = {}) {
...
test('best price wins even when admitted later', async () => {
  const book = market();
  await book.admitOrder(makeAdmission({ id: 10, maker: MAKER_A, buyAmount: '200' }));
  await book.admitOrder(makeAdmission({ id: 11, maker: MAKER_B, buyAmount: '190' }));
...
test('equal economic price uses admission sequence before hash', async () => {
  const book = market();
  await book.admitOrder(makeAdmission({ id: 21, maker: MAKER_A, sellAmount: '50', buyAmount: '100' }));
  await book.admitOrder(makeAdmission({ id: 20, maker: MAKER_B, sellAmount: '100', buyAmount: '200' }));
...
test('route can span multiple makers and reproduces exact signed payloads', async () => {
  const book = market();
  const a = makeAdmission({ id: 30, maker: MAKER_A, sellAmount: '40', buyAmount: '80' });
  const b = makeAdmission({ id: 31, maker: MAKER_B, sellAmount: '60', buyAmount: '114' });
...
test('insufficient liquidity rolls back all provisional reservations', async () => {
  ...
  await admitMany(book, [
    makeAdmission({ id: 40, maker: MAKER_A, sellAmount: '40', buyAmount: '80' }),
    makeAdmission({ id: 41, maker: MAKER_B, sellAmount: '60', buyAmount: '120' }),
  ]);
...
test('same maker order cannot be promised to two routes concurrently', async () => {
  const book = market();
  await book.admitOrder(makeAdmission({ id: 50, maker: MAKER_A, sellAmount: '100', buyAmount: '200' }));
...
test('confirmed chain fills are exact and duplicate events are idempotent', async () => {
  const book = market();
  await book.admitOrder(makeAdmission({ id: 90, sellAmount: '100', buyAmount: '200' }));
  const route = book.reserveExactOutput({
  ...
  assert.deepEqual(book.confirmRoute({ routeId: 'confirm-route', eventId: 'tx:1:log:0', fills }), {
    duplicate: false,
  });
...
test('failed settlement releases reservation without inventing a fill', async () => {
  ...
  const result = book.failRoute({ routeId: 'failed-route', eventId: 'failed:1' });
...
test('aggregate depth combines equivalent prices and leaks no maker identity or signed payload', async () => {
  const book = market();
  await admitMany(book, [
    makeAdmission({ id: 130, maker: MAKER_A, sellAmount: '50', buyAmount: '100' }),
    makeAdmission({ id: 131, maker: MAKER_B, sellAmount: '100', buyAmount: '200' }),
  ]);
