...
test('two independent SQLite connections cannot double-reserve the same maker order', async () => {
  ...
  try {
    ...
    const [a, b] = await Promise.all([startWorker(path, 'route-a'), startWorker(path, 'route-b')]);
    const results = [a, b];
    assert.equal(results.filter((result) => result.ok).length, 1);
    assert.equal(results.filter((result) => !result.ok).length, 1);
    assert.match(results.find((result) => !result.ok).error, /insufficient eligible liquidity/);
