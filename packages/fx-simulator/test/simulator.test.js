...
function asBig(value) { return BigInt(value); }

function assertPrincipalInvariant(result) {
  assert.ok(asBig(result.peakPrincipalExposureAbs) <= asBig(result.principalHardLimit));
...
test('90 percent one-way flow pushes principal exposure harder than balanced flow', () => {
  ...
  assert.ok(asBig(oneWay.peakPrincipalExposureAbs) > asBig(balanced.peakPrincipalExposureAbs));
...
test('institutional LP outage reduces LP contribution after the outage', () => {
  ...
  assert.ok(asBig(outage.routeComposition.INSTITUTIONAL_LP) < asBig(control.routeComposition.INSTITUTIONAL_LP));
...
test('issuer outage reduces issuer contribution', () => {
  ...
  assert.ok(asBig(outage.routeComposition.ISSUER) < asBig(control.routeComposition.ISSUER));
  assertPrincipalInvariant(outage);
});
...
test('cancellation storm reduces private-market routed volume', () => {
  ...
  assert.ok(asBig(storm.routeComposition.PRIVATE_MARKET) < asBig(control.routeComposition.PRIVATE_MARKET));
