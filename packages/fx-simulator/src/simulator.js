...
function makeSlice(source, direction, now, principalCapacity) {
  ...
  if (source.sourceType === "PRIVATE_MARKET") {
    capacity =
      (capacity * BigInt(source.privateCapacityBps ?? 10_000)) / 10_000n;
  }
...
function pct(n, d) {
  return d === 0 ? 0 : Number((BigInt(n) * 1_000_000n) / BigInt(d)) / 10_000;
}
...
export function runSimulation(config) {
  ...
  if (hardLimit <= 0n)
    throw new RangeError("principalHardLimit must be positive");
  ...
  function applyEvent(event) {
    if (event.type === "REFERENCE_UNAVAILABLE")
  ...
  for (let i = 0; i < requests.length; i += 1) {
    while (eventIndex < events.length && events[eventIndex].at === i)
      applyEvent(events[eventIndex++]);
    const request = requests[i];
    const direction = request.direction;
    if (!["BUY_B", "SELL_B"].includes(direction))
      throw new RangeError("request direction invalid");
    const amount = BigInt(String(request.outputAmount));
    if (amount <= 0n)
      throw new RangeError("request outputAmount must be positive");

    metrics.requestedOrders += 1;
    metrics.requestedVolume += amount;
    if (!state.referenceAvailable) metrics.referenceOutageRequests += 1;

    const principalCapacity =
      direction === "BUY_B"
    ...
    for (const source of sources) {
      if (source.sourceType === "BANK_PRINCIPAL" && !state.referenceAvailable)
        continue;
    ...
    try {
      ...
      const anyPrincipal = sources.some(
        (s) => s.sourceType === "BANK_PRINCIPAL" && s.online,
      );
  ...
  return {
    ...
    routeComposition: Object.fromEntries(
      Object.entries(metrics.routeComposition).map(([k, v]) => [
        k,
        v.toString(),
      ]),
    ),
