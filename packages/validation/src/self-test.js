...
assert(
  "ibanValid: known-good DE IBAN",
  ibanValid("DE89370400440532013000") === true,
);
assert(
  "ibanValid: spaces stripped",
  ibanValid("DE89 3704 0044 0532 0130 00") === true,
);
assert(
  "ibanValid: corrupted checksum rejected",
  ibanValid("DE89370400440532013001") === false,
);
assert("ibanValid: garbage rejected", ibanValid("not-an-iban") === false);
assert(
  "ibanCheckDigits: matches known vector",
  ibanCheckDigits("DE", "370400440532013000") === "89",
);
assert(
  "ibanGenerate: round-trips to known vector",
  ibanGenerate("DE", "370400440532013000") === "DE89370400440532013000",
);
assert(
  "ibanGenerate: different BBAN -> different, still-valid IBAN",
  (() => {
...
assert(
  "luhnValid: known-good test card",
  luhnValid("4111111111111111") === true,
);
assert(
  "luhnValid: off-by-one digit rejected",
  luhnValid("4111111111111112") === false,
);
assert(
  "luhnCheckDigit + luhnValid round-trip",
  (() => {
...
assert(
  "luhnGenerate produces a Luhn-valid number",
  luhnValid(luhnGenerate("511111111111111")) === true,
);
...
assert(
  "abaValid: corrupted checksum rejected",
  abaValid("021000022") === false,
);
...
assert(
  "abaGenerate: two different prefixes -> two different, both-valid numbers",
  (() => {
