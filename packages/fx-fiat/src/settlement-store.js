...
export class FiatSettlementStore {
  ...
  registerVerifier({
    ...
    if (typeof verifierType !== "string" || verifierType.length === 0)
      throw new TypeError("verifierType required");
    this.db
      .prepare(
        `
  ...
  #transition(intentId, fromStates, toState, updates = {}) {
    if (!INTENT_STATES.has(toState))
      throw new RangeError("invalid target state");
    return this.#transaction(() => {
      const row = this.db
        .prepare("SELECT * FROM fiat_intents WHERE intent_id = ?")
        .get(intentId);
      ...
      if (!fromStates.includes(row.state))
        throw new Error(`cannot transition ${row.state} to ${toState}`);
      if (
        row.expires_at <= this.now() &&
        !["EXPIRED", "FAILED"].includes(toState)
      )
      ...
      this.db
        .prepare(
          `UPDATE fiat_intents SET ${fields.join(", ")} WHERE intent_id = ?`,
        )
        .run(...values);
  ...
  submitIntent(intentId, submissionRef) {
    if (typeof submissionRef !== "string" || submissionRef.length === 0)
      throw new TypeError("submissionRef required");
    return this.#transition(intentId, ["RESERVED"], "SUBMITTED", {
      submissionRef,
    });
  }
  ...
  acceptAttestation(attestation) {
    if (!attestation || typeof attestation !== "object")
      throw new TypeError("attestation required");
    const now = this.now();
    ...
    return this.#transaction(() => {
      const intent = this.db
        .prepare("SELECT * FROM fiat_intents WHERE intent_id = ?")
        .get(attestation.intentId);
      if (!intent) throw new Error("intent not found");
      if (!["SUBMITTED", "PAYMENT_OBSERVED"].includes(intent.state)) {
        throw new Error(
          `intent cannot accept attestation from ${intent.state}`,
        );
      }
      if (intent.expires_at <= now) throw new Error("intent expired");
      if (attestation.intentHash !== intent.intent_hash)
        throw new Error("intent hash mismatch");
      if (String(attestation.currency) !== intent.currency)
        throw new Error("currency mismatch");
      if (BigInt(String(attestation.amount)) !== BigInt(intent.amount))
        throw new Error("amount mismatch");
      if (attestation.payerRefHash !== intent.payer_ref_hash)
        throw new Error("payer mismatch");
      if (attestation.payeeRefHash !== intent.payee_ref_hash)
        throw new Error("payee mismatch");
      if (
        typeof attestation.paymentId !== "string" ||
        attestation.paymentId.length === 0
      )
      ...
      this.db
        .prepare(
          "UPDATE fiat_intents SET state = 'VERIFIED', attestation_id = ? WHERE intent_id = ?",
        )
