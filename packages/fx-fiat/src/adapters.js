...
export class InternalLedgerVerifier {
  constructor({ store, verifierId = 'internal-ledger', now = () => Date.now() } = {}) {
    requireStore(store);
    this.store = store;
    this.verifierId = verifierId;
    this.now = now;
    this.store.registerVerifier({ verifierId, verifierType: 'INTERNAL_LEDGER' });
  }
...
export class AttestedPaymentVerifier {
  ...
  verify(intent, evidence) {
    ...
    if (evidence.status === 'MANUAL_REVIEW') {
      return { status: 'MANUAL_REVIEW', reason: evidence.reason ?? 'PROVIDER_REVIEW' };
    }
    if (evidence.status !== 'VERIFIED') {
      return { status: 'REJECTED', reason: evidence.reason ?? 'PROVIDER_REJECTED' };
    }
