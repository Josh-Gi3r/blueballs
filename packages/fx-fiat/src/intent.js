...
export function normalizeFiatIntent(intent) {
  if (!intent || typeof intent !== "object")
    throw new TypeError("intent required");
  if (!EDGE_TYPES.has(intent.edgeType))
    throw new RangeError("edgeType invalid");
  if (!FINALITY.has(intent.finalityClass))
  ...
  if (!Number.isSafeInteger(intent.createdAt) || intent.createdAt < 0)
    throw new RangeError("createdAt invalid");
  if (
  ...
  return {
    ...
    payerParticipantId: requiredString(
      intent.payerParticipantId,
      "payerParticipantId",
    ),
    payeeParticipantId: requiredString(
