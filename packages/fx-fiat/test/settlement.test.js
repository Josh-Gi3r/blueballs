...
test('external fiat intent cannot be cancelled after submission', () => {
  ...
  assert.throws(() => store.cancelIntent(i.intentId), /cannot transition SUBMITTED to CANCELLED/);
...
test('verified attestation must match exact intent economics and counterparties', () => {
  for (const [field, value, message] of [
    ...
    assert.throws(() => store.acceptAttestation(attestation(i, { [field]: value })), message);
    assert.equal(store.getIntent(i.intentId).state, 'SUBMITTED');
    store.close();
  }
});
...
test('disabled verifier cannot attest a fiat payment', () => {
  const store = new FiatSettlementStore({ now: () => NOW });
  store.registerVerifier({ verifierId: 'verifier-1', verifierType: 'TEE', enabled: false });
  const i = submitted(store);
  assert.throws(() => store.acceptAttestation(attestation(i)), /verifier not enabled/);
  store.close();
});
...
test('one payment id cannot satisfy two intents', () => {
  ...
  assert.throws(
    () => store.acceptAttestation(attestation(second, { paymentId: 'shared-payment' })),
...
test('duplicate attestation delivery is idempotent', () => {
  const store = new FiatSettlementStore({ now: () => NOW });
  store.registerVerifier({ verifierId: 'verifier-1', verifierType: 'BANK_API' });
  const i = submitted(store);
  const proof = attestation(i);
  assert.deepEqual(store.acceptAttestation(proof), { duplicate: false, attestationId: proof.attestationId });
  assert.deepEqual(store.acceptAttestation(proof), { duplicate: true, attestationId: proof.attestationId });
...
test('verified intent settles once and duplicate settlement event is idempotent', () => {
  const store = new FiatSettlementStore({ now: () => NOW });
  store.registerVerifier({ verifierId: 'verifier-1', verifierType: 'INTERNAL_LEDGER' });
  const i = submitted(store);
  store.acceptAttestation(attestation(i));

  assert.deepEqual(store.settleVerifiedIntent(i.intentId, 'event-1'), { duplicate: false });
  assert.equal(store.getIntent(i.intentId).state, 'SETTLED');
  assert.deepEqual(store.settleVerifiedIntent(i.intentId, 'event-1'), { duplicate: true });
  store.close();
});
