...
test("same atomic boundary may truthfully advertise atomic route", () => {
  ...
  graph.upsertEdge(edge({ edgeId: "b", fromAsset: "EURC", toAsset: "SGD" }));
...
test("mixed fiat-token route explicitly reports mixed finality", () => {
  ...
  graph.upsertEdge(
    edge({ edgeId: "usdc-eurc", fromAsset: "USDC", toAsset: "EURC" }),
  );
  graph.upsertEdge(
    edge({
      edgeId: "eurc-eur",
...
test("unavailable or unauthorized edge cannot enter route", () => {
  ...
  graph.upsertEdge(
    edge({ edgeId: "allowed-edge", policyAuthorizationId: "allowed" }),
  );
...
test("route continuity is enforced", () => {
  const graph = new SettlementGraph();
  graph.upsertEdge(edge({ edgeId: "a", fromAsset: "USDC", toAsset: "EURC" }));
  graph.upsertEdge(edge({ edgeId: "b", fromAsset: "SGD", toAsset: "MYR" }));
...
test("internal ledger adapter converts an authoritative posted event into verified evidence", () => {
  ...
  const verifier = new InternalLedgerVerifier({
    store,
    now: () => NOW + 2_000,
  });
...
test("generic attested-payment adapter preserves pending/review/rejected/verified semantics", () => {
  ...
  const verifier = new AttestedPaymentVerifier({
    store,
    verifierId: "peer-style",
    verifierType: "TEE",
  });

  assert.deepEqual(verifier.verify(intent, { status: "PENDING" }), {
    status: "PENDING",
  });
  assert.deepEqual(
