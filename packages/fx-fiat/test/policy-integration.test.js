...
test("fiat settlement edge authorization is bound to provider pair and capacity", () => {
  ...
  assert.throws(
    ...
    (error) =>
      ...
      /AMOUNT_EXCEEDS_AUTHORIZATION/.test(error.message),
  );
  assert.throws(
    () =>
      graph.upsertEdge(
        edge(decision.authorizationId, {
          edgeId: "wrong-provider",
          providerId: "other-provider",
        }),
      ),
    (error) =>
      error.code === "POLICY_AUTHORIZATION_INVALID" &&
      /PROVIDER_MISMATCH/.test(error.message),
  );
  assert.throws(
    () =>
      graph.upsertEdge(
    ...
    (error) =>
      error.code === "POLICY_AUTHORIZATION_INVALID" &&
      /OUTPUT_ASSET_MISMATCH/.test(error.message),
  );
  policy.close();
});
...
test("credential change removes fiat edge at route-selection time", () => {
  ...
  assert.throws(
    ...
    (error) =>
      error.code === "POLICY_AUTHORIZATION_INVALID" &&
      /PARTICIPANT_CHANGED/.test(error.message),
  );
  policy.close();
});
