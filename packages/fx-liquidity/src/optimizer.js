...
function validateSlice(slice, { inputAsset, outputAsset, now }) {
  ...
  if (slice.inputAsset !== inputAsset || slice.outputAsset !== outputAsset)
    return false;
  if (
    typeof slice.policyAuthorizationId !== "string" ||
    !slice.policyAuthorizationId
  )
    return false;
  if (!Number.isSafeInteger(slice.expiresAt) || slice.expiresAt <= now)
    return false;
  try {
    if (asPositiveBigInt(slice.maxOutput, "maxOutput") <= 0n) return false;
    if (asPositiveBigInt(slice.inputNumerator, "inputNumerator") <= 0n)
      return false;
    if (asPositiveBigInt(slice.inputDenominator, "inputDenominator") <= 0n)
      return false;
  } catch {
    return false;
  }
...
export function planExactOutput({
  inputAsset,
  outputAsset,
  desiredOutput,
  slices,
  ...
  if (typeof inputAsset !== "string" || !inputAsset)
    throw new TypeError("inputAsset required");
  if (typeof outputAsset !== "string" || !outputAsset)
  ...
  if (!Number.isSafeInteger(now) || now < 0)
    throw new RangeError("now invalid");
