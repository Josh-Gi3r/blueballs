...
export async function reservePlan({ routeId, plan, adapters }) {
  if (typeof routeId !== "string" || !routeId)
    throw new TypeError("routeId required");
  ...
  try {
    for (let index = 0; index < plan.legs.length; index += 1) {
      ...
      if (
        !result ||
        typeof result.reservationHandle !== "string" ||
        !result.reservationHandle
      ) {
        throw new Error(
          `${leg.sourceType} returned invalid reservation handle`,
        );
      }
      reserved.push({
        ...leg,
        reservationHandle: result.reservationHandle,
        index,
      });
    }
  } catch (error) {
...
export async function releaseReservedRoute({
  route,
  adapters,
  reason = "RELEASED",
}) {
