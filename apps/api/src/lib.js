...
class PersistentEvents {
  constructor() {
    ...
    this._insert = sqlite.prepare(`INSERT INTO events (id, type, created_at, data, tenant_id) VALUES (?, ?, ?, ?, ?)`);
  ...
  push(evt) {
    this._insert.run(evt.id, evt.type, evt.created_at, JSON.stringify(evt.data), evt.tenant_id ?? null);
  }
...
export function emit(type, data, { tenantId } = {}) {
  ...
  const evt = { id: ksuid("evt"), type, created_at: new Date().toISOString(), data, tenant_id: tenantId };
  db.events.push(evt);
