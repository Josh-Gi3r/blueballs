...
import { ksuid, toMinor, fromMinor, ApiError, db, emit, post, balanceOf, collection, subscribeToEvents } from "./lib.js";

export { ksuid, toMinor, fromMinor, ApiError, db, emit, post, balanceOf, collection, subscribeToEvents, randomBytes };
...
// definition, so EURX carries EUR's rate.
//
// The stablecoin entries were missing: assets.js has defined EURX, SGDX and
// MYRX all along and the FX engine prices corridors in them, but this list is
// what POST /v2/accounts validates against, so an account could only ever be
// opened in USDX. A liquidity provider could not hold the currency they were
// being asked to commit. Keep in sync with STABLECOINS in assets.js.
export const RATES = {
  USD: 1, EUR: 1.083, GBP: 1.271, SGD: 0.742, MYR: 0.213,
  USDX: 1, EURX: 1.083, SGDX: 0.742, MYRX: 0.213,
};
...
export const route = (method, pattern, handler, opts = {}) => {
  ...
  routes.push({ method, pattern, parts: pattern.split("/").filter(Boolean), handler, access, successStatus });
};
...
export function positiveMinor(value, field = "amount") {
  ...
  if (minor <= 0n) {
    throw new ApiError("validation-error", 400, `${field} must be greater than zero`, [
...
export function principalId(key) {
  if (!key?.tenant_id) {
    throw new ApiError("internal-error", 500, "Authenticated key has no tenant principal");
  }
...
export const paginate = (rows, url) => {
  ...
  if (!Number.isInteger(requestedLimit) || requestedLimit < 1) {
    throw new ApiError("validation-error", 400, "limit must be a positive integer");
  }
  const limit = Math.min(requestedLimit, 100);
  const after = url.searchParams.get("starting_after");
  const before = url.searchParams.get("ending_before");
  if (after && before) {
    throw new ApiError("validation-error", 400, "Use starting_after or ending_before, not both");
  }
  if (before) {
    const end = rows.findIndex((r) => r.id === before);
    if (end === -1) throw new ApiError("invalid-identifier", 400, `Unknown cursor ${before}`);
