...
import {
  ...
  toMinor, fromMinor, post, balanceOf, emit, RATES, THIN, collection, visibleTo, principalId, positiveMinor } from "../kernel.js";
...
route("GET", "/v2/vaults/:id", ({ params, key }) => vaultView(must(vaults, params.id, "vault", key)));

route("GET", "/v2/vaults", ({ url, key }) => paginate(visibleTo([...vaults.values()], key).map(vaultView), url));
...
route("POST", "/v2/vaults/:id/deposit", ({ params, body, key }) => {
  ...
  emit("vault.deposited", { id: v.id, amount: body.amount, currency: v.currency }, { tenantId: v.owner });
  return vaultView(v);
});
...
route("POST", "/v2/vaults/:id/withdraw", ({ params, body, key }) => {
  ...
  emit("vault.withdrawn", { id: v.id, amount: body.amount, currency: v.currency }, { tenantId: v.owner });
  return vaultView(v);
});
...
function maxDrawableMinor(c) {
  ...
  const valueInCreditCcy = convertMinor(collMinor, c.collateral.currency, c.currency);
  const ltvCapMinor = (valueInCreditCcy * BigInt(c.ltv_bps ?? ltvBasisPoints(c.ltv_max))) / 10_000n;
...
function ltvBasisPoints(value) {
  ...
  if (typeof value !== "number" || !Number.isFinite(value) || value <= 0 || value > 1) {
    throw new ApiError("validation-error", 400, "ltv_max must be a number greater than 0 and no greater than 1");
  }
  const basisPoints = Math.round(value * 10_000);
  if (basisPoints < 1) {
    throw new ApiError("validation-error", 400, "ltv_max must be at least 0.0001");
  }
  return basisPoints;
}
...
route("GET", "/v2/credit/:id", ({ params, key }) => creditView(must(creditLines, params.id, "credit line", key)));

route("GET", "/v2/credit", ({ url, key }) => paginate(visibleTo([...creditLines.values()], key).map(creditView), url));
...
route("POST", "/v2/credit/:id/draw", ({ params, body, key }) => {
  ...
  emit("credit_line.drawn", { id: c.id, amount: body.amount, currency: c.currency }, { tenantId: c.owner });
  return creditView(c);
});
...
route("POST", "/v2/credit/:id/repay", ({ params, body, key }) => {
  ...
  emit("credit_line.repaid", { id: c.id, amount: body.amount, currency: c.currency }, { tenantId: c.owner });
  return creditView(c);
});
...
route("PATCH", "/v2/credit/:id/collateral", ({ params, body, key }) => {
  ...
  emit("credit_line.collateral_adjusted", { id: c.id, collateral: c.collateral }, { tenantId: c.owner });
  return creditView(c);
});
...
route("POST", "/v2/quotes/:id/execute", ({ params, body, key }) => {
  ...
  emit("quote.executed", { id: q.id, from_account: fromAcc.id, to_account: toAcc.id }, { tenantId: q.owner });
  return q;
});
...
route("POST", "/v2/transfers/:id/cancel", ({ params, key }) => {
  ...
  t.canceled_at = now();
  emit("transfer.status_changed", { id: t.id, previous_status: previous, current_status: "canceled" }, { tenantId: t.owner });
  return t;
});
