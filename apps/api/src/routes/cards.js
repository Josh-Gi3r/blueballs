 ...
 *
 *  Storage: `db` (kernel.js) only knows about keys/customers/accounts/recipients/
 *  quotes/transfers/ledger/events/idempotency — cards, authorisations and disputes
 *  are not in that fixed set and kernel.js/lib.js are frozen surfaces, so this file
 *  keeps its own in-process Maps for them (same Map-shaped interface `must()` and
 *  `paginate()` already expect from db.customers etc).
 *
 *  One extra, non-catalogued endpoint lives here too:
...
import {
  ...
  toMinor, fromMinor, randomBytes, emit, post, balanceOf, collection } from "../kernel.js";
...
function defaultLimits(currency, overrides = {}) {
  ...
  for (const k of Object.keys(base)) {
    if (overrides[k]?.amount) {
      toMinor(overrides[k].amount); // validates it's a decimal string
...
route("POST", "/v2/cards", ({ body, key }) => {
  ...
  const account = must(db.accounts, body.account, "account");
  ...
  emit("card.issued", card);
  return card;
});

route("GET", "/v2/cards/:id", ({ params }) => must(cards, params.id, "card"));

route("GET", "/v2/cards", ({ url }) => {
...
route("POST", "/v2/cards/:id/freeze", ({ params, body, key }) => {
  const card = must(cards, params.id, "card");
  ...
  card.freeze = { by: key.id, at: now(), reason };
  emit("card.status_changed", { id: card.id, status: card.status, status_reason: card.status_reason });
  return card;
});
...
route("POST", "/v2/cards/:id/unfreeze", ({ params, key }) => {
  ...
  if (card.freeze && card.freeze.by !== key.id) {
...
route("POST", "/v2/cards/:id/pin", ({ params }) => {
  const card = must(cards, params.id, "card");
...
route("POST", "/v2/cards/:id/reveal", ({ params }) => {
  const card = must(cards, params.id, "card");
  if (card.status === "closed") throw new ApiError("conflict", 409, `Card ${card.id} is closed`);
...
route("PATCH", "/v2/cards/:id/controls", ({ params, body }) => {
  const card = must(cards, params.id, "card");
  ...
  emit("card.controls_updated", { id: card.id, spend_limits: card.spend_limits, merchant_categories: card.merchant_categories });
  return card;
});

route("GET", "/v2/cards/:id/transactions", ({ params, url }) => {
  const card = must(cards, params.id, "card");
...
route("GET", "/v2/cards/:id/statements", ({ params, url }) => {
  const card = must(cards, params.id, "card");
...
route("POST", "/v2/cards/:id/authorisations", ({ params, body }) => {
  const card = must(cards, params.id, "card");
  ...
  const declineWith = (decline_reason, network_response_code) => {
    ...
    emit("authorisation.declined", aut);
    return aut;
  };
  ...
  authorisations.set(aut.id, aut);
  emit("authorisation.created", aut);
  return aut;
});

/* ================= AUTHORISATIONS ================= */

route("GET", "/v2/authorisations/:id", ({ params }) => must(authorisations, params.id, "authorisation"));

route("GET", "/v2/authorisations", ({ url }) => {
...
route("POST", "/v2/authorisations/:id/approve", ({ params }) => {
  const aut = must(authorisations, params.id, "authorisation");
  ...
  emit("authorisation.approved", aut);
  return aut;
});

route("POST", "/v2/authorisations/:id/decline", ({ params, body }) => {
  const aut = must(authorisations, params.id, "authorisation");
  if (aut.status !== "pending") throw new ApiError("conflict", 409, `Authorisation ${aut.id} is ${aut.status}, not pending`);
  aut.status = "declined";
...
route("POST", "/v2/disputes", ({ body }) => {
  need(body, ["authorisation", "reason_code"]);
  const aut = must(authorisations, body.authorisation, "authorisation");
  ...
  disputes.set(d.id, d);
  emit("dispute.created", d);
  return d;
});

route("GET", "/v2/disputes/:id", ({ params }) => must(disputes, params.id, "dispute"));

route("GET", "/v2/disputes", ({ url }) => {
...
route("POST", "/v2/disputes/:id/evidence", ({ params, body }) => {
  const d = must(disputes, params.id, "dispute");
  ...
  emit("dispute.evidence_submitted", { id: d.id, evidence: entry });
  return d;
});
