...
function genPan() {
  ...
  while (digits.length < 15)
    digits += randomBytes(4).readUInt32BE(0).toString().padStart(10, "0");
...
function defaultLimits(currency, overrides = {}) {
  ...
  for (const k of Object.keys(base)) {
    if (overrides[k]?.amount) {
      ...
      const limitCurrency = String(
        overrides[k].currency ?? currency,
      ).toUpperCase();
      if (limitCurrency !== currency) {
        throw new ApiError(
          "validation-error",
          400,
          `spend_limits.${k}.currency must match the card currency ${currency}`,
        );
      }
      base[k] = { amount: overrides[k].amount, currency };
    }
  }
  return base;
}

function enforceAttachedPolicies(card, minor, currency) {
  for (const policy of db.policies?.values?.() ?? []) {
    if (
      !policy.attached.some(
        (attachment) => attachment.type === "card" && attachment.id === card.id,
      )
    )
      continue;
    for (const rule of policy.rules) {
      if (
        ...
        continue;
      if (minor > toMinor(rule.amount)) {
        throw new ApiError(
          "limit-exceeded",
          400,
          `Policy ${policy.id} caps card authorisations at ${rule.amount} ${currency}`,
        );
      }
    }
  }
  ...
  ({ body, key }) => {
    ...
    if (!["virtual", "physical"].includes(body.type)) {
      throw new ApiError(
        "validation-error",
        400,
        "type must be virtual or physical",
      );
    }
    ...
    const card = {
      ...
      merchant_categories: {
        blocked: Array.isArray(body.merchant_categories?.blocked)
          ? body.merchant_categories.blocked
          : [],
        allowed: Array.isArray(body.merchant_categories?.allowed)
          ? body.merchant_categories.allowed
          : [],
      },
...
route("GET", "/v2/cards/:id", ({ params, key }) =>
  must(cards, params.id, "card", key),
);
...
route("POST", "/v2/cards/:id/freeze", ({ params, body, key }) => {
  ...
  if (card.status === "closed")
    throw new ApiError("conflict", 409, "A closed card cannot be frozen");
  ...
  emit(
    "card.status_changed",
    { id: card.id, status: card.status, status_reason: card.status_reason },
    { tenantId: card.owner },
  );
  return card;
});
...
route("POST", "/v2/cards/:id/unfreeze", ({ params, key }) => {
  ...
  if (card.status !== "frozen")
    throw new ApiError("conflict", 409, `Card ${card.id} is not frozen`);
...
/** PIN management belongs behind a configured card-processor adapter. The
 * reference runtime fails closed instead of returning a URL that cannot work. */
route("POST", "/v2/cards/:id/pin", ({ params, key }) => {
  const card = must(cards, params.id, "card", key);
  if (card.status === "closed")
    throw new ApiError("conflict", 409, `Card ${card.id} is closed`);
  throw new ApiError(
    "service-unavailable",
    503,
    "Card PIN management requires a configured processor adapter",
  );
});

/** PAN reveal belongs behind a configured PCI widget/processor adapter. */
route("POST", "/v2/cards/:id/reveal", ({ params, key }) => {
  ...
  throw new ApiError(
    "service-unavailable",
    503,
    "Card reveal requires a configured PCI-compliant processor adapter",
  );
});
  ...
  if (body.spend_limits) {
    for (const k of ["per_authorization", "daily", "monthly"]) {
      ...
      if (v?.amount) {
        ...
        const currency = String(v.currency ?? card.currency).toUpperCase();
        if (currency !== card.currency) {
          throw new ApiError(
            "validation-error",
            400,
            `spend_limits.${k}.currency must match the card currency ${card.currency}`,
          );
        }
        card.spend_limits[k] = { amount: v.amount, currency: card.currency };
      }
    }
  }
  if (body.merchant_categories) {
    if (Array.isArray(body.merchant_categories.blocked))
      card.merchant_categories.blocked = body.merchant_categories.blocked;
    if (Array.isArray(body.merchant_categories.allowed))
      card.merchant_categories.allowed = body.merchant_categories.allowed;
  }
  emit(
    "card.controls_updated",
    {
      id: card.id,
      spend_limits: card.spend_limits,
...
route("GET", "/v2/cards/:id/statements", ({ params, url, key }) => {
  const card = must(cards, params.id, "card", key);
  const settled = [...authorisations.values()].filter(
    (a) => a.card === card.id && a.status === "settled",
  );
  ...
  const statements = [...byPeriod.entries()]
    .map(([period, txns]) => {
      const totalMinor = txns.reduce(
        (n, t) => n + toMinor(t.amount.amount),
        0n,
      );
      return {
  ...
  ({ params, body, key }) => {
    ...
    if (currency !== card.currency) {
      throw new ApiError(
        "validation-error",
        400,
        `Card ${card.id} settles in ${card.currency}, not ${currency}`,
      );
    }
    const minor = positiveMinor(body.amount);
    enforceAttachedPolicies(card, minor, currency);
    ...
    if (card.status === "pending_activation")
      return declineWith("card_inactive", "05");

    if (card.merchant_categories.blocked.includes(base.merchant.mcc))
      return declineWith("merchant_category_blocked", "57");
    if (
      card.merchant_categories.allowed.length &&
      !card.merchant_categories.allowed.includes(base.merchant.mcc)
    ) {
    ...
    const reservedStatuses = new Set(["pending", "approved", "settled"]);
    ...
    const monthlyLimit = toMinor(card.spend_limits.monthly.amount);
    const monthSpent = [...authorisations.values()]
      .filter(
        (a) =>
          a.card === card.id &&
          reservedStatuses.has(a.status) &&
      ...
      .reduce((n, a) => n + toMinor(a.amount.amount), 0n);
    if (monthSpent + minor > monthlyLimit) {
      throw new ApiError(
        "limit-exceeded",
        400,
        `Authorisation would push this month's spend to ${fromMinor(monthSpent + minor)} ${currency}, over the monthly limit of ${card.spend_limits.monthly.amount} ${currency}`,
      );
    }
...
route("GET", "/v2/authorisations/:id", ({ params, key }) =>
  must(authorisations, params.id, "authorisation", key),
);
...
route("POST", "/v2/authorisations/:id/approve", ({ params, key }) => {
  ...
  if (aut.status !== "pending")
    throw new ApiError(
      "conflict",
      409,
      `Authorisation ${aut.id} is ${aut.status}, not pending`,
    );
  ...
  post(
    [
      ...
      {
        account: "clearing:card_authorisations",
        currency: aut.amount.currency,
        amount: minor,
      },
    ],
  ...
  ({ body, key }) => {
    ...
    if (aut.status !== "settled") {
      throw new ApiError(
        "validation-error",
        400,
        `Only settled authorisations can be disputed (this one is ${aut.status})`,
      );
    }
    ...
    if (!reason) {
      throw new ApiError(
        "validation-error",
        400,
        `Unknown reason_code ${body.reason_code}`,
        [
          {
            field: "reason_code",
            message: `must be one of: ${Object.keys(DISPUTE_REASON_CODES).join(", ")}`,
    ...
    const existing = [...disputes.values()].find(
      (d) => d.authorisation === aut.id && d.decision === null,
    );
    if (existing)
      throw new ApiError(
        "conflict",
        409,
        `Authorisation ${aut.id} already has an open dispute ${existing.id}`,
      );

    const d = {
      id: ksuid("dsp"),
      authorisation: aut.id,
      card: aut.card,
      customer: aut.customer,
      amount: aut.amount,
      ...
      decision: null,
      deadline: new Date(
        Date.now() + reason.days_to_respond * 86_400_000,
      ).toISOString(),
      evidence: [],
...
route("GET", "/v2/disputes/:id", ({ params, key }) =>
  must(disputes, params.id, "dispute", key),
);
...
route("POST", "/v2/disputes/:id/evidence", ({ params, body, key }) => {
  ...
  if (d.decision !== null)
    throw new ApiError(
      "conflict",
      409,
      `Dispute ${d.id} is already closed (${d.decision})`,
    );
  if (Date.now() > Date.parse(d.deadline)) {
    throw new ApiError(
      "conflict",
      409,
      `The evidence deadline for dispute ${d.id} passed at ${d.deadline}`,
    );
  }
