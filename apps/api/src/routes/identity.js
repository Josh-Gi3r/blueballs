...
import {
  ...
  emit, randomBytes, balanceOf, fromMinor, RAILS, visibleTo, collection, principalId,
} from "../kernel.js";
...
route("POST", "/v2/keys", ({ body, key }) => {
  ...
  if (requestedScope !== key.scope) {
    throw new ApiError("forbidden", 403, "A key can only create another key with the same scope");
  }
  const secret = "bb_sandbox_" + randomBytes(18).toString("base64url");
  const rec = {
    id: ksuid("key"), tenant_id: principalId(key), email: key.email, scope: requestedScope,
...
route("GET", "/v2/keys/:id", ({ params, key }) => {
  const rec = [...db.keys.values()].find((k) => k.id === params.id && k.tenant_id === principalId(key));
  if (!rec) throw new ApiError("not-found", 404, `No key ${params.id}`);
  return rec;
});

route("DELETE", "/v2/keys/:id", ({ params, key }) => {
  const entry = [...db.keys.entries()].find(([, v]) => v.id === params.id && v.tenant_id === principalId(key));
...
route("DELETE", "/v2/keys", ({ key }) => {
  const hashes = [...db.keys.entries()].filter(([, v]) => v.tenant_id === principalId(key)).map(([h]) => h);
  for (const h of hashes) db.keys.delete(h);
  emit("key.revoked_all", { tenant_id: principalId(key), count: hashes.length }, { tenantId: principalId(key) });
...
route("GET", "/v2/applications/:id", ({ params, key }) => must(db.applications, params.id, "application", key));
...
route("POST", "/v2/applications/:id/individuals", ({ params, body, key }) => {
  const app = must(db.applications, params.id, "application", key);
  guardMutable(app);
  need(body, ["name"]);
  ...
  app.updated_at = now();
  emit("application.individual_added", { id: app.id, individual: ind.id }, { tenantId: app.owner });
  return ind;
}, { created: true });

route("PATCH", "/v2/applications/:id/individuals/:iid", ({ params, body, key }) => {
  const app = must(db.applications, params.id, "application", key);
  guardMutable(app);
...
route("POST", "/v2/applications/:id/documents", ({ params, body, key }) => {
  const app = must(db.applications, params.id, "application", key);
  guardMutable(app);
  ...
  app.updated_at = now();
  emit("application.document_uploaded", { id: app.id, document: doc.id }, { tenantId: app.owner });
  return doc;
}, { created: true });
...
route("POST", "/v2/applications/:id/submit", ({ params, key }) => {
  ...
  app.updated_at = now();
  emit("application.status_changed", { id: app.id, previous_status: "pending", current_status: "submitted" }, { tenantId: app.owner });
  return app;
});
...
route("POST", "/v2/applications/:id/attestation", ({ params, body, key }) => {
  ...
  app.updated_at = now();
  emit("application.attestation_submitted", { id: app.id, attestation: att.id }, { tenantId: app.owner });
  return att;
});
...
route("POST", "/v2/applications/:id/edd", ({ params, body, key }) => {
  ...
  if (app.status !== previous) {
    emit("application.status_changed", { id: app.id, previous_status: previous, current_status: app.status }, { tenantId: app.owner });
  }
  return app;
});
...
route("POST", "/v2/accounts/:id/details", ({ params, body, key }) => {
  const a = must(db.accounts, params.id, "account", key);
  need(body, ["rail"]);
...
route("GET", "/v2/accounts/:id/details", ({ params, url, key }) => {
  ...
  return paginate(visibleTo([...db.details.values()].filter((d) => d.account === params.id), key), url);
});

route("GET", "/v2/details/:id", ({ params, key }) => must(db.details, params.id, "account details", key));
