...
import {
  ...
  emit, collection, now, must, visibleTo, principalId,
} from "../kernel.js";
 ...
 *  EURX/USDX funded a different account than USDX/EURX while creditProviders()
 *  normalised for the payout, so a provider could be paid out of a pool their
 *  capital was never in. */
export const poolAccount = (pair, currency) => `lp:${normPair(pair.toUpperCase())}:${currency}`;
...
route("POST", "/v2/fx/lp", ({ body, key }) => {
  ...
  const p = {
    ...
    status: "active", created_at: now(), owner: principalId(key),
  };
  positions.set(p.id, p);
  emit("fx.lp.committed", { id: p.id, pair: p.pair, currency: cur }, { tenantId: p.owner });
...
export function creditProviders(pair, currency, spreadMinor, fillId) {
  ...
  for (const earning of credited) {
    emit("fx.lp.earned", { fill: fillId, position: earning.position, amount: earning.amount, currency }, { tenantId: earning.owner });
  }
  return credited;
}
...
route("POST", "/v2/fx/lp/:id/withdraw", ({ params, key }) => {
  ...
  positions.set(p.id, p);
  emit("fx.lp.withdrawn", { id: p.id, amount: p.withdrawn }, { tenantId: p.owner });
