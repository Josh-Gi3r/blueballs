...
import {
  ...
  ApiError, toMinor, fromMinor, post, balanceOf, emit, RAILS, randomBytes, collection, visibleTo, principalId, positiveMinor } from "../kernel.js";
...
function executeWalletSend(a) {
  ...
  emit("wallet.sent", { wallet: w.id, amount: a.amount.amount, currency: a.amount.currency, to: a.to, approval: a.id }, { tenantId: w.owner });
}
...
route("GET", "/v2/policies/:id", ({ params, key }) => must(policies, params.id, "policy", key));
...
route("POST", "/v2/policies/:id/rules", ({ params, body, key }) => {
...
route("POST", "/v2/policies/:id/attach", ({ params, body, key }) => {
  ...
  if (body.type === "account") must(db.accounts, body.id, "account", key);
...
route("POST", "/v2/approval-chains", ({ body, key }) => {
  ...
  const approvers = Array.isArray(body.approvers) ? [...new Set(body.approvers)] : [];
  const tenantKeyIds = new Set(
    [...db.keys.values()].filter((candidate) => candidate.tenant_id === principalId(key)).map((candidate) => candidate.id),
  );
  const unknownApprover = approvers.find((approver) => !tenantKeyIds.has(approver));
  if (unknownApprover) {
    throw new ApiError("validation-error", 400, `Approver ${unknownApprover} is not an active key in this tenant`);
  }
  const steps = body.steps ?? (approvers.length || 1);
  if (!Number.isInteger(steps) || steps < 1) {
    throw new ApiError("validation-error", 400, "steps must be a positive integer");
  }
  if (approvers.length && steps > approvers.length) {
    throw new ApiError("validation-error", 400, "steps cannot exceed the number of unique approvers");
  }
  const c = {
    id: ksuid("apc"),
    name: body.name,
...
route("GET", "/v2/approval-chains/:id", ({ params, key }) => must(chains, params.id, "approval chain", key));

/** Pending approvals inbox. */
route("GET", "/v2/approvals", ({ url, key }) => {
  const status = url.searchParams.get("status") ?? "pending";
  const rows = visibleTo([...approvalQueue.values()], key)
...
route("POST", "/v2/approvals/:id/approve", ({ params, body, key }) => {
  ...
  if (chain.approvers.length && !chain.approvers.includes(key.id)) {
    throw new ApiError("forbidden", 403, `Key ${key.id} is not an approver on chain ${chain.id}`);
  }
  if (a.decisions.some((decision) => decision.approver === key.id)) {
    throw new ApiError("conflict", 409, `Key ${key.id} has already decided approval ${a.id}`);
  }
  ...
  emit("approval.step_recorded", { id: a.id, status: a.status, decisions: a.decisions.length }, { tenantId: a.owner });
  return a;
});
...
route("GET", "/v2/orgs/:id", ({ params, key }) => must(orgs, params.id, "organisation", key));
...
route("POST", "/v2/orgs/:id/members", ({ params, body, key }) => {
  const o = must(orgs, params.id, "organisation", key);
  ...
  o.members.push(m);
  emit("org.member_invited", { org: o.id, member: m.id, role: m.role }, { tenantId: o.owner });
  return m;
}, { created: true });
...
route("DELETE", "/v2/orgs/:id/members/:mid", ({ params, key }) => {
  ...
  emit("org.member_removed", { org: o.id, member: params.mid }, { tenantId: o.owner });
...
route("POST", "/v2/wallets/:id/credit", ({ params, body, key }) => {
  ...
  if (currency !== w.currency) {
    throw new ApiError("validation-error", 400, `Wallet ${w.id} is denominated in ${w.currency}, not ${currency}`);
  }
  ...
  emit("wallet.credited", { wallet: w.id, amount: body.amount, currency }, { tenantId: w.owner });
  return { ...w, balance: { amount: fromMinor(balanceOf(w.id, currency)), currency } };
});
...
route("POST", "/v2/wallets/:id/send", ({ params, body, key }) => {
  ...
  if (chain && chain.threshold.currency === currency && minor >= toMinor(chain.threshold.amount)) {
    const a = {
      ...
      status: "pending", decisions: [], required: chain.steps, owner: principalId(key), created_at: now(),
    };
  ...
  emit("wallet.sent", { wallet: w.id, amount: body.amount, currency, to: body.to }, { tenantId: w.owner });
 ...
 * ========================================================================= */
route("GET", "/v2/recipients/:id", ({ params, key }) => must(db.recipients, params.id, "recipient", key));
 ...
 * ========================================================================= */
route("POST", "/v2/recipients/:id/destinations", ({ params, body, key }) => {
  const r = must(db.recipients, params.id, "recipient", key);
  need(body, ["rail", "name"]);
  ...
  db.recipients.set(r.id, r);
  emit("destination.created", { recipient: r.id, destination: d.id, rail: d.rail }, { tenantId: r.owner });
  return d;
}, { created: true });
