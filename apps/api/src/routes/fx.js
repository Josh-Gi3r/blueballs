...
import { route, ApiError, need, toMinor, fromMinor, ksuid, db, post, balanceOf, emit, collection, now, must, visibleTo, principalId } from "../kernel.js";
...
route("POST", "/v2/ramps/on", ({ body, key }) => {
  ...
  const r = {
    ...
    issuer: STABLECOINS[to].issuer, status: "settled", created_at: now(), owner: principalId(key),
...
route("POST", "/v2/ramps/off", ({ body, key }) => {
  ...
  const r = {
    ...
    rate: "1.000000", spread_bps: 0,
    issuer: STABLECOINS[from].issuer, status: "settled", created_at: now(), owner: principalId(key),
  };
  ramps.set(r.id, r);
  emit("ramp.settled", r, { tenantId: r.owner });
  return r;
