 ...
 */
import { route, db, ksuid, need, must, paginate, now, ApiError, toMinor, RATES, collection, visibleTo, principalId, positiveMinor } from "../kernel.js";
...
route("POST", "/v2/links", ({ body, key }) => {
  ...
  if (body.amount !== undefined && body.amount !== null) positiveMinor(body.amount); // open-amount links omit this

  const ttlSeconds = Number(body.expires_in_seconds ?? 7 * 24 * 60 * 60); // default 7 days
  if (!Number.isInteger(ttlSeconds) || ttlSeconds < 1) {
    throw new ApiError("validation-error", 400, "expires_in_seconds must be a positive integer");
  }
  const id = ksuid("lnk");
  const l = {
    id,
...
route("POST", "/v2/mandates", ({ body, key }) => {
  ...
  const currency = String(body.currency).toUpperCase();
  if (!RATES[currency]) throw new ApiError("validation-error", 400, `${currency} is not a supported currency`);
  if (body.max_amount !== undefined && body.max_amount !== null) positiveMinor(body.max_amount, "max_amount");
...
route("GET", "/v2/mandates/:id", ({ params, key }) => must(mandates, params.id, "mandate", key));
...
route("POST", "/v2/subscriptions", ({ body, key }) => {
  ...
  if (mandate.customer !== customer.id) {
    throw new ApiError("validation-error", 400, `Mandate ${mandate.id} does not belong to customer ${customer.id}`);
  }
  if (mandate.currency !== currency) {
    throw new ApiError("validation-error", 400, `Mandate ${mandate.id} is denominated in ${mandate.currency}, not ${currency}`);
  }
  if (mandate.max_amount && minor > toMinor(mandate.max_amount.amount)) {
    throw new ApiError("limit-exceeded", 400, `Subscription amount exceeds mandate maximum of ${mandate.max_amount.amount} ${currency}`);
  }
  ...
  if (!Number.isInteger(intervalCount) || intervalCount < 1) {
    throw new ApiError("validation-error", 400, "interval_count must be a positive integer");
  }
