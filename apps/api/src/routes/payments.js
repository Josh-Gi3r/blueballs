 ...
 *
 *  Owns exactly this file (M2 fan-out contract, see kernel.js header). Payment
 *  links, mandates and subscriptions use the shared SQLite-backed collection
 *  adapter. QR generate/decode is stateless by nature (decode must accept any
...
const crcHex = (str) =>
  crc16ccitt(str).toString(16).toUpperCase().padStart(4, "0");
...
const ISO4217_ALPHA = Object.fromEntries(
  Object.entries(ISO4217_NUMERIC).map(([a, n]) => [n, a]),
);
  ...
  while (i < s.length) {
    if (i + 4 > s.length)
      throw new ApiError(
        "validation-error",
        400,
        "Malformed TLV: truncated tag/length near position " + i,
      );
    ...
    if (!/^\d{2}$/.test(lenStr))
      throw new ApiError(
        "validation-error",
        400,
        `Malformed TLV: non-numeric length for tag ${id}`,
      );
    ...
    if (i + len > s.length)
      throw new ApiError(
        "validation-error",
        400,
        `Malformed TLV: value for tag ${id} runs past end of payload`,
      );
...
route("POST", "/v2/qr/generate", ({ body }) => {
  ...
  if (!numeric) {
    throw new ApiError(
      "validation-error",
      400,
      `${currency} has no ISO 4217 numeric code, which EMVCo tag 53 requires`,
      [
        {
          field: "currency",
          message: `try one of: ${Object.keys(ISO4217_NUMERIC).join(", ")}`,
  ...
  if (!/^[A-Z]{2}$/.test(country)) {
    throw new ApiError(
      ...
      [
        {
          field: "country",
          message: 'expected two letters, e.g. "SG"',
          code: "invalid_format",
        },
      ],
    );
  ...
  if (overLong.length) {
    throw new ApiError(
      "validation-error",
      400,
      "Field exceeds the EMVCo maximum length",
      overLong.map((field) => ({
        field,
        message: `max ${MAX_LEN[field]} characters`,
        code: "too_long",
      })),
    );
  }
  ...
  if (dynamic) positiveMinor(body.amount);

  const merchantId = body.merchant_id
    ? String(body.merchant_id)
    : ksuid("mid").slice(-16);
  const mcc = body.mcc
    ? String(body.mcc).replace(/\D/g, "").padStart(4, "0").slice(0, 4)
    : "0000";
...
route("POST", "/v2/qr/decode", ({ body }) => {
  ...
  if (payload.length < 8 || payload.slice(-8, -4) !== "63" + "04") {
    throw new ApiError(
      "validation-error",
      400,
      "Payload does not terminate with a well-formed CRC tag (6304xxxx)",
      [
        {
          field: "payload",
          message: "expected the last field to be tag 63, length 04",
          code: "missing_crc_tag",
        },
      ],
    );
  }
  ...
  if (crcComputed !== crcClaimed) {
    throw new ApiError(
      "validation-error",
      400,
      "CRC check failed — payload is corrupted or has been tampered with",
      [
        {
          field: "payload",
          message: `expected CRC ${crcComputed}, payload carries ${crcClaimed}`,
          code: "crc_mismatch",
        },
      ],
    );
...
route("GET", "/v2/links/:id", ({ params, key }) => {
  const l = must(links, params.id, "payment link", key);
...
route(
  ...
  ({ body, key }) => {
    ...
    if (body.max_amount !== undefined && body.max_amount !== null)
      positiveMinor(body.max_amount, "max_amount");
...
route("GET", "/v2/mandates/:id", ({ params, key }) =>
  must(mandates, params.id, "mandate", key),
);
...
function computeNextRun(startISO, interval, count) {
  ...
  if (Number.isNaN(d.getTime()))
    throw new ApiError(
      "validation-error",
      400,
      "start_date must be a valid RFC 3339 timestamp",
    );
...
route(
  ...
  ({ body, key }) => {
    ...
    const customer = must(db.customers, body.customer, "customer", key);
    const mandate = must(mandates, body.mandate, "mandate", key);
    if (mandate.status !== "active") {
      throw new ApiError(
        "conflict",
        409,
        `Mandate ${mandate.id} is ${mandate.status}, not active`,
      );
    }
    const currency = String(body.currency).toUpperCase();
    if (!RATES[currency])
    ...
    if (mandate.customer !== customer.id) {
      throw new ApiError(
        "validation-error",
        400,
        `Mandate ${mandate.id} does not belong to customer ${customer.id}`,
      );
    }
    if (mandate.currency !== currency) {
      throw new ApiError(
        "validation-error",
        400,
        `Mandate ${mandate.id} is denominated in ${mandate.currency}, not ${currency}`,
      );
    }
    if (mandate.max_amount && minor > toMinor(mandate.max_amount.amount)) {
      throw new ApiError(
        "limit-exceeded",
        400,
        `Subscription amount exceeds mandate maximum of ${mandate.max_amount.amount} ${currency}`,
      );
    }
    ...
    if (!Number.isInteger(intervalCount) || intervalCount < 1) {
      throw new ApiError(
        "validation-error",
        400,
        "interval_count must be a positive integer",
      );
    }
...
route("GET", "/v2/subscriptions", ({ url, key }) =>
  paginate(visibleTo([...subscriptions.values()], key), url),
);
