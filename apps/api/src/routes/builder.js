...
const MARKET_CURRENCIES = {
  ...
  EU: ["EUR", "GBP", "USD"],
  US: ["USD", "USDC"],
  GLOBAL: ["USD", "EUR", "GBP", "SGD"],
};
const MARKET_RAILS = {
  SG: ["paynow", "wire"],
  MY: ["wire"],
  GB: ["faster_payments", "sepa"],
  EU: ["sepa_instant", "sepa"],
  US: ["ach", "wire"],
...
function cleanText(value, field, max = 500) {
  ...
  if (!text)
    throw new ApiError("validation-error", 400, `${field} is required`);
  if (text.length > max)
    throw new ApiError(
      "validation-error",
      400,
...
function uniqueCodes(value, allowed, field, fallback, max = 8) {
  ...
  const codes = [
    ...new Set(input.map((item) => String(item).trim().toUpperCase())),
  ];
  if (!codes.length || codes.length > max) {
    throw new ApiError(
      "validation-error",
      400,
      `${field} must contain between 1 and ${max} values`,
    );
  }
  ...
  if (unknown)
    throw new ApiError(
      "validation-error",
      400,
      `Unsupported ${field} value ${unknown}`,
    );
  return codes;
}
...
function inferCapabilities(brief) {
  ...
  const hints = [
    ["cards", /card|spend/],
    ["wallets", /wallet|crypto|stablecoin/],
    ["fx", /fx|exchange|currency|cross.border/],
    ["savings", /saving|vault/],
    ["business", /business|company|sme|merchant/],
    ["payment_links", /payment link|qr|invoice/],
    ["webhooks", /webhook|api/],
  ];
  for (const [capability, pattern] of hints)
    if (pattern.test(lower)) selected.push(capability);
...
function blueprintFrom(body) {
  ...
  const fallbackCurrencies = [
    ...new Set(markets.flatMap((market) => MARKET_CURRENCIES[market])),
  ].slice(0, 4);
  const currencies = uniqueCodes(
    body.currencies,
    new Set(Object.keys(RATES)),
    "currencies",
    fallbackCurrencies,
    6,
  );
  ...
  const possibleRails = [
    ...new Set(markets.flatMap((market) => MARKET_RAILS[market])),
  ];
  const rails = uniqueCodes(
    ...
    .map((item) => item.toLowerCase())
    .filter((rail) => currencies.includes(RAILS[rail].currency));
  const accent = /^#[0-9a-f]{6}$/i.test(body.brand?.accent ?? "")
    ? body.brand.accent
    : "#0868FF";
  const personality = String(
    body.brand?.personality ?? "clear, credible and human",
  )
    .trim()
    .slice(0, 120);
  ...
  return {
    ...
    providers: {
      ...
      cards: capabilities.includes("cards")
        ? "sandbox card records"
        : "disabled",
      wallets: capabilities.includes("wallets")
        ? "sandbox wallet records"
        : "disabled",
...
function projectAccounts(project) {
  return (project.resources?.accounts ?? [])
    .map((id) => {
...
function publicProject(project) {
  ...
  const customers = (project.resources?.customers ?? [])
    .map((id) => db.customers.get(id))
    .filter(Boolean)
    .map(({ owner: _customerOwner, ...customer }) => customer);
  const recentJourneys = visibleTo([...journeys.values()], {
  ...
  return {
    ...safe,
    customers,
    accounts: projectAccounts(project).map(
      ({ owner: _accountOwner, ...account }) => account,
    ),
    ...
    const project = {
      ...
      build: {
        ...
        steps: [
          ...
          ["financial", "Financial primitives"],
...
route("GET", "/v2/builder/projects", ({ url, key }) =>
  paginate(
...
route("GET", "/v2/builder/projects/:id", ({ params, key }) =>
  publicProject(must(projects, params.id, "sandbox project", key)),
);
...
route("PATCH", "/v2/builder/projects/:id", ({ params, body, key }) => {
  ...
  if (project.status !== "blueprint_ready") {
    throw new ApiError(
      "conflict",
      409,
      "A provisioned blueprint is immutable; create a new project to change financial configuration",
    );
  }
  project.blueprint = blueprintFrom({
    ...project.blueprint,
    ...body,
    brand: { ...project.blueprint.brand, ...body.brand },
  });
...
route("POST", "/v2/builder/projects/:id/provision", ({ params, key }) => {
  ...
  if (project.status !== "blueprint_ready")
    throw new ApiError(
      "conflict",
      409,
      `Project ${project.id} is ${project.status}`,
    );
  ...
  for (let index = 0; index < SEED_USERS.length; index++) {
    ...
    for (const currency of currencies) {
      const account = {
        ...
        type: "holding",
        status: "open",
        label: `${name.split(" ")[0]}'s ${currency}`,
        details: {
          type: "sandbox",
          reference: `${project.id}:${currency}:${index + 1}`,
        },
        created_at: stamp,
        owner: project.owner,
      };
      ...
      post(
        [
          ...
          {
            account: `external:sandbox:${project.id}`,
            currency,
            amount: -minor,
          },
        ],
  ...
  project.build = {
    state: "complete",
    steps: project.build.steps.map((step) => ({
      ...step,
      status: "complete",
      completed_at: stamp,
    })),
  ...
  project.environment = {
    ...
    mode: "sandbox",
    state: "ready",
    path: `/sandbox?project=${project.id}`,
    real_money: false,
    created_at: stamp,
  };
...
route(
  "POST",
  "/v2/builder/projects/:id/test-payments",
  ({ params, body, key }) => {
    const project = must(projects, params.id, "sandbox project", key);
    if (project.status !== "ready")
      throw new ApiError(
        "conflict",
        409,
        "Provision the project before running a payment",
      );
    ...
    if (!project.resources.accounts.includes(body.from_account)) {
      throw new ApiError(
        "not-found",
        404,
        `No project account ${body.from_account}`,
      );
    }
    ...
    if (balanceOf(account.id, account.currency) < minor) {
      throw new ApiError(
        ...
        400,
        `Account holds ${fromMinor(balanceOf(account.id, account.currency))} ${account.currency}`,
      );
    }
    const recipient = cleanText(body.recipient, "recipient", 120);
    const journey = {
      id: ksuid("jny"),
      object: "sandbox_test_payment",
      project: project.id,
      from_account: account.id,
      recipient,
      amount: { amount: body.amount, currency: account.currency },
      rail:
        body.rail && project.blueprint.rails.includes(body.rail)
          ? body.rail
          : (project.blueprint.rails[0] ?? "sandbox"),
      status: "settled",
      owner: project.owner,
      created_at: now(),
    };
    post(
      [
        ...
        {
          account: `external:sandbox:payout:${project.id}`,
          currency: account.currency,
          amount: minor,
        },
      ],
