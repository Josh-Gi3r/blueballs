...
import {
  ...
  fromMinor, toMinor, post, balanceOf, RAILS, emit, collection, visibleTo, principalId,
  subscribeToEvents } from "../kernel.js";
import { WEBHOOK_DELIVERY_MODE, validateWebhookUrl, fetchWebhook } from "../webhook-egress.js";
...
async function deliver(wh, evt, opts = {}) {
  ...
  const record = {
    ...
    owner: wh.owner, status: "pending", response_code: null, error: null,
    attempted_at: now(),
  };
...
route("POST", "/v2/webhooks", ({ body, key }) => {
  ...
  const wh = {
    id: ksuid("whk"), object: "webhook_target", url: targetUrl, events,
    status: "enabled",
    delivery_mode: WEBHOOK_DELIVERY_MODE,
    secret: "whsec_" + randomBytes(24).toString("base64url"),
    owner: principalId(key), client_reference_id: body.client_reference_id ?? null,
    created_at: now(),
  };
...
route("GET", "/v2/webhooks/:id", ({ params, key }) => withoutSecret(must(webhooks, params.id, "webhook target", key)));

route("GET", "/v2/webhooks", ({ url, key }) => paginate(visibleTo([...webhooks.values()], key).map(withoutSecret), url));
...
route("PATCH", "/v2/webhooks/:id", ({ params, body, key }) => {
  ...
  if (body.status !== undefined) {
    ...
    if (body.status === "enabled" && WEBHOOK_DELIVERY_MODE !== "allowlist") {
      throw new ApiError("conflict", 409, "Webhook delivery is disabled on this host");
    }
...
route("POST", "/v2/webhooks/deliveries/:did/replay", async ({ params, key }) => {
  if (WEBHOOK_DELIVERY_MODE !== "allowlist") {
    throw new ApiError("service-unavailable", 503, "Webhook delivery is disabled on this host");
  }
...
route("POST", "/v2/sandbox/payments", ({ body, key }) => {
...
route("POST", "/v2/sandbox/onboarding", ({ body, key }) => {
  need(body, ["scenario"]);
...
route("GET", "/v2/events/:id", ({ params, key }) => {
  const evt = [...db.events].find((e) => e.id === params.id && e.tenant_id === principalId(key));
...
route("POST", "/v2/statements", ({ body, key }) => {
  need(body, ["account"]);
  const acc = must(db.accounts, body.account, "account", key);
...
route("GET", "/v2/statements/:id", ({ params, key }) => must(statements, params.id, "statement", key));
...
route("GET", "/v2/fees/config", ({ key }) => {
  const cfg = feeConfigs.get(principalId(key)) ?? { payout_account: null, updated_at: null };
