...
import {
  ...
  fromMinor, toMinor, post, balanceOf, RAILS, emit, collection } from "../kernel.js";
...
async function deliver(wh, evt, opts = {}) {
  ...
  const payload = {
    id: evt.id, type: evt.type, created: evt.created_at,
    api_version: API_VERSION, delivery_id: deliveryId, data: evt.data,
  };
  const body = JSON.stringify(payload);
  const signature = createHmac("sha256", wh.secret).update(`${timestamp}.${body}`).digest("hex");

  const record = {
    id: deliveryId, object: "webhook_delivery",
    webhook: wh.id, event_id: evt.id, event_type: evt.type, data: evt.data,
    url: wh.url, replay: !!opts.replay, replayed_from: opts.replayedFrom ?? null,
    status: "pending", response_code: null, error: null,
    attempted_at: now(),
  };
...
/** Every event that has already been dispatched to matching webhooks, so the
 *  background poller (below) never double-fires an event `notify()` already sent. */
...
route("POST", "/v2/webhooks", ({ body, key }) => {
  ...
  if (!/^https?:\/\//.test(body.url)) {
    throw new ApiError("validation-error", 400, "url must start with http:// or https://", [
      { field: "url", message: "must be an http(s) URL", code: "invalid_format" },
    ]);
  }
  const events = Array.isArray(body.events) && body.events.length ? body.events : ["*"];
  const wh = {
    id: ksuid("whk"), object: "webhook_target", url: body.url, events, status: "enabled",
    secret: "whsec_" + randomBytes(24).toString("base64url"),
    owner: key.id, client_reference_id: body.client_reference_id ?? null,
    created_at: now(),
  };
...
route("GET", "/v2/webhooks/:id", ({ params }) => must(webhooks, params.id, "webhook target"));

route("GET", "/v2/webhooks", ({ url }) => paginate([...webhooks.values()], url));

route("PATCH", "/v2/webhooks/:id", ({ params, body }) => {
  const wh = must(webhooks, params.id, "webhook target");
  ...
  if (body.events !== undefined) {
    if (!Array.isArray(body.events) || !body.events.length) {
      throw new ApiError("validation-error", 400, "events must be a non-empty array");
    }
    wh.events = body.events;
  }
  if (body.status !== undefined) {
    if (!["enabled", "disabled"].includes(body.status)) {
      throw new ApiError("validation-error", 400, "status must be enabled or disabled");
    }
...
route("DELETE", "/v2/webhooks/:id", ({ params }) => {
  const wh = must(webhooks, params.id, "webhook target");
  webhooks.delete(params.id);
  return { ...wh, deleted: true };
});

route("GET", "/v2/webhooks/:id/deliveries", ({ params, url }) => {
  must(webhooks, params.id, "webhook target");
  const rows = [...deliveries.values()]
    .filter((d) => d.webhook === params.id)
    .sort((a, b) => a.attempted_at.localeCompare(b.attempted_at));
  return paginate(rows, url);
});

route("POST", "/v2/webhooks/deliveries/:did/replay", async ({ params }) => {
  const original = must(deliveries, params.did, "delivery");
  const wh = must(webhooks, original.webhook, "webhook target");
  const evtLike = { id: original.event_id, type: original.event_type, created_at: original.attempted_at, data: original.data };
...
const ONBOARDING_SCENARIOS = {
  "onboarding.approved": {
    awaiting_advance: false, status: "completed", decision: "approved",
    does: "Onboarding completes and is approved immediately.",
  },
  "onboarding.manual_review": {
    awaiting_advance: true, status: "in_review", decision: null,
  ...
  "onboarding.compliance_hold": {
    awaiting_advance: true, status: "compliance_hold", decision: null,
    does: "Onboarding is placed on compliance hold. Pauses with awaiting_advance: true until you call advance.",
  },
  "onboarding.declined": {
    awaiting_advance: false, status: "completed", decision: "declined",
...
route("GET", "/v2/sandbox/scenarios", () => ({
  ...
  data: [
    ...Object.entries(PAYMENT_SCENARIOS).map(([name, s]) => ({
      name, kind: "payment", awaiting_advance: s.awaiting_advance, does: s.does,
    })),
    ...Object.entries(ONBOARDING_SCENARIOS).map(([name, s]) => ({
      name, kind: "onboarding", awaiting_advance: s.awaiting_advance, does: s.does,
    })),
  ],
}));
...
route("POST", "/v2/sandbox/payments", ({ body }) => {
  need(body, ["scenario", "amount", "currency"]);
  const scenario = PAYMENT_SCENARIOS[body.scenario];
  if (!scenario) {
    throw new ApiError("validation-error", 400, `Unknown scenario ${body.scenario}`, [
      { field: "scenario", message: `try one of: ${Object.keys(PAYMENT_SCENARIOS).join(", ")}`, code: "unknown_scenario" },
    ]);
  }
  const simId = body.simulation_id || ksuid("sim");
  const existing = simulations.get(simId);
  if (existing) return existing; // idempotent on simulation_id
  ...
  if (body.account) {
    ...
    if (acc.currency !== String(body.currency).toUpperCase()) {
      throw new ApiError("validation-error", 400, `Account ${acc.id} holds ${acc.currency}, not ${body.currency}`);
    }
  }

  const nowStamp = now();
  const sim = {
    id: simId, object: "sandbox.payment", scenario: body.scenario,
    status: scenario.status, awaiting_advance: scenario.awaiting_advance,
    amount: { amount: body.amount, currency: String(body.currency).toUpperCase() },
    account: body.account ?? null,
    event_ids: [],
    history: [{ status: scenario.status, awaiting_advance: scenario.awaiting_advance, at: nowStamp }],
    created_at: nowStamp, updated_at: nowStamp,
  };

  if (scenario.status === "settled") creditSandboxAccount(sim);

  const evt = notify(`sandbox.payment.${scenario.status === "awaiting_confirmation" ? "unconfirmed" : scenario.status}`, {
    simulation_id: sim.id, previous_status: null, current_status: sim.status,
    amount: sim.amount, awaiting_advance: sim.awaiting_advance,
  });
...
route("POST", "/v2/sandbox/onboarding", ({ body }) => {
  need(body, ["scenario"]);
  const scenario = ONBOARDING_SCENARIOS[body.scenario];
  if (!scenario) {
    throw new ApiError("validation-error", 400, `Unknown scenario ${body.scenario}`, [
      { field: "scenario", message: `try one of: ${Object.keys(ONBOARDING_SCENARIOS).join(", ")}`, code: "unknown_scenario" },
    ]);
  }
  const simId = body.simulation_id || ksuid("sim");
  const existing = simulations.get(simId);
  ...
  if (body.customer && !db.customers.get(body.customer)) {
    throw new ApiError("not-found", 404, `No customer ${body.customer}`);
  }

  const nowStamp = now();
  const sim = {
    id: simId, object: "sandbox.onboarding", scenario: body.scenario,
    status: scenario.status, decision: scenario.decision, awaiting_advance: scenario.awaiting_advance,
    customer: body.customer ?? null,
    event_ids: [],
    history: [{ status: scenario.status, decision: scenario.decision, awaiting_advance: scenario.awaiting_advance, at: nowStamp }],
    created_at: nowStamp, updated_at: nowStamp,
  };
  ...
  const evt = notify(`sandbox.onboarding.${body.scenario.split(".")[1]}`, {
    simulation_id: sim.id, previous_status: null, current_status: sim.status,
    decision: sim.decision, awaiting_advance: sim.awaiting_advance,
  });
  sim.event_ids.push(evt.id);
...
route("POST", "/v2/sandbox/:id/advance", ({ params, body }) => {
  const sim = must(simulations, params.id, "simulation");
  if (!sim.awaiting_advance) {
    throw new ApiError("conflict", 409, `Simulation ${sim.id} is not awaiting advance (status: ${sim.status})`);
  }
  ...
  if (sim.object === "sandbox.payment") {
    ...
    const evt = notify(`sandbox.payment.${sim.status === "failed" ? "failed" : "confirmed"}`, {
      simulation_id: sim.id, previous_status: previous, current_status: sim.status, amount: sim.amount,
    });
    sim.event_ids.push(evt.id);
  } else {
    sim.status = "completed";
    sim.decision = body.decision ?? (sim.scenario === "onboarding.compliance_hold" ? "declined" : "approved");
    sim.awaiting_advance = false;
    if (sim.customer) {
      const c = db.customers.get(sim.customer);
      if (c) { c.status = sim.status; c.decision = sim.decision; db.customers.set(c.id, c); }
    }
    const evt = notify("sandbox.onboarding.reviewed", {
      simulation_id: sim.id, previous_status: previous, current_status: sim.status, decision: sim.decision,
    });
    sim.event_ids.push(evt.id);
  }

  sim.updated_at = now();
  sim.history.push({ status: sim.status, decision: sim.decision ?? null, awaiting_advance: sim.awaiting_advance, at: sim.updated_at });
  simulations.set(sim.id, sim);
  return sim;
});

route("GET", "/v2/sandbox/:id", ({ params }) => {
  const sim = must(simulations, params.id, "simulation");
  const callbacks = sim.event_ids.flatMap((eid) => [...deliveries.values()].filter((d) => d.event_id === eid))
    .map(({ id, webhook, status, response_code, replay, attempted_at }) => ({ id, webhook, status, response_code, replay, attempted_at }));
...
route("GET", "/v2/events/:id", ({ params }) => {
  const evt = [...db.events].find((e) => e.id === params.id);
...
route("GET", "/v2/ledger/balances", ({ url }) => {
  const acctFilter = url.searchParams.get("account");
  const pairs = new Map();
  for (const row of db.ledger) {
    if (acctFilter && row.account !== acctFilter) continue;
    pairs.set(`${row.account}|${row.currency}`, { account: row.account, currency: row.currency });
  }
  const rows = [...pairs.values()]
    .map((p) => ({
      id: `bal_${p.account}_${p.currency}`, account: p.account, currency: p.currency,
...
route("POST", "/v2/statements", ({ body }) => {
  need(body, ["account"]);
  const acc = must(db.accounts, body.account, "account");
  const format = body.format ?? "json";
  if (!["csv", "pdf", "json"].includes(format)) {
    throw new ApiError("validation-error", 400, "format must be csv, pdf or json");
  }
  ...
  if ((body.from && Number.isNaN(from)) || (body.to && Number.isNaN(to))) {
    throw new ApiError("validation-error", 400, "from/to must be parseable dates");
  }
  ...
  const rows = allRows.filter((r) => txnIds.has(r.txn)).sort((a, b) => a.at.localeCompare(b.at));

  const totalDebits = rows.filter((r) => r.amount < 0n).reduce((n, r) => n - r.amount, 0n);
  const totalCredits = rows.filter((r) => r.amount > 0n).reduce((n, r) => n + r.amount, 0n);
  ...
  for (const li of lineItems) {
    csvLines.push([li.txn, li.at, li.account, li.currency, li.debit ?? "", li.credit ?? "", li.memo ?? ""]
      .map((v) => `"${String(v).replace(/"/g, '""')}"`).join(","));
  }
  csvLines.push(`TOTAL,,,,${fromMinor(totalDebits)},${fromMinor(totalCredits)},`);
  const csv = csvLines.join("\n");

  const s = {
    id: ksuid("stm"), object: "statement", account: acc.id, currency: acc.currency,
    from: body.from ?? null, to: body.to ?? null, format,
    opening_balance: fromMinor(opening), closing_balance: fromMinor(closing),
    total_debits: fromMinor(totalDebits), total_credits: fromMinor(totalCredits),
  ...
  s.signature = createHash("sha256")
    .update(JSON.stringify({ account: s.account, from: s.from, to: s.to, line_items: s.line_items, total_debits: s.total_debits, total_credits: s.total_credits }))
    .digest("hex");
  if (format === "pdf") {
    s.pdf_base64 = Buffer.from(`Statement ${s.id}\nAccount ${s.account}\n${csv}`).toString("base64");
    s.note = "Sandbox PDF is a base64-encoded plaintext stand-in, not a rendered PDF binary.";
  }
...
route("GET", "/v2/statements/:id", ({ params }) => must(statements, params.id, "statement"));
...
const feeConfigs = collection("feeConfigs"); // key.id -> { payout_account, updated_at }

route("GET", "/v2/fees/config", ({ key }) => {
  const cfg = feeConfigs.get(key.id) ?? { payout_account: null, updated_at: null };
  return { object: "fee_config", schedule: FEE_SCHEDULE, payout_account: cfg.payout_account, updated_at: cfg.updated_at };
});

route("PUT", "/v2/fees/config", ({ key, body }) => {
  need(body, ["payout_account"]);
  must(db.accounts, body.payout_account, "account");
  const cfg = { payout_account: body.payout_account, updated_at: now() };
  feeConfigs.set(key.id, cfg);
...
const HOLIDAYS = {
  EUR: ["2026-01-01", "2026-04-03", "2026-04-06", "2026-05-01", "2026-12-25", "2026-12-26"],
  GBP: ["2026-01-01", "2026-04-03", "2026-04-06", "2026-05-25", "2026-12-25", "2026-12-28"],
  USD: ["2026-01-01", "2026-07-04", "2026-11-26", "2026-12-25"],
  SGD: ["2026-01-01", "2026-02-17", "2026-02-18", "2026-12-25"],
};

route("GET", "/v2/rails/:id/calendar", ({ params, url }) => {
  ...
  for (let i = 0; i < days; i++) {
    ...
    data.push({
      date: iso, weekend: isWeekend, holiday: isHoliday,
      business_day: !isHoliday && !closedForWeekend,
    });
  }

  return { object: "list", rail: rail.id, currency: rail.currency, cutoff: rail.cutoff, runs_weekends: rail.weekend, data };
}, { public: true });
