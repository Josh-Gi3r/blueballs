...
export function priceCorridor(resting, from, to, sizeMinor = 0n) {
  ...
  let bps = BASE_BPS + MAX_SKEW_BPS * Math.max(0, imbalance) + MAX_SKEW_BPS * 0.4 * impact;
  let rebate = 0;

  // trading against the flow — the corridor wants this, so pay for it
  if (imbalance < -REBATE_THRESHOLD) {
    const strength = Math.min(1, (Math.abs(imbalance) - REBATE_THRESHOLD) / (1 - REBATE_THRESHOLD));
  ...
  return {
    ...
    inputs: { base_bps: BASE_BPS, max_skew_bps: MAX_SKEW_BPS, imbalance, impact: Math.round(impact * 1000) / 1000 },
  };
}
...
export function principalQuote(appetite, position, sizeMinor, imbalanceBps) {
  if (!appetite.enabled) {
    return { available: false, reason: "The operator has switched off principal fills for this corridor." };
  }
