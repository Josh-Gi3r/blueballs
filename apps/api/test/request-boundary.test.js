...
test("rotating invalid key strings cannot evade the source rate limit", async (t) => {
  const api = await createApiFixture({ env: { SOURCE_RATE_LIMIT_PER_MIN: "4" } });
  t.after(() => api.close());

  assert.equal((await api.request("GET", "/v2/customers", { key: "invalid-one" })).status, 401);
  assert.equal((await api.request("GET", "/v2/customers", { key: "invalid-two" })).status, 401);
  assert.equal((await api.request("GET", "/v2/customers", { key: "invalid-three" })).status, 401);
  assert.equal((await api.request("GET", "/v2/customers", { key: "invalid-four" })).status, 429);
});
...
test("body limits count bytes and CORS only reflects configured origins", async (t) => {
  ...
  const allowed = await api.request("GET", "/v2/customers", {
    key: tenant.key, headers: { origin: "https://app.example.test" },
  });
  const denied = await api.request("GET", "/v2/customers", {
    key: tenant.key, headers: { origin: "https://attacker.example" },
  });
