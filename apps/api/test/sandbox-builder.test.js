...
test("builder creates, provisions, tests and persists an isolated sandbox", async (t) => {
  ...
  const created = await api.request("POST", "/v2/builder/projects", {
    ...
    body: {
      name: "Maker Bank",
      brief:
        "A Singapore account and card product for independent designers with cross-border FX.",
  ...
  assert.equal(
    (
      await api.request("GET", `/v2/builder/projects/${projectId}`, {
        key: beta.key,
      })
    ).status,
    404,
  );
  const betaProjects = await api.request("GET", "/v2/builder/projects", {
    key: beta.key,
  });
  ...
  const provisioned = await api.request(
    "POST",
    `/v2/builder/projects/${projectId}/provision`,
    { key: alpha.key },
  );
  ...
  assert.ok(
    provisioned.body.build.steps.every((step) => step.status === "complete"),
  );

  const reprovisioned = await api.request(
    "POST",
    `/v2/builder/projects/${projectId}/provision`,
    { key: alpha.key },
  );
  ...
  assert.equal(
    reprovisioned.body.accounts.length,
    9,
    "provisioning is idempotent after the environment is ready",
  );

  const source = provisioned.body.accounts.find(
    (account) => account.currency === "SGD",
  );
  assert.ok(source);
  assert.equal(source.balance.amount, "10000.00");
  const payment = await api.request(
    "POST",
    `/v2/builder/projects/${projectId}/test-payments`,
    {
      key: alpha.key,
      body: {
  ...
  assert.equal(
    payment.body.accounts.find((account) => account.id === source.id).balance
      .amount,
    "9874.50",
  );
  ...
  assert.equal(
    restored.body.accounts.find((account) => account.id === source.id).balance
      .amount,
    "9874.50",
  );
});
...
test("builder validates blueprints and protects provisioned financial configuration", async (t) => {
  ...
  const created = await api.request("POST", "/v2/builder/projects", {
    ...
    body: {
      name: "Freelance Bank",
      brief: "Accounts and invoices for UK freelancers",
  ...
  const revised = await api.request(
    "PATCH",
    `/v2/builder/projects/${created.body.id}`,
    {
      key: tenant.key,
  ...
  await api.request(
    "POST",
    `/v2/builder/projects/${created.body.id}/provision`,
    { key: tenant.key },
  );
  assert.equal(
    (
      await api.request("PATCH", `/v2/builder/projects/${created.body.id}`, {
        key: tenant.key,
        body: { currencies: ["USD"] },
      })
    ).status,
