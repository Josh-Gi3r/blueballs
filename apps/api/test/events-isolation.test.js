...
test("events are durable and visible only to their tenant", async (t) => {
  ...
  const customerEvent = eventsA.body.data.find(
    (event) => event.type === "customer.created",
  );
  assert.ok(customerEvent);
  assert.equal(customerEvent.data.id, created.body.id);
  assert.equal("tenant_id" in customerEvent, false);
  assert.equal(
    eventsB.body.data.some((event) => event.data?.id === created.body.id),
    false,
  );
  assert.equal(
    (
      await api.request("GET", `/v2/events/${customerEvent.id}`, {
        key: tenantB.key,
      })
    ).status,
    404,
  ...
  await api.restart();
  const afterRestart = await api.request(
    "GET",
    `/v2/events/${customerEvent.id}`,
    { key: tenantA.key },
  );
...
test("one tenant cannot evict another tenant's retained events", async (t) => {
  const api = await createApiFixture({
    env: { EVENT_RETENTION_PER_TENANT: "3" },
  });
  t.after(() => api.close());
  const tenantA = await api.signup("retention-a@example.test");
  const tenantB = await api.signup("retention-b@example.test");

  for (let index = 0; index < 6; index++) {
    const response = await api.request("POST", "/v2/customers", {
      key: tenantA.key,
      body: { type: "individual", name: `Noisy tenant ${index}` },
    });
  ...
  const eventsA = await api.request("GET", "/v2/events", { key: tenantA.key });
  const eventsB = await api.request("GET", "/v2/events", { key: tenantB.key });
  assert.equal(eventsA.body.data.length, 3);
  assert.ok(eventsB.body.data.some((event) => event.type === "key.issued"));
});
...
test("webhook metadata and delivery logs cannot cross tenants", async (t) => {
  const api = await createApiFixture({
    env: {
      ...
      WEBHOOK_ALLOWED_HOSTS: "example.test",
    },
  });
  t.after(() => api.close());
  ...
  const reread = await api.request("GET", `/v2/webhooks/${created.body.id}`, {
    key: tenantA.key,
  });
  ...
  assert.equal(
    (
      await api.request("GET", `/v2/webhooks/${created.body.id}`, {
        key: tenantB.key,
      })
    ).status,
    404,
  ...
  assert.deepEqual(
    (await api.request("GET", "/v2/webhooks", { key: tenantB.key })).body.data,
    [],
  );
...
test("allowlisted delivery attempts only the event owner's webhook", async (t) => {
  ...
  for (let attempt = 0; attempt < 20; attempt++) {
    deliveriesA = await api.request(
      "GET",
      `/v2/webhooks/${webhookA.body.id}/deliveries`,
      { key: tenantA.key },
    );
    if (deliveriesA.body.data.length) break;
    await new Promise((resolve) => setTimeout(resolve, 10));
  }
  const deliveriesB = await api.request(
    "GET",
    `/v2/webhooks/${webhookB.body.id}/deliveries`,
    { key: tenantB.key },
  );
