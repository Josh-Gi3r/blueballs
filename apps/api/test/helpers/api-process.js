...
async function waitForReady(baseUrl, child, output) {
  ...
  throw new Error(`API did not become ready within ${START_TIMEOUT_MS}ms\n${output()}`);
}
...
async function terminate(child) {
  ...
  const timedOut = new Promise((resolve) => setTimeout(() => resolve("timeout"), 2_000));
...
export async function createApiFixture({ env = {} } = {}) {
  ...
  async function start() {
    if (child && child.exitCode === null) throw new Error("API fixture is already running");
