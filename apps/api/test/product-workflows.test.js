...
test("cards, policy controls, authorisation settlement and disputes form one usable sandbox journey", async (t) => {
  ...
  const policy = await api.request("POST", "/v2/policies", {
    ...
    body: {
      name: "Card controls",
      rules: [{ type: "max_amount", amount: "250.00", currency: "USD" }],
    },
  });
  ...
  assert.deepEqual(
    attached.body.attached.map(({ type, id }) => ({ type, id })),
    [{ type: "card", id: card.body.id }],
  );

  const authorisation = await api.request(
    "POST",
  ...
  const settled = await api.request(
    "POST",
    `/v2/authorisations/${authorisation.body.id}/approve`,
    {
      key: tenant.key,
    },
  );
...
test("credit collateral math stays exact and each draw and repayment is a coherent four-leg operation", async (t) => {
  ...
  const expectedAvailable = fromMinor(
    (convertMinor(toMinor(collateral), "EUR", "USD") * 8_000n) / 10_000n,
  );
  ...
  const repay = await api.request(
    "POST",
    `/v2/credit/${credit.body.id}/repay`,
    {
      key: tenant.key,
      body: { amount: "75.00" },
    },
...
test("business wallet funding and approval chains require distinct configured approvers", async (t) => {
  ...
  const first = await api.request(
    "POST",
    `/v2/approvals/${send.body.approval}/approve`,
    {
      key: tenant.key,
  ...
  const balance = await api.request("GET", `/v2/wallets/${wallet.body.id}`, {
    key: tenant.key,
  });
...
test("money-moving banking endpoints reject zero and negative amounts", async (t) => {
  ...
  for (const amount of ["0.00", "-1.00"]) {
    const funding = await api.request(
      "POST",
      `/v2/accounts/${account.id}/credit`,
      {
        key: tenant.key,
  ...
  const walletCredit = await api.request(
    "POST",
    `/v2/wallets/${wallet.body.id}/credit`,
    {
      key: tenant.key,
...
test("onboarding progresses from application data to an approved completed decision", async (t) => {
  ...
  const business = await api.request(
    "PATCH",
    `/v2/applications/${application.body.id}/business`,
    {
      key: tenant.key,
  ...
  const controller = await api.request(
    "POST",
    `/v2/applications/${application.body.id}/individuals`,
    {
      key: tenant.key,
      body: {
  ...
  const document = await api.request(
    "POST",
    `/v2/applications/${application.body.id}/documents`,
    {
      ...
      body: {
        ...
        content: Buffer.from("sandbox document").toString("base64"),
      },
    },
  );
  ...
  const attestation = await api.request(
    "POST",
    `/v2/applications/${application.body.id}/attestation`,
    {
      key: tenant.key,
  ...
  const decision = await api.request(
    "POST",
    `/v2/applications/${application.body.id}/edd`,
    {
      key: tenant.key,
...
test("recipient transfer, savings vault and statement share one balanced account ledger", async (t) => {
  ...
  // PayNow, not ACH: this test is about one balanced ledger across three
  // products, and a weekday-only rail made it fail every Saturday and Sunday
  // for a reason that has nothing to do with what it asserts. The weekend rule
  // itself is covered deliberately below.
  const { account } = await createCustomerAndAccount(api, tenant.key, "SGD");
  assert.equal(
    (
  ...
  const deposited = await api.request(
    "POST",
  ...
  const withdrawn = await api.request(
    "POST",
    `/v2/vaults/${vault.body.id}/withdraw`,
    {
      key: tenant.key,
      body: { amount: "50.00" },
    },
...
test("QR, payment link, mandate and subscription form a validated payment-artifact journey", async (t) => {
  ...
  assert.equal(
    (await api.request("GET", `/v2/links/${link.body.id}`, { key: tenant.key }))
      .status,
    200,
  );
...
/** The rail business calendar and the settlement gate are two implementations of
 *  one rule (`server.js` weekend check vs `/v2/rails/:id/calendar`). They must
 *  agree on today, whatever today is — that is what makes this deterministic
 *  rather than a test that only passes Monday to Friday. */
test("a rail's published calendar agrees with whether its window runs today", async (t) => {
  const api = await createApiFixture();
  t.after(() => api.close());
  const tenant = await api.signup("rail-calendar@example.test");

  for (const [rail, currency, destination] of [
    ["ach", "USD", { account_number: "000123456789" }],
    ["paynow", "SGD", { proxy: "+6591234567" }],
  ]) {
    const calendar = await api.request(
      "GET",
      `/v2/rails/${rail}/calendar?days=1`,
    );
    assert.equal(calendar.status, 200);
    const today = calendar.body.data[0];
    ...
    const recipient = await api.request("POST", "/v2/recipients", {
      key: tenant.key,
      body: { name: "Calendar supplier" },
    });
    const dest = await api.request(
      "POST",
      `/v2/recipients/${recipient.body.id}/destinations`,
      {
        key: tenant.key,
        body: { rail, name: "Calendar supplier", currency, ...destination },
      },
    );
    const transfer = await api.request("POST", "/v2/transfers", {
      key: tenant.key,
      body: {
        from: account.id,
        recipient: recipient.body.id,
        destination: dest.body.id,
        rail,
        amount: "125.00",
      },
    });
    // A closed rail never refuses the instruction — a customer can ask for an ach
    // payment on a Sunday. It refuses to run, which happens at the window.
    assert.equal(
      transfer.status,
      201,
      `${rail} must accept the instruction whatever ${today.date} is`,
    );
    ...
    if (transfer.body.status === "settled") {
      assert.equal(
        settled.status,
        409,
        `${rail} settles in seconds, so there is no window left to run`,
      );
    } else if (today.business_day) {
      assert.equal(
        settled.status,
        200,
        `${rail} publishes ${today.date} as a business day, so its window must run`,
      );
      assert.equal(settled.body.status, "settled");
    } else {
      assert.equal(
        settled.status,
        422,
        `${rail} publishes ${today.date} as closed, so its window must not run`,
      );
      assert.equal(settled.body.type, "rail-unavailable");
    }

    // whatever today is, a rail that runs weekends is never closed for one
    if (calendar.body.runs_weekends) {
      assert.equal(
        today.weekend && !today.holiday ? today.business_day : true,
        true,
        `${rail} runs weekends, so a weekend must not close it`,
      );
    }
  }
});
