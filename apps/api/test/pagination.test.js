...
test("ending_before returns the immediately preceding page", () => {
  const page = paginate(rows, new URL("https://example.test/items?ending_before=e&limit=2"));
  assert.deepEqual(page.data.map(({ id }) => id), ["c", "d"]);
...
test("pagination rejects conflicting cursors and invalid limits", () => {
  assert.throws(
    () => paginate(rows, new URL("https://example.test/items?starting_after=a&ending_before=e")),
    /starting_after or ending_before/,
  );
  assert.throws(() => paginate(rows, new URL("https://example.test/items?limit=0")), /positive integer/);
});
