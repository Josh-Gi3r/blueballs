...
test("duplicate emails create isolated tenant principals", async (t) => {
  ...
  assert.deepEqual(keysA.body.data.map((key) => key.id), [tenantA.id]);
  assert.deepEqual(keysB.body.data.map((key) => key.id), [tenantB.id]);

  const crossTenantGet = await api.request("GET", `/v2/keys/${tenantA.id}`, { key: tenantB.key });
  const crossTenantDelete = await api.request("DELETE", `/v2/keys/${tenantA.id}`, { key: tenantB.key });
  ...
  const aStillWorks = await api.request("GET", "/v2/keys", { key: tenantA.key });
  assert.equal(aStillWorks.status, 200);
  assert.deepEqual(aStillWorks.body.data.map((key) => key.id), [tenantA.id]);
});
...
test("secondary keys inherit tenant and cannot escalate scope", async (t) => {
  ...
  const sharedList = await api.request("GET", "/v2/keys", { key: secondary.key });
  assert.equal(sharedList.status, 200);
  assert.deepEqual(new Set(sharedList.body.data.map((key) => key.id)), new Set([primary.id, secondary.id]));
  ...
  const revokePrimary = await api.request("DELETE", `/v2/keys/${primary.id}`, { key: secondary.key });
  assert.equal(revokePrimary.status, 200);

  const secondaryStillWorks = await api.request("GET", "/v2/keys", { key: secondary.key });
  assert.equal(secondaryStillWorks.status, 200);
  assert.deepEqual(secondaryStillWorks.body.data.map((key) => key.id), [secondary.id]);
});
