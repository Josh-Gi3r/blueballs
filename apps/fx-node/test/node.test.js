...
async function setup({ adapter = {} } = {}) {
  ...
  return {
    market, quotes, fiat, node, adapter, request,
    async close() {
      await node.close();
      quotes.close();
      fiat.close();
...
test('execute fails closed without adapter and does not change reservation state', async () => {
  ...
  try {
    ...
    const quoted = await env.request('/v2/fx/quotes', {
      method: 'POST', body: { inputAsset: INPUT, outputAsset: OUTPUT, exactOutput: '100' },
    });
...
test('ambiguous adapter failure leaves route submitted for reconciliation instead of releasing liquidity', async () => {
  const adapter = {
    async submit() { throw new Error('network connection lost after send'); },
  };
  const env = await setup({ adapter });
  try {
    ...
    const route = await env.request(`/v2/fx/routes/${quoted.json.routeId}`);
    assert.equal(route.json.state, 'SUBMITTED');
    assert.throws(() => env.market.releaseRoute(quoted.json.routeId), /submitted route cannot be released/);
  } finally {
    await env.close();
  }
});
...
test('fiat intent lifecycle is exposed without pretending submission equals settlement', async () => {
  ...
  try {
    const intent = {
      intentId: 'fiat-node-1', routeId: 'mixed-route', edgeId: 'fiat-edge',
      edgeType: 'VERIFIED_FIAT_PAYMENT', finalityClass: 'ATTESTED_EXTERNAL',
      payerParticipantId: 'payer', payeeParticipantId: 'payee',
      payerAccountRef: 'payer:bank', payeeAccountRef: 'payee:bank',
      currency: 'MYR', amount: '10000', rail: 'DUITNOW', providerId: 'provider',
      policyAuthorizationId: 'auth-fiat', createdAt: NOW, expiresAt: NOW + 60_000, nonce: '1',
    };
    const created = await env.request('/v2/fx/fiat/intents', { method: 'POST', body: intent });
    assert.equal(created.response.status, 201);
    assert.equal(created.json.state, 'CREATED');

    const reserved = await env.request('/v2/fx/fiat/intents/fiat-node-1/reserve', { method: 'POST' });
    assert.equal(reserved.json.state, 'RESERVED');
    const submitted = await env.request('/v2/fx/fiat/intents/fiat-node-1/submit', {
      method: 'POST', body: { submissionRef: 'bank-transfer-1' },
    });
