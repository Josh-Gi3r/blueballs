...
test('HTTP customer trade, market scenario and route state all use the same runtime', async () => {
  ...
  try {
    ...
    const anonymousScenarioMutation = await env.request('/v2/fx/reference/scenario', {
      method: 'POST', auth: false, body: { id: 'lp_offline' },
    });
    ...
    const anonymousFetch = await env.request(`/v2/fx/reference/trades/${reserved.body.id}`, { auth: false });
