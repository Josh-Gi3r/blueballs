...
test("monetary reference endpoints expose public evidence and protect mutations", async () => {
  const env = await serverSetup();
  try {
    const instruments = await env.request(
      "/v2/fx/reference/monetary/instruments",
      { auth: false },
    );
    assert.equal(instruments.response.status, 200);
    assert.deepEqual(
      instruments.body.data.map((item) => item.code),
      ["EUR", "USD"],
    );
    ...
    assert.equal(preview.body.object, "fx_trade_preview");
    assert.equal(preview.body.evidence.reserved, false);
    assert.equal(
      preview.body.pricing.reference.providerType,
      "REFERENCE_FIXTURE",
    );
    assert.equal(
      preview.body.pricing.reference.intendedUse,
      "VALUATION_CONTROL_ONLY",
    );
    assert.equal(preview.body.pricing.reference.executable, false);
    assert.equal(preview.body.pricing.reference.sourceIds.length, 3);
    assert.equal(
      Number.isInteger(preview.body.pricing.client.referenceSpreadBps),
      true,
    );

    const malformed = await env.request(
      "/v2/fx/reference/monetary/remittance/preview",
      {
        method: "POST",
        auth: false,
        body: { inputAmount: "1.2" },
      },
    );
    assert.equal(malformed.response.status, 400);
    assert.equal(malformed.body.error.code, "VALIDATION_ERROR");

    const rejected = await env.request("/v2/fx/reference/monetary/reserves", {
      method: "POST",
      auth: false,
      body: {
        reserveCurrency: "USD",
        amount: "1000",
        providerRef: "bank:test",
      },
    });
    assert.equal(rejected.response.status, 401);

    const created = await env.request("/v2/fx/reference/monetary/reserves", {
      method: "POST",
      body: {
        reserveCurrency: "USD",
        amount: "1000",
    ...
    const settled = await env.request(
      `/v2/fx/reference/monetary/reserves/${created.body.depositId}/settle`,
      { method: "POST" },
    );
    assert.equal(settled.response.status, 200);
    const minted = await env.request(
      "/v2/fx/reference/monetary/instruments/USD/mint",
      {
        method: "POST",
        body: { amount: "1000", beneficiaryRef: "wallet:test" },
      },
    );
    assert.equal(minted.response.status, 201);
    const health = await env.request("/v2/fx/reference/monetary/health", {
      auth: false,
    });
    assert.equal(
      health.body.reserves.find((item) => item.reserveCurrency === "USD")
        .coverageBps,
      10_000,
    );
  } finally {
...
test("one BRL to EUR preview uses the canonical six-source token market", async () => {
  ...
  try {
    ...
    assert.equal(preview.tokenRoute.inputSymbol, "BRL deposit claim");
    ...
    assert.equal(preview.pricing.reference.providerType, "REFERENCE_FIXTURE");
    assert.equal(preview.pricing.reference.sourceIds.length, 3);
    assert.equal(preview.pricing.client.executable, false);
...
test("reserving the public trade reserves every selected source and returns one connected object", async () => {
  const env = await runtime();
  try {
    const trade = await env.trades.reserveExactInput({
    ...
    assert.equal(trade.pricing.client.reserved, true);
    assert.equal(trade.pricing.client.quoteType, "FIRM_SANDBOX");
    assert.equal(trade.pricing.client.executable, false);
...
test("policy revocation removes the issuer before pricing", async () => {
  ...
  try {
    ...
    const issuer = state.sources.find(
      (source) => source.sourceType === "ISSUER",
    );
    ...
    const preview = env.trades.previewExactInput({ inputAmount: "50000.00" });
    assert.equal(
      preview.sources.some((source) => source.type === "ISSUER"),
      false,
    );
    assert.ok(
      preview.sources.some((source) => source.type === "BANK_PRINCIPAL"),
    );
  } finally {
    env.close();
  }
...
test("HTTP customer trade, market scenario and route state all use the same runtime", async () => {
  ...
  try {
    ...
    const anonymousReservation = await env.request("/v2/fx/reference/trades", {
      method: "POST",
      auth: false,
      body: { inputAmount: "50000.00", expiresInMs: 30_000 },
    });
    assert.equal(anonymousReservation.response.status, 401);

    const anonymousScenarioMutation = await env.request(
      "/v2/fx/reference/scenario",
      {
        method: "POST",
        auth: false,
        body: { id: "lp_offline" },
    ...
    assert.equal(anonymousScenarioMutation.response.status, 401);
    ...
    const anonymousFetch = await env.request(
      `/v2/fx/reference/trades/${reserved.body.id}`,
      { auth: false },
    );
    assert.equal(anonymousFetch.response.status, 401);

    const fetched = await env.request(
      `/v2/fx/reference/trades/${reserved.body.id}`,
    );
    assert.equal(fetched.body.quoteId, reserved.body.quoteId);

    const released = await env.request(
      `/v2/fx/reference/trades/${reserved.body.id}`,
      { method: "DELETE" },
    );
    ...
    assert.equal(
      scenario.body.sources.find(
        (source) => source.sourceType === "INSTITUTIONAL_LP",
      ).eligible,
      false,
    );
  } finally {
