...
async function serverSetup() {
  ...
  async function request(
    path,
    { method = "GET", body, auth = true, headers = {}, parse = "json" } = {},
  ) {
...
test("reference runtime reserves one firm route across every authorised source class", async () => {
  ...
  try {
    ...
    assert.equal(
      env.riskBook.getPosition(OUTPUT).reserved,
      (-50_000n * UNIT).toString(),
    );
  } finally {
    env.close();
  }
...
test("revoked issuer authorisation removes it before pricing and increases principal use", async () => {
  const env = await runtime();
  try {
    env.policy.revokeAuthorization(
      env.seed.sourceAuthorizations["issuer-eurc"],
    );
    const quote = await env.quotes.reserveExactOutput({
      inputAsset: INPUT,
      outputAsset: OUTPUT,
    ...
    assert.equal(
      quote.sources.some((source) => source.type === "ISSUER"),
      false,
    );
    const principal = quote.sources.find(
      (source) => source.type === "BANK_PRINCIPAL",
    );
    assert.ok(principal);
...
test("submitted integrated route reconciles every source and settles principal risk", async () => {
  ...
  try {
    ...
    const confirmed = env.quotes.confirm(quote.id, {
    ...
    assert.equal(
      env.riskBook.getPosition(OUTPUT).settled,
      (-50_000n * UNIT).toString(),
    );
  } finally {
    env.close();
...
test("HTTP reference endpoints expose the same runtime and quote reservations", async () => {
  ...
  try {
    ...
    const execution = await env.request(
      `/v2/fx/quotes/${quoted.body.id}/execute`,
      { method: "POST" },
    );
...
test("the node serves its OpenAPI contract without authentication", async () => {
  const env = await serverSetup();
  try {
    const openapi = await env.request("/openapi.yaml", {
      auth: false,
      parse: "text",
    });
    assert.equal(openapi.response.status, 200);
    assert.match(
      openapi.response.headers.get("content-type"),
      /^application\/yaml/,
    );
