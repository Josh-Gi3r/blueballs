...
export class ReferenceLiquidityBook {
  ...
  upsertSlice(slice) {
    if (!slice || typeof slice !== "object")
      throw new TypeError("slice required");
    if (!STATIC_SOURCE_TYPES.has(slice.sourceType)) {
      throw new RangeError(
        `unsupported reference source type: ${slice.sourceType}`,
      );
    }
    const normalized = {
      ...
      inputNumerator: positiveBigInt(
        slice.inputNumerator,
        "inputNumerator",
      ).toString(),
      inputDenominator: positiveBigInt(
        slice.inputDenominator,
        "inputDenominator",
      ).toString(),
      policyAuthorizationId: requiredString(
        slice.policyAuthorizationId,
  ...
  setOnline(sliceId, online) {
    ...
    if (Number(result.changes) !== 1)
      throw new Error("reference liquidity slice not found");
  }
  ...
  reserve({ routeId, leg, index }) {
    ...
    if (!Number.isSafeInteger(index) || index < 0)
      throw new RangeError("leg index invalid");
    ...
    return this.#transaction(() => {
      ...
      if (!row.online || row.expires_at <= this.now())
        throw new Error("reference liquidity unavailable");
      if (
        row.source_type !== leg.sourceType ||
        row.source_id !== leg.sourceId ||
        row.input_asset !== leg.inputAsset ||
        row.output_asset !== leg.outputAsset
      ) {
      ...
      if (!verification || verification.valid !== true) {
        const error = new Error(
          `reference liquidity authorization invalid: ${verification?.reason ?? "INVALID"}`,
        );
      ...
      if (input > BigInt(leg.inputAmount))
        throw new Error("reference liquidity price changed");
      ...
      this.db
        .prepare(
          "UPDATE reference_liquidity SET reserved_output = ? WHERE slice_id = ?",
        )
  ...
  getReservation(reservationHandle) {
    return rowToReservation(
      this.db
        .prepare(
          "SELECT * FROM reference_liquidity_reservations WHERE reservation_handle = ?",
        )
  ...
  validateReserved({ reservationHandle }) {
    ...
    const row = this.db
      .prepare("SELECT * FROM reference_liquidity WHERE slice_id = ?")
      .get(reservation.sliceId);
    if (!row || !row.online) throw new Error("reference source is offline");
    const verification = this.#verifyAuthorization(
      row,
      BigInt(reservation.outputAmount),
    );
    if (!verification || verification.valid !== true) {
      const error = new Error(
        `reference liquidity authorization invalid: ${verification?.reason ?? "INVALID"}`,
  ...
  markSubmitted({ reservationHandle, submissionRef }) {
    ...
    return this.#transaction(() => {
      ...
      if (row.state === "SUBMITTED" && row.submission_ref === submissionRef)
        return rowToReservation(row);
      if (row.state !== "ACTIVE")
        throw new Error(
          `reference reservation cannot submit from ${row.state}`,
        );
  ...
  confirm({ reservationHandle, eventId }) {
    ...
    return this.#transaction(() => {
      const row = this.db
        ...
        .get(reservationHandle);
      if (!row) throw new Error("reference reservation not found");
      if (row.state === "CONSUMED" && row.event_id === eventId)
        return { duplicate: true };
      if (row.state !== "SUBMITTED")
        throw new Error(
          `reference reservation cannot confirm from ${row.state}`,
        );
      ...
      if (reserved < output || capacity < output)
        throw new Error("reference source accounting mismatch");
      ...
      `,
        ...
        .run(
          (capacity - output).toString(),
          (reserved - output).toString(),
          row.slice_id,
        );
      this.db
        .prepare(
  ...
  #releaseFromStates(reservationHandle, states, nextState, reason) {
    return this.#transaction(() => {
      ...
      const reserved = BigInt(source.reserved_output);
      if (reserved < output)
        throw new Error("reference reserved capacity underflow");
      this.db
        .prepare(
          "UPDATE reference_liquidity SET reserved_output = ? WHERE slice_id = ?",
        )
...
export class ReferenceLiquidityAdapter {
  constructor(book) {
    if (!book || typeof book.reserve !== "function")
      throw new TypeError("ReferenceLiquidityBook required");
    this.book = book;
  }

  reserve(args) {
    return this.book.reserve(args);
  }
  release(args) {
    return this.book.release(args);
  }
  validateReserved(args) {
    return this.book.validateReserved(args);
  }
  markSubmitted(args) {
    return this.book.markSubmitted(args);
  }
  confirm(args) {
    return this.book.confirm(args);
  }
  fail(args) {
    return this.book.fail(args);
  }
}
