...
export class PrivateMarketQuoteCoordinator {
  ...
  reserveExactOutput({ inputAsset, outputAsset, exactOutput, expiresInMs = 15_000 }) {
    ...
    try {
      ...
      `).run(quoteId, routeId, now, expiresAt, JSON.stringify(publicQuote), JSON.stringify(route));
    } catch (error) {
  ...
  getQuote(quoteId) {
    const row = this.db.prepare('SELECT * FROM fx_node_quotes WHERE quote_id = ?').get(quoteId);
    if (!row) return null;
    return { ...parse(row.public_json), state: row.state, submissionRef: row.submission_ref };
  }

  getPrivateQuote(quoteId) {
    const row = this.db.prepare('SELECT * FROM fx_node_quotes WHERE quote_id = ?').get(quoteId);
    if (!row) return null;
    return { row, publicQuote: parse(row.public_json), route: parse(row.private_json) };
  }
  ...
  markSubmitted(quoteId, submissionRef) {
    ...
    this.db.prepare("UPDATE fx_node_quotes SET state = 'SUBMITTED', submission_ref = ? WHERE quote_id = ?")
  ...
  confirm(quoteId, { eventId, fills }) {
    const quote = this.getPrivateQuote(quoteId);
    if (!quote) throw new Error('quote not found');
    this.market.confirmSubmittedRoute({ routeId: quote.row.route_id, eventId, fills });
    this.db.prepare("UPDATE fx_node_quotes SET state = 'CONFIRMED' WHERE quote_id = ?").run(quoteId);
    return this.getQuote(quoteId);
  }
  ...
  fail(quoteId, { eventId = null, reason }) {
    const quote = this.getPrivateQuote(quoteId);
    if (!quote) throw new Error('quote not found');
    this.market.failSubmittedRoute({ routeId: quote.row.route_id, eventId, reason });
    this.db.prepare("UPDATE fx_node_quotes SET state = 'FAILED' WHERE quote_id = ?").run(quoteId);
    return this.getQuote(quoteId);
  }
}
