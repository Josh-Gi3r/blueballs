...
import { CompositePrincipalLiquidityAdapter, pairKey } from './composite-principal-adapter.js';
import { IntegratedQuoteCoordinator } from './integrated-quote-coordinator.js';
import { createReferenceRuntime, REFERENCE_ASSETS, referenceAsset } from './reference-runtime.js';
...
const BRLX_PAIR = pairKey(REFERENCE_ASSETS.BRLX.id, REFERENCE_ASSETS.EURC.id);
...
function authorize(policy, {
  ...
  if (!decision.eligible) {
    throw new Error(`reference policy rejected ${participantId}: ${decision.reasons.join(', ')}`);
  }
  return decision;
}

const BRLX_STATIC_DEFINITIONS = Object.freeze([
  {
...
async function seedBrlxPrivateOrder({ market, policy, now }) {
  const maker = '0x00000000000000000000000000000000000000a1';
  const existing = market.listOrdersForMaker(maker).some(({ order, state }) => (
    order.buyToken === REFERENCE_ASSETS.BRLX.id.toLowerCase()
  ...
  const authorization = authorize(policy, {
    ...
    inputAsset: REFERENCE_ASSETS.BRLX.id,
  ...
  const order = {
    ...
    buyToken: REFERENCE_ASSETS.BRLX.id,
    sellAmount,
    buyAmount,
    recipient: maker,
    validAfter: 0,
    ...
    epoch: 1,
    salt: digest(`public-reference-brlx-eurc:${authorization.authorizationId}`),
  };
  await market.admitOrder({
    orderHash: digest(JSON.stringify({ order, authorizationId: authorization.authorizationId })),
    order,
    signature: '0x01',
...
function createBrlxReferenceFeed(now) {
  ...
  return {
    quote(inputAsset, outputAsset) {
      if (inputAsset !== REFERENCE_ASSETS.BRLX.id || outputAsset !== REFERENCE_ASSETS.EURC.id) {
        return {
          available: false,
          base: outputAsset,
          quote: inputAsset,
      ...
      return engine.consensus({
        ...
        quote: inputAsset,
        observations: [
          { sourceId: 'reference-br-bank-a', base: outputAsset, quote: inputAsset, bid: '6.086', ask: '6.094', observedAt, status: 'OK' },
          { sourceId: 'reference-br-bank-b', base: outputAsset, quote: inputAsset, bid: '6.088', ask: '6.096', observedAt, status: 'OK' },
          { sourceId: 'reference-br-venue-c', base: outputAsset, quote: inputAsset, bid: '6.087', ask: '6.095', observedAt, status: 'OK' },
        ],
      });
    },
 ...
 * the public page uses BRLX/EURC so the phone, bank view and settlement graph all
...
export async function createPublicReferenceRuntime({
  ...
  await seedBrlxPrivateOrder({ market: base.market, policy: base.policy, now });
  base.riskBook.configureAsset(REFERENCE_ASSETS.BRLX.id, (1_000_000n * UNIT).toString());

  const sourceAuthorizations = new Map();
  function restoreStaticSources(overrides = {}) {
    for (const definition of BRLX_STATIC_DEFINITIONS) {
      ...
      base.referenceBook.upsertSlice({
        ...
        inputNumerator: BigInt(override.inputNumerator ?? definition.inputNumerator).toString(),
      ...
      sourceAuthorizations.set(definition.sourceId, authorization.authorizationId);
    }
  }
  restoreStaticSources();

  const brlxReferenceFeed = createBrlxReferenceFeed(now);
  ...
  const brlxPrincipalAdapter = new PrincipalLiquidityAdapter({
    engine: base.principalEngine,
    riskBook: base.riskBook,
    referenceFor: (inputAsset, outputAsset) => brlxReferenceFeed.quote(inputAsset, outputAsset),
  ...
  const principalAdapter = new CompositePrincipalLiquidityAdapter({
    ...
    [BRLX_PAIR]: brlxPrincipalAdapter,
  });
  ...
  function sourceStatus() {
    const privateDepth = base.market.aggregateDepth({
      inputToken: REFERENCE_ASSETS.BRLX.id,
      outputToken: REFERENCE_ASSETS.EURC.id,
    });
    const privateOutput = privateDepth.reduce((sum, level) => sum + BigInt(level.availableSell), 0n);
    const rows = [{
      ...
      reason: privateOutput > 0n ? 'SIGNED_AND_POLICY_AUTHORISED' : 'NO_OPEN_ORDERS',
    }];
    ...
    for (const definition of BRLX_STATIC_DEFINITIONS) {
      ...
      rows.push({
        ...
        eligible: Boolean(slice?.online) && verification.valid === true && available > 0n,
    ...
    const reference = brlxReferenceFeed.status();
    const principalAuthorizationStatus = base.policy.verifyAuthorization(principalAuthorization.authorizationId);
    rows.push({
      sourceType: 'BANK_PRINCIPAL',
      sourceId: 'bank-principal',
      label: 'Bank balance sheet',
      eligible: reference.available && principalAuthorizationStatus.valid === true
        && principalHeadroom(base.riskBook, REFERENCE_ASSETS.BRLX.id) > 0n,
      availableOutput: principalHeadroom(base.riskBook, REFERENCE_ASSETS.EURC.id).toString(),
  ...
  const scenario = {
    list() {
      return [
        ...
        { id: 'issuer_policy_blocked', label: 'Issuer policy authorisation revoked' },
        { id: 'treasury_near_limit', label: 'Treasury inventory nearly exhausted' },
        { id: 'principal_limit', label: 'Bank principal at its hard risk limit' },
        { id: 'reference_outage', label: 'Reference pricing unavailable' },
      ];
    },
    current() {
      return { id: currentScenario, sources: sourceStatus(), reference: brlxReferenceFeed.status() };
    },
    apply(id) {
      ...
      brlxReferenceFeed.setAvailable(true);
      base.riskBook.setSettledPosition(REFERENCE_ASSETS.BRLX.id, '0');
      base.riskBook.setSettledPosition(REFERENCE_ASSETS.EURC.id, '0');
      restoreStaticSources();

      if (id === 'lp_offline') {
        base.referenceBook.setOnline('reference:institutional-lp-1:brlx-eurc', false);
      } else if (id === 'issuer_policy_blocked') {
        base.policy.revokeAuthorization(sourceAuthorizations.get('issuer-eurc'));
      } else if (id === 'treasury_near_limit') {
        restoreStaticSources({ 'bank-treasury': { maxOutput: 200n * UNIT } });
      } else if (id === 'principal_limit') {
        base.riskBook.setSettledPosition(REFERENCE_ASSETS.BRLX.id, (995_000n * UNIT).toString());
        base.riskBook.setSettledPosition(REFERENCE_ASSETS.EURC.id, (-995_000n * UNIT).toString());
      } else if (id === 'reference_outage') {
        brlxReferenceFeed.setAvailable(false);
      }
  ...
  const tradesPath = memory || base.paths.quotes === ':memory:'
    ? ':memory:'
  ...
  const inspector = {
    ...
    liquidity({ inputAsset, outputAsset, exactOutput = (1n * UNIT).toString() }) {
...
export { BRLX_STATIC_DEFINITIONS };
