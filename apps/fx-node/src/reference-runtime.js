...
export function referenceAsset(value) {
  ...
  return ASSET_LOOKUP.get(value) ?? ASSET_LOOKUP.get(value.toLowerCase()) ?? null;
}
...
function ensureCredential(policy, {
  participantId,
  credentialType,
  providerRef,
  now,
}) {
  const existing = policy.db.prepare(`
    SELECT * FROM fx_credentials
    WHERE participant_id = ? AND credential_type = ?
  `).get(participantId, credentialType);
  if (existing && existing.status === 'PASSED' && (existing.expires_at === null || existing.expires_at > now + DAY)) {
    return;
  }
...
function ensureAccount(policy, participantId, accountRef, accountType = 'WALLET') {
  const existing = policy.db.prepare('SELECT * FROM fx_accounts WHERE account_ref = ?').get(accountRef);
  if (!existing) policy.mapAccount({ participantId, accountRef, accountType });
  else if (existing.participant_id !== participantId) throw new Error(`account ${accountRef} belongs to another participant`);
}

function ensureAuthorization(policy, {
  participantId,
  action,
  inputAsset,
  outputAsset,
...
function configureReferencePolicy(policy, now) {
  policy.configurePolicy({
    ...
    allowedAssets: [
      ...
      REFERENCE_ASSETS.BRLX.id,
      REFERENCE_ASSETS.BRL.id,
      REFERENCE_ASSETS.EUR.id,
    ],
    allowedCorridors: [
      `${REFERENCE_ASSETS.USDC.id}/${REFERENCE_ASSETS.EURC.id}`,
      `${REFERENCE_ASSETS.BRL.id}/${REFERENCE_ASSETS.BRLX.id}`,
      `${REFERENCE_ASSETS.BRLX.id}/${REFERENCE_ASSETS.EURC.id}`,
  ...
  const participants = [
    { participantId: 'sandbox-customer', participantType: 'CUSTOMER', jurisdiction: 'BR' },
    { participantId: 'customer-maker', participantType: 'CUSTOMER', jurisdiction: 'BR' },
    { participantId: 'issuer-eurc', participantType: 'ISSUER', jurisdiction: 'US' },
    { participantId: 'institutional-lp-1', participantType: 'INSTITUTIONAL_LP', jurisdiction: 'SG' },
    { participantId: 'neobank-1', participantType: 'NEOBANK', jurisdiction: 'GB' },
    { participantId: 'bank-treasury', participantType: 'BANK_TREASURY', jurisdiction: 'SG' },
    { participantId: 'bank-principal', participantType: 'BANK_PRINCIPAL', jurisdiction: 'SG' },
    { participantId: 'pix-provider', participantType: 'FIAT_PROVIDER', jurisdiction: 'BR' },
    { participantId: 'brl-issuer', participantType: 'ISSUER', jurisdiction: 'BR' },
    { participantId: 'eur-issuer', participantType: 'ISSUER', jurisdiction: 'EU' },
    { participantId: 'atomic-router-provider', participantType: 'NEOBANK', jurisdiction: 'SG' },
  ];
  for (const participant of participants) ensureParticipant(policy, participant);

  for (const participant of participants) {
    const credentials = participant.participantType === 'CUSTOMER'
      ? ['KYC', 'SANCTIONS']
      : ['KYB', 'SANCTIONS', 'AML'];
  ...
  ensureAccount(policy, 'customer-maker', '0x00000000000000000000000000000000000000a1');
  ...
  ensureAccount(policy, 'pix-provider', 'pix-provider:settlement', 'BANK_ACCOUNT');
  ensureAccount(policy, 'brl-issuer', 'brl-issuer:mint');
  ensureAccount(policy, 'eur-issuer', 'eur-issuer:redeem');
  ensureAccount(policy, 'atomic-router-provider', 'atomic-router-provider:router');
}
...
async function seedPrivateMarket({ market, policy, now }) {
  ...
  if (market.aggregateDepth({ inputToken: inputAsset, outputToken: outputAsset }).length > 0) return;
  ...
  await market.admitOrder({
    orderHash: hash(JSON.stringify({ order, authorizationId: authorization.authorizationId })),
    order,
    signature: '0x01',
...
function createReferencePriceFeed(now) {
  ...
  return (inputAsset, outputAsset) => {
    ...
    return engine.consensus({
      base: outputAsset,
      quote: inputAsset,
      observations: observations.map((item) => ({ ...item, observedAt: now() })),
    });
  };
}
...
function seedSettlementGraph({ graph, policy }) {
  const definitions = [
    {
      ...
      edge: {
        ...
        toAsset: REFERENCE_ASSETS.BRLX.id,
    ...
    },
    {
      ...
      edge: {
        ...
        fromAsset: REFERENCE_ASSETS.BRLX.id,
        ...
        unitOutput: '10000',
        cost: '0',
        metadata: { label: 'Atomic BRLX/EURC token FX' },
      },
    },
    {
  ...
  return { edgeIds: definitions.map(({ edge }) => edge.edgeId), authorizations };
}
...
export async function createReferenceRuntime({
  ...
  const market = new FxMarketService({
    ...
    policyAuthorizer: async (admission) => {
      ...
      return result.valid ? { eligible: true, authorizationId: result.authorizationId } : {
        eligible: false,
  ...
  const sourceAuthorizations = seedReferenceSources({ book: referenceBook, policy });
  ...
  const settlementGraph = new SettlementGraph({ authorizationVerifier: settlementVerifier });
  const settlementSeed = seedSettlementGraph({ graph: settlementGraph, policy });
