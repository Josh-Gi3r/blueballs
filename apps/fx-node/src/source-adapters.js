...
export class PrivateMarketLiquidityAdapter {
  ...
  listSlices({ inputAsset, outputAsset, expiresAt }) {
    return this.market
      .aggregateDepth({ inputToken: inputAsset, outputToken: outputAsset })
      .map((level, index) => ({
  ...
  markSubmitted({ reservationHandle, submissionRef }) {
    return this.market.markRouteSubmitted(
      reservationHandle,
      `${submissionRef}:private`,
    );
  }
...
export class PrincipalLiquidityAdapter {
  constructor({
    ...
    if (typeof referenceFor !== "function")
      throw new TypeError("referenceFor required");
    ...
    this.policyAuthorizationId = requiredString(
      policyAuthorizationId,
      "policyAuthorizationId",
    );
  ...
  #quoteArgs({ quoteId, inputAsset, outputAsset, outputAtomic, expiresAt }) {
    ...
    if (!input || !output)
      throw new Error("principal asset metadata unavailable");
    return {
      quoteId,
      inputAsset,
      outputAsset,
  ...
  listSlices({ inputAsset, outputAsset, desiredOutput, expiresAt }) {
    ...
    let preview;
    try {
      preview = this.engine.previewExactOutput(
        this.#quoteArgs({
  ...
  reserve({ routeId, leg, index }) {
    ...
    const quote = this.engine.quoteExactOutput(
      this.#quoteArgs({
        quoteId,
  ...
  validateReserved({ reservationHandle }) {
    ...
    if (rows.some((row) => row.state !== "ACTIVE"))
      throw new Error("principal reservation is not active");
    if (rows.some((row) => row.expires_at <= this.now()))
      throw new Error("principal reservation expired");
