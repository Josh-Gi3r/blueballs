...
import { planExactOutput, reservePlan } from '../../../packages/fx-liquidity/src/index.js';
...
export class IntegratedQuoteCoordinator {
  constructor({
    ...
    if (!market || typeof market.getRoute !== 'function') throw new TypeError('FxMarketService required');
    if (!policyEngine || typeof policyEngine.authorize !== 'function') throw new TypeError('FxPolicyEngine required');
    ...
    if (!principalAdapter || typeof principalAdapter.listSlices !== 'function') {
  ...
  #adapterFor(sourceType) {
    ...
    if (!adapter) throw new Error(`missing integrated adapter for ${sourceType}`);
    return adapter;
  }

  #authorizeCustomer({ participantId, accountRef, inputAsset, outputAsset, exactOutput }) {
    ...
    if (!decision.eligible) {
      throw errorWithCode('customer is not eligible for this FX request', 'POLICY_REJECTED', {
  ...
  async reserveExactOutput({
    ...
    const publicQuote = {
      ...
      finality: {
        ...
        atomic: true,
        class: 'ATOMIC',
        condition: 'atomic once executed through the Blueballs AtomicRouter; requires a configured execution adapter',
  ...
  getQuote(quoteId) {
    return rowToQuote(this.db.prepare('SELECT * FROM integrated_fx_quotes WHERE quote_id = ?').get(quoteId));
  }

  getPrivateQuote(quoteId) {
    const row = this.db.prepare('SELECT * FROM integrated_fx_quotes WHERE quote_id = ?').get(quoteId);
    if (!row) return null;
    return {
      row,
  ...
  getRoute(routeId) {
    const row = this.db.prepare('SELECT * FROM integrated_fx_quotes WHERE route_id = ?').get(routeId);
    if (!row) return null;
  ...
  markSubmitted(quoteId, submissionRef) {
    ...
    if (quote.row.state === 'SUBMITTED' && quote.row.submission_ref === submissionRef) {
      return this.getQuote(quoteId);
    }
    if (quote.row.state !== 'RESERVED') throw new Error(`quote cannot execute from ${quote.row.state}`);
    ...
    this.#transaction(() => {
      const current = this.db.prepare('SELECT state, expires_at FROM integrated_fx_quotes WHERE quote_id = ?').get(quoteId);
      if (!current) throw new Error('quote not found');
      if (current.state !== 'RESERVED') throw new Error(`quote cannot execute from ${current.state}`);
  ...
  confirm(quoteId, { eventId }) {
    ...
    if (quote.row.state !== 'SUBMITTED') throw new Error('quote must be SUBMITTED before confirmation');
  ...
  fail(quoteId, { eventId = null, reason = 'SETTLEMENT_FAILED' }) {
    const quote = this.getPrivateQuote(quoteId);
    if (!quote) throw new Error('quote not found');
    if (quote.row.state === 'FAILED') return { duplicate: true, quote: this.getQuote(quoteId) };
    if (quote.row.state !== 'SUBMITTED') throw new Error('only a submitted quote can fail settlement');
  ...
  expire(now = this.now()) {
    ...
    for (const { quote_id: quoteId } of rows) {
      ...
      this.db.prepare("UPDATE integrated_fx_quotes SET state = 'EXPIRED' WHERE quote_id = ?")
        .run(quoteId);
      expired += 1;
    }
    return expired;
