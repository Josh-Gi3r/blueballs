...
function parseDecimal(value, decimals, field) {
  ...
  if (!/^\d+(?:\.\d+)?$/.test(text)) throw new TypeError(`${field} must be a positive decimal string`);
...
function convertAtomicCeil(value, fromDecimals, toDecimals) {
  const atomic = BigInt(value);
  if (fromDecimals <= toDecimals) return convertAtomic(atomic, fromDecimals, toDecimals);
...
export class ReferenceTradeCoordinator {
  constructor({
    ...
    if (!quotes || typeof quotes.candidateSlices !== 'function' || typeof quotes.reserveExactOutput !== 'function') {
      throw new TypeError('IntegratedQuoteCoordinator required');
    }
    if (!settlementGraph || typeof settlementGraph.analyzeRoute !== 'function') {
    ...
    if (!assets?.BRL || !assets?.BRLX || !assets?.EURC || !assets?.EUR) {
      throw new TypeError('BRL, BRLX, EURC and EUR assets required');
    }
    if (typeof sourceStatus !== 'function') throw new TypeError('sourceStatus must be a function');
  ...
  #planExactInput({ inputMinor, expiresAt }) {
    const { BRL, BRLX, EURC, EUR } = this.assets;
    const tokenBudget = convertAtomic(inputMinor, BRL.decimals, BRLX.decimals);
    let lowCents = 1n;
    ...
    while (lowCents <= highCents) {
      ...
      const exactOutput = convertAtomic(outputCents, EUR.decimals, EURC.decimals);
    ...
    if (!best) {
      const error = new Error('insufficient executable liquidity for this BRL to EUR request');
  ...
  previewExactInput({
    ...
    if (from !== 'BRL' || to !== 'EUR') {
      const error = new Error('the public reference trade currently supports BRL to EUR');
      error.code = 'VALIDATION_ERROR';
      throw error;
    }
    if (!Number.isSafeInteger(expiresInMs) || expiresInMs <= 0 || expiresInMs > 120_000) {
      throw new RangeError('expiresInMs invalid');
    }

    const inputMinor = parseDecimal(inputAmount, this.assets.BRL.decimals, 'inputAmount');
    ...
    return {
      ...
      sources: result.plan.legs.map((leg) => ({
        ...
        inputAmount: formatDecimal(
          convertAtomic(leg.inputAmount, this.assets.BRLX.decimals, this.assets.BRL.decimals),
          this.assets.BRL.decimals,
        ),
        outputAmount: formatDecimal(
          convertAtomic(leg.outputAmount, this.assets.EURC.decimals, this.assets.EUR.decimals),
          this.assets.EUR.decimals,
        ),
      })),
      sourceStatus: this.sourceStatus(),
      tokenRoute: {
        inputAsset: this.assets.BRLX.id,
        inputSymbol: this.assets.BRLX.symbol,
  ...
  async reserveExactInput(args) {
    ...
    const publicTrade = {
      ...
      sources: quote.sources.map((source) => ({
        ...source,
        inputAmount: formatDecimal(
          convertAtomic(source.input, this.assets.BRLX.decimals, this.assets.BRL.decimals),
          this.assets.BRL.decimals,
        ),
        outputAmount: formatDecimal(
          convertAtomic(source.output, this.assets.EURC.decimals, this.assets.EUR.decimals),
          this.assets.EUR.decimals,
        ),
      })),
      sourceStatus: this.sourceStatus(),
  ...
  getTrade(tradeId) {
    ...
    const row = this.db.prepare('SELECT * FROM reference_fx_trades WHERE trade_id = ?').get(tradeId);
    if (!row) return null;
    const stored = JSON.parse(row.public_json);
  ...
  async releaseTrade(tradeId, reason = 'CUSTOMER_ABANDONED') {
    ...
    if (['RELEASED', 'EXPIRED', 'FAILED'].includes(quote.row.state)) return this.getTrade(tradeId);
    if (quote.row.state !== 'RESERVED') {
      throw new Error(`trade cannot be released from ${quote.row.state}; reconcile it instead`);
    }

    for (let index = quote.route.reserved.legs.length - 1; index >= 0; index -= 1) {
