...
function errorResponse(error) {
  ...
  if (!error.code) {
    if (/not found/i.test(message)) {
      ...
      /insufficient eligible liquidity|insufficient executable liquidity/i.test(
        message,
      )
    ) {
...
async function readJson(req, maxBytes = 1_000_000) {
  ...
  for await (const chunk of req) {
    size += chunk.length;
    if (size > maxBytes)
      throw apiError("VALIDATION_ERROR", 413, "request body too large");
    chunks.push(chunk);
  }
...
export function createFxNodeServer({
  ...
  if (
    monetary !== null &&
    (typeof monetary.health !== "function" ||
      typeof monetary.previewRemittance !== "function" ||
      typeof monetary.createReserveDeposit !== "function")
  ) {
    throw new TypeError(
  ...
  async function submitQuote(quoteId) {
    ...
    if (privateQuote.row.expires_at <= now())
      throw apiError("QUOTE_EXPIRED", 409, "quote expired");
  ...
  async function handle(req, res) {
    try {
      ...
      if (method === "GET" && path === "/v2/fx/depth") {
        ...
        if (!inputAsset || !outputAsset)
          throw apiError(
            "VALIDATION_ERROR",
            400,
            "inputAsset and outputAsset required",
          );
        return send(res, 200, {
          object: "depth",
          inputAsset,
          outputAsset,
          levels: market.aggregateDepth({
      ...
      if (
        inspector &&
        method === "GET" &&
        path === "/v2/fx/reference/liquidity"
      ) {
        const inputAsset = url.searchParams.get("inputAsset");
        const outputAsset = url.searchParams.get("outputAsset");
      ...
      if (
        inspector &&
        method === "GET" &&
        path === "/v2/fx/reference/settlement-route"
      ) {
        return send(res, 200, inspector.settlementRoute());
      }
      if (
        inspector &&
        typeof inspector.marketState === "function" &&
        method === "GET" &&
        path === "/v2/fx/reference/market"
      ) {
      ...
      if (
        monetary &&
        method === "GET" &&
        path === "/v2/fx/reference/monetary/health"
      ) {
        return send(res, 200, monetary.health());
      }
      if (
        monetary &&
        method === "GET" &&
        path === "/v2/fx/reference/monetary/instruments"
      ) {
        return send(res, 200, { object: "list", data: monetary.instruments() });
      }
      if (
        monetary &&
        method === "POST" &&
        path === "/v2/fx/reference/monetary/remittance/preview"
      ) {
        return send(res, 200, monetary.previewRemittance(await readJson(req)));
      }
      if (
        monetary &&
        method === "POST" &&
        path === "/v2/fx/reference/monetary/reserves"
      ) {
        return send(
          res,
          201,
          monetary.createReserveDeposit(await readJson(req)),
        );
      }
      let monetaryParams = match(
        path,
        "/v2/fx/reference/monetary/reserves/:depositId",
      );
      if (monetary && method === "GET" && monetaryParams) {
        const deposit = monetary.getReserveDeposit(monetaryParams.depositId);
        if (!deposit)
          throw apiError("NOT_FOUND", 404, "reserve deposit not found");
        return send(res, 200, deposit);
      }
      monetaryParams = match(
        path,
        "/v2/fx/reference/monetary/reserves/:depositId/settle",
      );
      if (monetary && method === "POST" && monetaryParams) {
        return send(
          res,
          200,
          monetary.settleReserveDeposit(monetaryParams.depositId),
        );
      }
      if (
        monetary &&
        method === "POST" &&
        path === "/v2/fx/reference/monetary/risk-capital"
      ) {
        return send(res, 200, monetary.setRiskCapital(await readJson(req)));
      }
      monetaryParams = match(
        path,
        "/v2/fx/reference/monetary/instruments/:instrumentCode/mint",
      );
      if (monetary && method === "POST" && monetaryParams) {
        return send(
          res,
          201,
          monetary.mint(monetaryParams.instrumentCode, await readJson(req)),
        );
      }
      monetaryParams = match(
        path,
        "/v2/fx/reference/monetary/instruments/:instrumentCode/redeem",
      );
      if (monetary && method === "POST" && monetaryParams) {
        return send(
          res,
          200,
          monetary.redeem(monetaryParams.instrumentCode, await readJson(req)),
        );
      }
      if (
        monetary &&
        method === "POST" &&
        path === "/v2/fx/reference/monetary/receipts"
      ) {
        return send(res, 201, monetary.createReceipt(await readJson(req)));
      }
      monetaryParams = match(
        path,
        "/v2/fx/reference/monetary/receipts/:receiptId",
      );
      if (monetary && method === "GET" && monetaryParams) {
        const receipt = monetary.getReceipt(monetaryParams.receiptId);
        if (!receipt) throw apiError("NOT_FOUND", 404, "receipt not found");
        return send(res, 200, receipt);
      }
      monetaryParams = match(
        path,
        "/v2/fx/reference/monetary/receipts/:receiptId/consume",
      );
      if (monetary && method === "POST" && monetaryParams) {
        return send(
          res,
          200,
          monetary.consumeReceipt(monetaryParams.receiptId),
        );
      }
      if (
        monetary &&
        method === "GET" &&
        path === "/v2/fx/reference/monetary/events"
      ) {
        const rawLimit = url.searchParams.get("limit");
        const limit = rawLimit === null ? 50 : Number(rawLimit);
        return send(res, 200, {
          object: "list",
          data: monetary.events({ limit }),
        });
      }
      ...
      if (
        trades &&
        method === "POST" &&
        path === "/v2/fx/reference/trades/preview"
      ) {
      ...
      if (trades && method === "POST" && path === "/v2/fx/reference/trades") {
        return send(
          res,
          201,
          await trades.reserveExactInput(await readJson(req)),
        );
      }
      ...
      if (method === "GET" && path === "/v2/fx/orders") {
        ...
        return send(res, 200, {
          object: "list",
          data: market.listOrdersForMaker(maker),
        });
      }
      ...
      if (method === "POST" && params) {
        ...
        return send(
          res,
          200,
          market.cancelOrderOffchain(params.orderHash, {
      ...
      if (method === "POST" && path === "/v2/fx/quotes") {
        ...
        if (!body.inputAsset || !body.outputAsset || body.exactOutput == null) {
          throw apiError(
            "VALIDATION_ERROR",
            400,
            "inputAsset, outputAsset and exactOutput required",
          );
        }
        return send(
          res,
          201,
          await quotes.reserveExactOutput({
      ...
      if (method === "GET" && params) {
        const route =
          typeof quotes.getRoute === "function"
      ...
      if (method === "POST" && params)
        return send(res, 200, fiat.reserveIntent(params.intentId));
      ...
      if (method === "POST" && params) {
        ...
        return send(
          res,
          202,
          fiat.submitIntent(params.intentId, body.submissionRef),
        );
      }
      ...
      if (method === "POST" && params) {
        ...
        return send(
          res,
          200,
          fiat.settleVerifiedIntent(params.intentId, body.eventId),
        );
      }
      ...
      if (method === "POST" && params) {
        ...
        return send(
          res,
          200,
          await quotes.confirm(params.quoteId, {
