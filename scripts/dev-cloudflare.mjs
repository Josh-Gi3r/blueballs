...
const commands = [
  ["banking", ["exec", "wrangler", "dev", "-c", "wrangler.api.jsonc", "--port", "5381", "--inspector-port", "9331", "--local", "--persist-to", ".wrangler/state/banking"]],
  ["fx", ["exec", "wrangler", "dev", "-c", "wrangler.fx.jsonc", "--port", "5382", "--inspector-port", "9332", "--local", "--persist-to", ".wrangler/state/fx", "--var", `FX_API_KEY:${process.env.FX_API_KEY ?? "bb_test_local_fx"}`]],
];
const children = [];
let stopping = false;
...
console.log("Local FX key      →", process.env.FX_API_KEY ? "from FX_API_KEY" : "bb_test_local_fx");
