...
function versionFromNode(node) {
  ...
  return String(node.version ?? node.resolved ?? 'unknown').replace(/^link:/, 'workspace:');
}

const listed = JSON.parse(run('pnpm', ['list', '-r', '--json', '--depth', 'Infinity']) || '[]');
...
const forgeStd = makefile.match(/^FORGE_STD_VERSION\s*:=\s*(.+)$/m)?.[1]?.trim();
if (oz) add('openzeppelin-contracts', oz, 'library', [{ name: 'blueballs:ecosystem', value: 'solidity' }]);
if (forgeStd) add('forge-std', forgeStd, 'library', [{ name: 'blueballs:ecosystem', value: 'foundry' }]);

const gitCommit = run('git', ['rev-parse', 'HEAD'], { optional: true }) ?? 'unknown';
const gitTimestamp = run('git', ['show', '-s', '--format=%cI', 'HEAD'], { optional: true }) ?? new Date(0).toISOString();
const forgeVersion = run('forge', ['--version'], { optional: true });
const serialHex = createHash('sha256').update(`blueballs:${gitCommit}`).digest('hex').slice(0, 32);
  ...
  metadata: {
    ...
    tools: {
      components: [
        ...
        ...(forgeVersion ? [{ type: 'application', name: 'foundry', version: forgeVersion }] : []),
      ],
    },
    component: {
      type: 'application',
      ...
      properties: [
        ...
        { name: 'blueballs:inventory-source', value: 'pnpm locked workspace plus pinned Foundry libraries' },
      ],
    },
  },
