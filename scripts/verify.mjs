#!/usr/bin/env node
/** Run the complete local repository verification suite. */
...
step("Syntax");
{
  const files = run("git", ["ls-files", "*.js", "*.mjs"])
    .stdout.trim()
    .split("\n")
    .filter(Boolean)
  ...
  record(
    `node --check on ${files.length} JS files`,
    bad.length === 0,
    bad.length ? bad.join(", ") : "all parse",
  );
...
step("Test suites");
{
  ...
  record(
    ...
    api.status === 0
      ? "passed"
      : (api.stderr || api.stdout).trim().split("\n").slice(-3).join(" | "),
  );

  const r = run("node", ["scripts/test-fx.mjs"]);
  const fail = [...r.stdout.matchAll(/^ℹ fail (\d+)$/gm)].reduce(
    (n, m) => n + Number(m[1]),
    0,
  );
  record(
    "FX suites",
    r.status === 0 && fail === 0,
    r.status === 0 ? "all 9 package suites passed" : `${fail} failed`,
  );
  ...
  record(
    ...
    workers.status === 0,
    workers.status === 0
      ? "passed, including Durable Object eviction"
      : (workers.stderr || workers.stdout)
          .trim()
          .split("\n")
          .slice(-3)
          .join(" | "),
  ...
  const packedFiles = (sdk.stderr.match(/^npm notice total files: (\d+)$/m) ??
    [])[1];
  record(
    "FX SDK package boundary",
    sdk.status === 0 && packedFiles === "5",
    sdk.status === 0
      ? `${packedFiles ?? "unknown"} files in dry-run tarball`
      : (sdk.stderr || sdk.stdout).trim().split("\n").slice(-3).join(" | "),
  );
  ...
  const contractSummary =
    contracts.status === 0
      ? ((contracts.stdout.match(/Ran \d+ test suites[^\n]*/g) ?? []).at(-1) ??
        "format, build, fuzz and invariants passed")
      : (contracts.stderr || contracts.stdout)
          .trim()
          .split("\n")
          .slice(-3)
          .join(" | ");
...
step("API catalogue");
{
  ...
  record(
    "router vs documented catalogue",
    r.status === 0,
...
step("Product and publication truth");
{
  const checks = [
    ...
    ["product screenshots", "scripts/check-doc-assets.mjs"],
...
step("Machine-readable contracts");
{
  const before = run("git", [
    ...
    "--",
    "public/openapi.yaml",
    "workers/site/openapi.generated.js",
    "apps/fx-node/src/openapi.generated.js",
  ]).stdout;
  ...
  record(
    "generated OpenAPI is in sync with its sources",
    before === after,
    before === after ? "no drift" : "regenerating changed files — commit them",
  );
  ...
  record(
    "banking OpenAPI describes the catalogue",
    ops === 181,
    `${ops} operations, ${requestBodies} typed request bodies`,
  );
  ...
  record(
    ...
    (requestContracts.stdout || requestContracts.stderr)
      .trim()
      .split("\n")
      .pop(),
  );

  const responseContracts = run("node", [
    "scripts/check-openapi-responses.mjs",
  ]);
  record(
    "banking response contracts match catalogue",
    responseContracts.status === 0,
    (responseContracts.stdout || responseContracts.stderr)
      .trim()
      .split("\n")
      .pop(),
  );
...
step("Repository hygiene");
{
  ...
  record(
    ...
    phHits.length === 0,
    phHits.length ? `${phHits.length}: ${phHits[0]}` : "none",
  );
}
...
step("Live API behaviour (scratch instance)");
{
  ...
  const api = spawn("node", ["apps/api/src/server.js"], {
    ...
    env: {
      ...process.env,
      PORT: String(port),
      DB_PATH: join(dir, "verify.sqlite"),
  ...
  try {
    ...
    record(
      "API boots on this Node version",
      up,
      up
        ? `${process.version} on :${port}`
        : `no response (${process.version})`,
    );
    ...
    const signup = async (email) =>
      (await (await post("/auth/signup", null, { email })).json()).key;
    ...
    const cus = await (
      await post("/customers", keyA, { type: "individual", name: "Verify A" })
    ).json();
    await post(`/customers/${cus.id}/verify`, keyA, {});
    const acc = await (
      await post("/accounts", keyA, { customer: cus.id, currency: "EUR" })
    ).json();
    ...
    const attacks = [
      [
        "read another tenant's customer",
        (await get(`/customers/${cus.id}`, keyB)).status,
      ],
      [
        "read another tenant's account",
        (await get(`/accounts/${acc.id}`, keyB)).status,
      ],
      [
        "approve another tenant's KYC",
        (
          await post(`/customers/${cus.id}/verify`, keyB, {
            decision: "approved",
          })
        ).status,
      ],
      [
        "credit another tenant's account",
        (await post(`/accounts/${acc.id}/credit`, keyB, { amount: "1.00" }))
          .status,
      ],
      [
        "debit another tenant's account",
        (
          await post("/transfers", keyB, {
            from: acc.id,
            amount: "100.00",
            rail: "sepa_instant",
          })
    ...
    for (const [label, status] of attacks)
      record(label, status === 404, `HTTP ${status} (want 404)`);

    const listed = await (await get("/accounts", keyB)).json();
    record(
      "another tenant lists no accounts",
      (listed.data?.length ?? -1) === 0,
      `${listed.data?.length} rows`,
    );

    const ledgerB = await (await get("/ledger?limit=500", keyB)).json();
    record(
      "another tenant reads no ledger rows",
      (ledgerB.data?.length ?? -1) === 0,
      `${ledgerB.data?.length} rows`,
    );
    ...
    const unownedVisible = (allA.data ?? []).filter(
      (r) => r.owner === undefined,
    ).length;
    record(
      "no unowned row is visible to anyone",
      unownedVisible === 0,
      `${unownedVisible} unowned rows returned`,
    );

    // --- the owner still works
    const own = await post("/transfers", keyA, {
      from: acc.id,
      amount: "100.00",
      rail: "sepa_instant",
    });
    record(
      "owner can still move their own money",
      own.status === 201,
      `HTTP ${own.status}`,
    );
    ...
    const usd = await (
      await post("/accounts", keyA, { customer: cus.id, currency: "USDC" })
    ).json();
    await post(`/accounts/${usd.id}/credit`, keyA, { amount: "1000.00" });
    const before = (await (await get("/ledger?limit=500", keyA)).json()).data
      .length;
    const refused = await post("/fx/intents", keyA, {
      ...
      to: "EURC",
      amount: "1000.00",
      min_receive: "99999.00",
    });
    const after = (await (await get("/ledger?limit=500", keyA)).json()).data
      .length;
    record(
      "swap below min_receive is refused",
      refused.status === 400,
      `HTTP ${refused.status}`,
    );
    record(
      "refused swap wrote nothing to the ledger",
      before === after,
    ...
    const lpCus = await (
      await post("/customers", lpKey, { type: "business", name: "Verify LP" })
    ).json();
    await post(`/customers/${lpCus.id}/verify`, lpKey, {});
    const lpAcc = await (
      await post("/accounts", lpKey, { customer: lpCus.id, currency: "EURC" })
    ).json();
    record(
      "an account can be opened in a traded stablecoin",
      Boolean(lpAcc.id),
      lpAcc.id ? "EURC" : JSON.stringify(lpAcc).slice(0, 120),
    );
    if (lpAcc.id) {
      await post(`/accounts/${lpAcc.id}/credit`, lpKey, { amount: "50000.00" });
      const commit = await post("/fx/lp", lpKey, {
        account: lpAcc.id,
        currency: "EURC",
        amount: "20000.00",
        pair: "USDC/EURC",
      ...
      record(
        "liquidity can be committed to a corridor",
        commit.status === 201,
        `HTTP ${commit.status}`,
      );

      const swap = await (
        await post("/fx/intents", keyA, {
          account: usd.id,
          from: "USDC",
          to: "EURC",
          amount: "100.00",
      ...
      const filled = (swap.fill_legs ?? [])
        .filter((l) => !l.declined)
        .map((l) => l.source);
      record(
        "a trade fills from the committed pool",
        filled.includes("lp"),
        filled.join(",") || "no fill",
      );
      ...
      record(
        "the provider earned from that fill",
        (earnings.data?.length ?? 0) > 0,
        `${earnings.data?.length ?? 0} earning rows`,
      );
    }
