...
acc_b, _ = funded_account(kb, "Bank LP",   "EUR", "EURC", "200000.00", ctype="business")
acc_m, _ = funded_account(km, "Member LP", "EUR", "EURC", "20000.00")
acc_t, _ = funded_account(kt, "Taker",     "USD", "USDC", "50000.00")

lp_b = req("POST", "/v2/fx/lp", {"account": acc_b, "currency": "EURC",
                                 "amount": "200000.00", "pair": "EURC/USDC"}, kb)
lp_m = req("POST", "/v2/fx/lp", {"account": acc_m, "currency": "EURC",
                                 "amount": "20000.00", "pair": "EURC/USDC"}, km)
print("LP positions:")
...
earn_b = delta(acc_b, "EURC")
earn_m = delta(acc_m, "EURC")
retained = prefix_delta("spread:", "EURC")
...
print(f"     leg priced at {bps} bps; at mid the taker would get {at_mid.quantize(D('0.01'))} EURC")
print(f"     actually received {received} EURC  => implied spread {implied} EURC")
...
check("taker EURC gained", delta(acc_t, "EURC"), received)
check("taker USDC spent",  -delta(acc_t, "USDC"), D("50000.00"))
