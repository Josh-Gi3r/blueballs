...
acc_biz = funded(kbiz, "Integrator Bank", "USD", "USDC", "500000.00", "business")
acc_ret = funded(kret, "Retail Person",   "USD", "USDC", "50000.00",  "individual")

print("1. tier gating — a firm quote is the integrator tier")
st, q = req("POST", "/v2/fx/rfq",
            {"account": acc_ret, "from": "USDC", "to": "EURC", "amount": "10000.00"}, kret)
check("retail is refused a firm quote", st == 403, f"HTTP {st}: {q.get('detail','')[:70]}")

st, q = req("POST", "/v2/fx/rfq",
            {"account": acc_biz, "from": "USDC", "to": "EURC", "amount": "10000.00"}, kbiz)
...
funded_taker = funded(kbiz, "Flow Mover", "USD", "USDC", "400000.00", "business")
req("POST", "/v2/fx/intents",
    {"account": funded_taker, "from": "USDC", "to": "EURC",
