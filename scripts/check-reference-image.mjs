...
for (const command of [
  ...
  if (!compose.includes(command))
    failures.push(`compose.reference.yml omits ${command}`);
}
if (
  !compose.includes("Dockerfile.reference") ||
  !compose.includes("condition: service_healthy")
) {
  failures.push(
    "reference Compose does not build the pinned image or gate the site on healthy APIs",
  );
}
...
console.log(
  "reference image contract: API, FX, site and public assets are packaged; service health ordering is explicit",
);
