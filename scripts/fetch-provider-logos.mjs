...
function iconCandidates(html, pageUrl) {
  return [...html.matchAll(/<link\b[^>]*>/gi)]
    ...
    .filter(
      ({ rel, href }) => /icon/i.test(rel) && href && !href.startsWith("data:"),
    )
    .map((candidate) => {
      let score = 20;
      if (/apple-touch/i.test(candidate.rel)) score += 50;
      if (/\.svg(?:$|\?)/i.test(candidate.href) || /any/i.test(candidate.sizes))
        score += 40;
      const size = Number(candidate.sizes.match(/(\d+)x\d+/)?.[1] || 0);
      score += Math.min(size, 512) / 16;
      return {
        url: new URL(candidate.href.replaceAll("&amp;", "&"), pageUrl).href,
        score,
      };
    })
...
async function toPng(id, image) {
  ...
  const extension = svg
    ? ".svg"
    : extname(new URL(image.finalUrl).pathname) || ".img";
...
for (const provider of providers) {
  ...
  try {
    ...
    if (response.ok)
      candidates = iconCandidates(await response.text(), response.url);
  } catch {}
  candidates.push(
...
process.stdout.write(
  `Saved ${Object.keys(manifest).length}/${providers.length} provider logos.\n`,
);
