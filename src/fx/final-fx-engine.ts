...
export function selectedSources(state: SimulationState) {
  const profiles: Record<LiquidityPlaneId, Record<Source["id"], number>> = {
    ...
    customer: { customer: 48, issuer: 18, maker: 19, treasury: 10, external: 5 },
    "issuer-heavy": { customer: 10, issuer: 50, maker: 25, treasury: 10, external: 5 },
    institutional: { customer: 0, issuer: 20, maker: 48, treasury: 27, external: 5 },
    external: { customer: 5, issuer: 12, maker: 18, treasury: 10, external: 55 },
  };

  const profile = profiles[state.liquidityPlane];
  let sources = BASE_SOURCES.map((source) => ({ ...source, pct: profile[source.id] })).filter(
    (source) => source.pct > 0,
  );
  if (!state.customerLiquidity) sources = sources.filter((source) => source.id !== "customer");
  if (state.scenario === "maker_offline") sources = sources.filter((source) => source.id !== "maker");
  if (state.scenario === "issuer_revoked") sources = sources.filter((source) => source.id !== "issuer");
  if (state.scenario === "treasury_limit" || state.scenario === "reference_outage") {
    sources = sources.filter((source) => source.id !== "treasury");
  }
  if (state.scenario === "quote_expired" || state.scenario === "proof_replay") return [];

  const total = sources.reduce((sum, source) => sum + source.pct, 0);
  if (total <= 0) return [];
  return sources.map((source) => ({ ...source, pct: (source.pct / total) * 100 }));
}

export function routeStatus(state: SimulationState, sources: Source[]) {
  if (state.scenario === "quote_expired") return { label: "QUOTE EXPIRED", complete: false };
  if (state.scenario === "proof_replay") return { label: "PAYMENT PROOF REJECTED", complete: false };
  if (state.scenario === "payout_pending") return { label: "PAYOUT PENDING", complete: true };
...
export function executionCoreStep(state: SimulationState, corridor: Corridor): JourneyStep {
...
export function buildJourneySteps(state: SimulationState, corridor: Corridor): JourneyStep[] {
  ...
  const byJourney: Record<JourneyId, JourneyStep[]> = {
    ...
    own: [
      ...
      },
      {
        ...
        detail: "The institution records the input-side debit on its authoritative ledger.",
        finality: "AUTHORITATIVE LEDGER",
      },
      commonFx,
      {
    ...
    provider: [
      ...
      commonFx,
      {
        owner: "PROVIDER",
        title: `${corridor.to} payout submitted`,
        detail: "The provider or connected bank rail starts the recipient payout.",
        finality: "ASYNC EXTERNAL",
      },
      {
        owner: "BLUEBALLS",
...
export function executionFinalityLeg(state: SimulationState, corridor: Corridor): FinalityLeg {
  if (state.executionPlane === "internal") {
    return {
      kick: "TOKEN LEG",
...
export function finalityLegs(state: SimulationState, corridor: Corridor): FinalityLeg[] {
  ...
  return [
    {
      kick: "INPUT LEG",
      label: `${corridor.from} → ${corridor.fromToken}`,
      type: state.journey === "provider" ? "provider-confirmed deposit" : "account or provider funding",
      class: "ASYNC EXTERNAL",
    },
    core,
    {
...
export function availableScenarios(journey: JourneyId): ScenarioId[] {
  ...
  if (["hybrid", "provider", "issuer", "p2p"].includes(journey)) ids.push("payout_pending");
  return ids;
}
