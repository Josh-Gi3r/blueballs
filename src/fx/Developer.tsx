...
export function DeveloperInspector({ trade, amount }: { trade: PublicTrade | null; amount: string }) {
  ...
  return <section id="fx-developer" className="fxp-section fxp-developer"><div className="fxp-section-head"><div><Label>DEVELOPER VIEW</Label><h2>Inspect the request, response, IDs and implementation.</h2><p>{FX_RUNTIME_MODE === "demo" ? "The website demo follows the downloadable FX node response shape, so you can inspect the flow before running the stack." : "The IDs and numbers below come from the FX node response shown in the phone."}</p></div><a href={openApiHref} target="_blank" rel="noreferrer"><Pill>OpenAPI ↗</Pill></a></div><div className="fxp-dev-tabs">{(["request", "response", "events", "source"] as DevView[]).map((item) => <button type="button" key={item} className={view === item ? "active" : ""} onClick={() => setView(item)}>{item}</button>)}</div><div className="fxp-code-window"><div className="fxp-code-window-top"><span>{view === "source" ? "IMPLEMENTATION MAP" : FX_RUNTIME_MODE === "demo" ? "WEBSITE DEMO" : "REFERENCE NODE"}</span><b>{trade?.quoteId ?? "unreserved preview"}</b></div><pre>{data}</pre></div><div className="fxp-proof-links"><a href={source("apps/fx-node/test/public-reference-runtime.test.js")} target="_blank" rel="noreferrer">Integration test ↗</a><a href={source("packages/fx-contracts/script/ControlledProof.s.sol")} target="_blank" rel="noreferrer">Controlled EVM proof ↗</a><a href={source("spec/fx/PUBLIC-REFERENCE.md")} target="_blank" rel="noreferrer">Public reference contract ↗</a></div></section>;
}
