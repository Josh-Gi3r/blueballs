...
/** Link a path in the repository. Written while the source was private, this
 *  returned "/contact" for every path — the repository is public now. */
export const source = (path: string) => `${REPO}/tree/main/${path}`;
...
export const SCENARIO_COPY: Record<string, { title: string; short: string }> = {
  balanced: {
    ...
    short: "All six source classes can compete.",
  },
  lp_offline: {
    title: "LP offline",
    short: "The route rebuilds without the institutional LP.",
  },
  issuer_policy_blocked: {
    title: "Issuer revoked",
    short: "The issuer is removed before price comparison.",
  },
  treasury_near_limit: {
    title: "Treasury nearly used",
    short: "Only a small amount of treasury inventory remains.",
  },
  principal_limit: {
    title: "Balance sheet at limit",
    short: "The configured principal limit has been reached.",
  },
  reference_outage: {
    title: "Reference price unavailable",
    short: "The bank stops quoting its own balance sheet.",
  },
};
...
export function errorMessage(result: ApiResult) {
  ...
  return (
    result.error ?? body?.error?.message ?? `FX node returned ${result.status}`
  );
}

/** Map an on-chain asset identifier back to its currency symbol using the
 *  trade's own context. The live node returns token addresses where the demo
 *  returns symbols — never show a raw address to a reader. */
export function assetSymbol(trade: PublicTrade | null, value: string) {
  if (!value) return "—";
  if (trade) {
    const map: Record<string, string> = {
      [trade.from.asset]: trade.from.symbol,
      [trade.to.asset]: trade.to.symbol,
      [trade.tokenRoute.inputAsset]: trade.tokenRoute.inputSymbol,
      [trade.tokenRoute.outputAsset]: trade.tokenRoute.outputSymbol,
    };
    if (map[value]) return map[value];
  }
  if (/^0x[a-fA-F0-9]{20,}$/.test(value))
