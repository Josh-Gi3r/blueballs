...
export type JourneyId = "hybrid" | "own" | "provider" | "issuer" | "p2p" | "stable";
export type ScenarioId =
  ...
  | "payout_pending"
  | "reference_outage";
export type LiquidityPlaneId = "mixed" | "customer" | "issuer-heavy" | "institutional" | "external";
export type FiatPlaneId = "hybrid" | "own" | "provider" | "issuer" | "bank" | "p2p" | "none";
export type ExecutionPlaneId = "blueballs" | "internal" | "external";
export type InspectorView = "request" | "route" | "policy" | "finality" | "events" | "source";
export type CapabilityTone = "built" | "reference" | "not-connected" | "simulated";
...
export const BASE_SOURCES: Source[] = [
  { id: "customer", label: "Customer orders", pct: 20, type: "PRIVATE ORDER", rateAdj: -2 },
  { id: "issuer", label: "Issuer liquidity", pct: 25, type: "ISSUER INVENTORY", rateAdj: 0 },
  { id: "maker", label: "Professional maker", pct: 35, type: "FIRM QUOTE", rateAdj: 2 },
  { id: "treasury", label: "Treasury", pct: 15, type: "LIMITED PRINCIPAL", rateAdj: 5 },
  { id: "external", label: "External venue", pct: 5, type: "EXTERNAL QUOTE", rateAdj: 8 },
];

export const SCENARIO_CATALOG: Record<ScenarioId, { label: string; short: string }> = {
  normal: { label: "Normal", short: "All selected sources are available." },
  maker_offline: { label: "Maker unavailable", short: "The route is recalculated without the professional maker." },
  issuer_revoked: { label: "Issuer authorisation revoked", short: "The issuer is excluded before prices are compared." },
  treasury_limit: { label: "Treasury limit reached", short: "No additional principal capacity is available." },
  quote_expired: { label: "Quote expired", short: "A new quote is required before execution." },
  proof_replay: { label: "Payment proof replay", short: "The payment identifier has already been used." },
  payout_pending: { label: "Payout pending", short: "The token exchange is complete but the external payout is not final." },
  reference_outage: { label: "Reference price unavailable", short: "Principal pricing stops until a valid reference price is available." },
};
...
export const money = (corridor: Corridor, value: number, side: "from" | "to" = "from") => {
  const code = side === "from" ? corridor.from : corridor.to;
  const symbol = side === "from" ? corridor.fromSymbol : corridor.toSymbol;
  if (code === "USDC" || code === "EURC") return `${formatNumber(value, 2)} ${code}`;
  return `${symbol}${formatNumber(value, 2)}`;
};

export const tokenMoney = (code: string, value: number) => `${formatNumber(value, 2)} ${code}`;
