...
export function BankView({ trade, selected, onSelect }: { trade: PublicTrade | null; selected: string | null; onSelect: (id: string) => void }) {
  ...
  return <div className="fxp-hero-panel"><div className="fxp-panel-heading"><div><Label>INSTITUTION VIEW</Label><h3>Sources considered for this reference quote.</h3></div><Pill>{trade?.sources.length ?? 0} selected</Pill></div><div className="fxp-bank-source-list">{statuses.map((status) => {
