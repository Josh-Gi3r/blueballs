...
export const FX_CAPABILITIES: FxCapability[] = [
  { id:"market", title:"Private FX market", status:"BUILT", summary:"Maker orders, aggregate depth, partial fills, reservations and cancellation. Signature checking is a pluggable verifier and the bundled sandbox accepts any signature, so a deployment must supply a real one.", api:["POST /v2/fx/orders","GET /v2/fx/depth","POST /v2/fx/orders/:orderHash/cancel"], sourcePath:"packages/fx-market", testPath:"packages/fx-market/test" },
  ...
  { id:"settlement", title:"Atomic token settlement", status:"REFERENCE", summary:"Maker-signed fills, taker bounds and atomic multi-maker settlement, implemented and tested as Solidity. The Node runtime does not call these contracts: settlement in this repository is the double-entry ledger, and on-chain execution needs an execution adapter.", sourcePath:"packages/fx-contracts", testPath:"packages/fx-contracts/test" },
