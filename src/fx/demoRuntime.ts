...
const SOURCES: DemoSource[] = [
  { type: "PRIVATE_MARKET", sourceId: "customer-liquidity", label: "Customer liquidity", rate: 6.035, capacityEur: 1_250, reason: "SIGNED_AND_POLICY_AUTHORISED" },
  { type: "ISSUER", sourceId: "eurc-issuer", label: "Issuer", rate: 6.045, capacityEur: 1_500, reason: "POLICY_AUTHORISED" },
  { type: "NEOBANK", sourceId: "regional-bank", label: "Another institution", rate: 6.06, capacityEur: 900, reason: "POLICY_AUTHORISED" },
  { type: "INSTITUTIONAL_LP", sourceId: "institutional-lp", label: "Institutional LP", rate: 6.07, capacityEur: 1_800, reason: "POLICY_AUTHORISED" },
  { type: "BANK_TREASURY", sourceId: "bank-treasury", label: "Treasury", rate: 6.08, capacityEur: 1_200, reason: "WITHIN_INVENTORY_LIMIT" },
  { type: "BANK_PRINCIPAL", sourceId: "bank-principal", label: "Bank balance sheet", rate: 6.095, capacityEur: 5_000, reason: "WITHIN_RISK_LIMIT" },
];

const SETTLEMENT_EDGES: SettlementEdge[] = [
  { edgeId: "demo-pix-payment", edgeType: "VERIFIED_FIAT_PAYMENT", finalityClass: "ATTESTED_EXTERNAL", fromAsset: "BRL", toAsset: "BRLX", providerId: "demo-pix" },
  { edgeId: "demo-token-fx", edgeType: "TOKEN_SWAP", finalityClass: "ATOMIC", fromAsset: "BRLX", toAsset: "EURC", providerId: "blueballs-router" },
  { edgeId: "demo-eur-redemption", edgeType: "ISSUER_REDEEM", finalityClass: "ASYNC_EXTERNAL", fromAsset: "EURC", toAsset: "EUR", providerId: "demo-eur-issuer" },
];
...
const atomic = (value: number, decimals = 6) => Math.round(value * (10 ** decimals)).toString();
...
function failure(message: string, status = 400, code = "DEMO_ERROR", details?: unknown): DemoApiResult {
  return {
    ok: false,
    status,
    ms: 90,
...
function sourceForScenario(source: DemoSource): DemoSource & { eligible: boolean; statusReason: string } {
  let eligible = true;
  ...
  } else if (activeScenario === "issuer_policy_blocked" && source.type === "ISSUER") {
    eligible = false;
    statusReason = "AUTHORISATION_REVOKED";
  } else if (activeScenario === "treasury_near_limit" && source.type === "BANK_TREASURY") {
    capacityEur = 200;
    statusReason = "NEAR_INVENTORY_LIMIT";
  } else if (activeScenario === "principal_limit" && source.type === "BANK_PRINCIPAL") {
    capacityEur = 300;
    statusReason = "NEAR_RISK_LIMIT";
  } else if (activeScenario === "reference_outage" && source.type === "BANK_PRINCIPAL") {
    eligible = false;
    capacityEur = 0;
...
function buildTrade(inputAmount: string, reserved: boolean): PublicTrade | DemoApiResult {
  ...
  const trade: PublicTrade = {
    ...
    createdAt: Date.now(),
    expiresAt,
    from: { asset: "BRL", symbol: "BRL", requested: money(input), charged: money(input), atomic: Math.round(input * 100).toString() },
    to: { asset: "EUR", symbol: "EUR", amount: money(output), atomic: Math.round(output * 100).toString() },
    rate: (input / output).toFixed(6),
    sources: plan.allocations,
    sourceStatus: sourceStatuses(),
    tokenRoute: { inputAsset: "BRLX", inputSymbol: "BRLX", inputAtomic: atomic(input), outputAsset: "EURC", outputSymbol: "EURC", outputAtomic: atomic(output) },
    settlement: {
      guarantee: { atomic: false, class: "MIXED_FINALITY", finalityClasses: ["ATTESTED_EXTERNAL", "ATOMIC", "ASYNC_EXTERNAL"] },
      edges: SETTLEMENT_EDGES,
    },
    evidence: {
      reserved,
      label: reserved ? "WEBSITE DEMO · RESERVED" : "WEBSITE DEMO PREVIEW",
      note: reserved ? "Seeded demo capacity is held for this browser session." : "Seeded demo data. No money moves.",
    },
  };
  ...
  return {
    ...
    customerAuthorization: { authorizationId: `auth_demo_${id}`, expiresAt, policyId: "website-demo-policy", policyVersion: 1 },
  };
}
...
export async function demoFxCall(method: string, path: string, body?: unknown): Promise<DemoApiResult> {
  ...
  if (method === "GET" && path === "/v2/fx/reference/policy") {
    return success({ policyId: "website-demo-policy", version: 1, rule: "eligible sources compete on price and capacity" }, started);
  }
  ...
  if (method === "GET" && path === "/v2/fx/reference/settlement-route") {
    return success({ fromAsset: "BRL", toAsset: "EUR", guarantee: { atomic: false, class: "MIXED_FINALITY" }, edges: SETTLEMENT_EDGES }, started);
  }
  ...
  if (method === "POST" && executeMatch) {
    ...
    const confirmed: PublicTrade = {
      ...
      eventId: `event_${id}`,
      evidence: { reserved: true, label: "WEBSITE DEMO · COMPLETE", note: "Simulated receipt. No money moved." },
    };
    trades.set(id, confirmed);
    return success({ trade: confirmed, execution: { status: "CONFIRMED", demo: true } }, started, 202);
  }
  ...
  if (tradeMatch) {
    ...
    if (method === "DELETE") {
      const released: PublicTrade = {
        ...trade,
        state: "RELEASED",
        evidence: { reserved: false, label: "WEBSITE DEMO · RELEASED", note: "The seeded reservation was released." },
      };
  ...
  return failure(`The embedded demo does not implement ${method} ${path}.`, 404, "NOT_FOUND");
}
