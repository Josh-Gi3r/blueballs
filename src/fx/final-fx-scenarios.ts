import { CORRIDORS, JOURNEYS, money, type Corridor, type FinalityLeg, type InspectorView, type JourneyId, type ScenarioDetail, type ScenarioId, type SimulationState, type Source } from "./final-fx-definitions";
  ...
  const base: Record<ScenarioId, ScenarioDetail> = {
    normal: {
      ...
      flow: [
        ...
        ["Output leg", state.journey === "stable" ? "not required" : "simulated next step", "done"],
      ],
      timeline: [
      ...
      assurances: [
        ["Simulation only", "The page does not submit a payment or token transaction."],
        ["Illustrative capacity", "No real liquidity is reserved."],
        ["Inspector updates", "The request, route and event examples reflect the current selections."],
      ],
    },
    maker_offline: {
      ...
      assurances: [
        ["Unavailable source excluded", "The maker is removed before the new allocation is shown."],
        ["Quote is recalculated", "The previous price is not reused."],
        ["Treasury limit enforced", "Any replacement treasury allocation remains within its configured limit."],
      ],
    },
    issuer_revoked: {
      ...
      timeline: [
        ...
        ["Eligibility checked again", "Issuer excluded before price comparison"],
      ...
      assurances: [
        ["Policy applied before pricing", "A revoked source is not treated as executable liquidity."],
        ["Remaining sources listed", "The allocation identifies every selected source."],
    ...
    treasury_limit: {
      ...
      flow: [
        ...
        [
          "Customer result",
          sources.length >= 3 ? "complete without treasury" : "no complete route",
      ...
      timeline: [
        ...
        ["Result returned", sources.length >= 3 ? "Alternative route available" : "No quote available"],
      ],
      event: {
      ...
      assurances: [
        ["Exposure limit enforced", "A higher spread cannot override the configured limit."],
        ["Reservations included", "Active reservations reduce available treasury capacity."],
        ["Full coverage required", "No quote is returned unless the complete amount can be supported."],
      ],
    },
    quote_expired: {
      title: "Quote expired",
      ...
      event: { event: "QUOTE_EXPIRED", state: "EXPIRED", action: "REPRICE", moneyMoved: false },
      assurances: [
        ["Expired quote blocked", "The customer must receive a new quote."],
        ["Expiry enforced", "The validity period is part of the quote conditions."],
    ...
    proof_replay: {
      ...
      timeline: [
        ...
        ["Token release rejected", "No stablecoin released and no FX trade created"],
        ["Manual resolution may be required", "The external fiat payment remains a separate issue"],
      ],
      event: {
      ...
      assurances: [
        ["No second token release", "One payment identifier cannot release tokens twice."],
        ["No FX trade created", "The token exchange does not start."],
        ["External payment tracked separately", "Support may still need to resolve the off-chain payment."],
      ],
    },
    payout_pending: {
      ...
      flow: [
        ...
        ["Token exchange", `${corridor.fromToken} → ${corridor.toToken} marked complete in simulation`, "done"],
      ...
      assurances: [
        ["Separate states", "Token settlement and fiat payout are tracked independently."],
        ["Pending remains pending", "The route is not marked successful or failed without evidence."],
        ["No automatic resubmission", "The expected operator action is to reconcile, not submit another payout."],
      ],
    },
    reference_outage: {
      ...
      flow: [
        ...
        ["Customer result", sources.length ? "route without treasury" : "no complete route", sources.length ? "done" : "failed"],
      ],
      timeline: [
        ["Reference price unavailable", "Treasury pricing disabled"],
        ["Treasury source removed", "No fallback rate assumed"],
        ["Allocation recalculated", "Independently priced orders and issuer quotes remain eligible"],
        ["Result returned", sources.length ? "Alternative route available" : "No quote available"],
      ],
      event: {
      ...
      assurances: [
        ["No fallback rate", "Treasury pricing stops when the reference price is unavailable."],
        ["Independent sources retained", "Signed orders and firm quotes can still be considered on their own terms."],
        ["Full coverage required", "No quote is returned when the requested amount cannot be supported."],
      ],
    },
  };
...
export function inspectorPayload(
  ...
  const values: Record<InspectorView, { label: string; status: string; value: string }> = {
    request: {
    ...
    policy: {
      ...
      status: state.scenario === "issuer_revoked" ? "SOURCE EXCLUDED" : "ELIGIBLE",
      value: JSON.stringify(
        {
          ...
          guarantee:
            legs.length === 1
              ...
              : new Set(legs.map((leg) => leg.class)).size === 1
                ? legs[0].class
                : "MIXED_FINALITY",
          edges: legs.map((leg) => ({ edge: leg.label, mechanism: leg.type, finality: leg.class })),
...
export function setJourneyState(current: SimulationState, journeyId: JourneyId): SimulationState {
