        ...
        <div className="hero-copy fade-up">
          ...
          <div className="hero-proof">
            <div>
              ...
              <b>
                Customer orders, issuers, professional makers, treasury and
                external venues can contribute to one quote.
              </b>
            </div>
            <div>
              <span>FX SETUPS</span>
              <b>
                Use internal accounts, a payments provider, issuer mint and
                redemption, open P2P or a token-only route.
              </b>
            </div>
            <div>
            ...
            <DeviceShell badge="SIMULATION" scale={0.88}>
              <div className="fx-device-exchange hero-device-content">
                ...
                <div className="fx-device-field receive">
                  <div>
                    <span>TO</span>
                    <small>{corridor.to}</small>
                  </div>
                  <b>
                    {status.complete
                      ? money(corridor, output, "to")
                      : "No quote"}
                  </b>
                </div>
                <div className="fx-device-meta">
                  ...
                  <div>
                    ...
                    <b>
                      {sources.length
                        ? `${sources.length} selected sources`
                        : "Unavailable"}
                    </b>
                  </div>
                  <div>
                  ...
                  <div>
                    <span>Status</span>
                    <b>{status.complete ? "Quote ready" : status.label}</b>
                  </div>
                </div>
                <button type="button" onClick={() => scrollTo("route")}>
                  Review exchange
                </button>
          ...
          <div className="machine-side">
            ...
            <div className="machine-card">
              ...
              <div className="machine-route">
                ...
                {sources.length === 0 && (
                  <div className="machine-route-row">
                    <i style={{ background: "#b4453c" }} />
            ...
            <div className="machine-note">
              {state.scenario === "normal"
                ? journey.summary
                : `${SCENARIO_CATALOG[state.scenario].label}: ${SCENARIO_CATALOG[state.scenario].short}`}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="sim-console fade-up" aria-label="Simulation controls">
        <div className="sim-field">
          ...
          <select
            ...
            onChange={(event: { target: { value: string } }) =>
              changeCorridor(event.target.value as CorridorId)
            }
          >
        ...
        <div className="sim-field">
          ...
          <select
            ...
            onChange={(event: { target: { value: string } }) =>
              setState((current) =>
                setJourneyState(current, event.target.value as JourneyId),
              )
            }
          >
        ...
        <button
          className="btn secondary sim-reset"
          type="button"
  ...
  return (
    <section className="section" id="walkthrough">
      <div className="section-inner">
        <SectionHead
          eyebrow="01 · COMPLETE FLOW"
          title="Choose how funds enter, exchange and reach the recipient."
        >
          Each walkthrough shows the complete transaction: customer or account
          checks, input funds, token FX, payout and reconciliation. The same
          policy and routing modules can support internal accounts, providers,
          issuers, P2P or token-only exchange.
        </SectionHead>
        <div className="journey-tabs fade-up">
          {Object.entries(JOURNEYS).map(([id, item]) => (
            <button
              type="button"
              key={id}
              className={state.journey === id ? "active" : ""}
              onClick={() =>
                setState((current) => setJourneyState(current, id as JourneyId))
              }
            >
              {item.label}
            </button>
        ...
        <div className="walkthrough-shell fade-up">
          <div className="walkthrough-top">
            <div className="walkthrough-summary">
              <span className="small-label">SELECTED FLOW</span>
              <h3>{journey.title}</h3>
              <p>{journey.summary}</p>
              <div className="status-row">
                {journey.status.map(([label, tone]) => (
                  <span className={`status-chip ${tone}`} key={label}>
                    {label}
                  </span>
                ))}
              </div>
            ...
            <div className="journey-stage">
              <div className="journey-stage-head">
                <div>
                  <span className="small-label">TRANSACTION FLOW</span>
                  <b>
                    {corridor.from} input → {corridor.to} output
                  </b>
                </div>
                <button
                  ...
                  onClick={() =>
                    setState((current) => ({
                      ...current,
                      journeyPlaying: !current.journeyPlaying,
                    }))
                  }
                >
                  {state.journeyPlaying ? "Pause flow" : "Play flow"}
                </button>
              </div>
              <div className="journey-line">
                {journeySteps.map((step, index) => (
                  ...
                  >
                    <button
                      type="button"
                      className={`journey-step ${step.kind === "core" ? "core" : ""} ${state.journeyStep === index ? "active" : ""}`}
                      onClick={() =>
                        setState((current) => ({
                          ...current,
                          journeyStep: index,
          ...
          <div className="journey-detail">
            <div>
              <span className="small-label">{activeStep.owner}</span>
              <h3>{activeStep.title}</h3>
              <p>{activeStep.detail}</p>
            </div>
            <div className="ownership-tags">
              {[...new Set(journeySteps.map((step) => step.owner))].map(
                (owner) => (
    ...
    <section className="section" id="market">
      <div className="section-inner">
        ...
        <div className="market-stage fade-up">
          ...
          >
            <path
              id="bbfx-path-customer"
              d="M 236 190 C 330 110, 380 170, 450 240"
            />
            <path
              id="bbfx-path-maker"
              d="M 964 290 C 855 210, 820 210, 750 260"
            />
            <path
              id="bbfx-path-back"
              d="M 450 325 C 360 350, 330 320, 236 280"
            />
            <circle r="4" fill="#ffffff">
              <animateMotion dur="3.4s" repeatCount="indefinite" begin=".2s">
                <mpath href="#bbfx-path-maker" />
              </animateMotion>
            </circle>
            <circle r="4" fill="#ffffff">
              <animateMotion dur="3.1s" repeatCount="indefinite" begin="1.5s">
                <mpath href="#bbfx-path-customer" />
              </animateMotion>
            </circle>
            <circle r="4" fill="#ffffff">
              <animateMotion dur="3.6s" repeatCount="indefinite" begin="2.4s">
                <mpath href="#bbfx-path-back" />
              </animateMotion>
            </circle>
          </svg>
            ...
            <DeviceShell scale={0.78}>
              <div className="fx-device-market-content">
                ...
                <div className="fx-device-field receive">
                  ...
                  <b>{money(corridor, output, "to")}</b>
                </div>
                <div className="fx-device-market-stat">
                  <span>Liquidity</span>
                  <b>
                    {sources.length
                      ? `${sources.length} selected sources`
                ...
                <button
                  type="button"
                  onClick={() => setSettlePulse((value) => value + 1)}
                >
          ...
          <div className="market-core">
            <div className="market-core-head">
              <span>PRIVATE FX MARKET</span>
              <b>{corridor.pairLabel}</b>
            </div>
            <h3>
              Orders, inventory and firm quotes can contribute to one quote.
            </h3>
            <div className="book-rows">
              {BASE_SOURCES.map((source, index) => {
                ...
                return (
                  <div
                    className={`book-row ${source.id === "customer" ? "customer" : ""}`}
                    key={source.id}
                  >
                    <span>{source.label}</span>
                    <div className="depth-bar">
                      <i style={{ width: `${availableWidth}%` }} />
                      <u style={{ width: `${filledWidth}%` }} />
                    </div>
                    <strong>
                      {selected
                        ? `${formatNumber(selected.pct, 0)}% selected`
                        : "not selected"}
                    </strong>
                  </div>
                );
            ...
            >
              ...
              <b>
                {sources.length
                  ...
                  : "No token settlement is prepared"}
              </b>
            </div>
          </div>
          ...
          <div className="fx-market-device">
            <DeviceShell scale={0.78}>
              <div className="fx-device-market-content">
                <div className="fx-device-market-kicker">
                  CUSTOMER OR BUSINESS
                </div>
                <div className="fx-device-title">Place an FX order</div>
                <div className="fx-device-order-card">
                  <span>PAIR</span>
                  <b>{corridor.pairLabel}</b>
                </div>
                <div className="fx-device-order-card">
                  <span>OFFER</span>
                  <b>{tokenMoney(corridor.toToken, output * 0.2)}</b>
                </div>
                <div className="fx-device-order-card">
                  <span>LIMIT RATE</span>
                  <b>
                    1 {corridor.fromToken} ={" "}
                    {formatNumber(corridor.rate * 1.0013, 4)} {corridor.toToken}
                  </b>
                </div>
                <div className="fx-device-market-stat">
                  <span>Order visibility</span>
                  <b>Private to the institution</b>
                </div>
                <button
                  className={state.customerLiquidity ? "secondary-state" : ""}
                  type="button"
                  onClick={() =>
                    setState((current) => ({
                      ...current,
                      customerLiquidity: !current.customerLiquidity,
                      scenario: "normal",
                    }))
                  }
                >
                  {state.customerLiquidity
                    ? "Remove sample order"
                    : "Add sample order"}
                </button>
              </div>
