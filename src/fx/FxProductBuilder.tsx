...
const SECTION_LINKS = [
  {
    id: "walkthrough",
    title: "Complete flow",
    description: "Who does each step",
  },
  {
    id: "market",
    title: "Private market",
    description: "Who can provide liquidity",
  },
  {
    id: "route",
    title: "Customer quote",
    description: "How sources are combined",
  },
  { id: "privacy", title: "Privacy", description: "What stays private" },
  {
    id: "settlement",
    title: "Settlement",
    description: "What is final or pending",
  },
...
export default function FxProductBuilder({ ctx }: { ctx: FinalFxContext }) {
  ...
  const finalityLabel =
    finalityClasses.length === 1 ? finalityClasses[0] : "MIXED FINALITY";
    ...
    <section className="section fx-product-builder" id="compose">
      <div className="section-inner">
        <div className="section-head fade-up">
          ...
          <div>
            <h2>
              Configure one exchange and see the product and backend change
              together.
            </h2>
            <p>
              Choose how money enters and leaves, where the FX liquidity comes
              from and how the token leg settles. The customer view, backend
              route and every section below use these same choices.
            </p>
          </div>
        </div>
        ...
        <div className="fx-builder-shell fade-up">
          <div className="fx-builder-statusbar">
            ...
            {customSetup && (
              <button
                ...
                onClick={() =>
                  setState((current) =>
                    setJourneyState(current, current.journey),
                  )
                }
              >
                Reset to preset
          ...
          <div
            className="fx-builder-controls-row"
            aria-label="FX product configuration"
          >
            <label className="fx-builder-control-card">
              <span>1 · MONEY IN + OUT</span>
              <b>
                How does the customer fund the exchange and receive the result?
              </b>
              <div className="fx-select-wrap">
                <select
                  ...
                  onChange={(event: { target: { value: string } }) =>
                    changeFundingModel(event.target.value as FiatPlaneId)
                  }
                >
          ...
          <div className="fx-builder-preview-grid">
                ...
                <DeviceShell badge="SIMULATION">
                  <div className="fx-device-exchange">
                    ...
                    <div className="fx-device-meta">
                      <div>
                        <span>Funding</span>
                        <b>{FUNDING_LABELS[state.fiatPlane]}</b>
                      </div>
                      <div>
                        <span>Delivery</span>
                        <b>{DELIVERY_LABELS[state.fiatPlane]}</b>
                      </div>
                      <div>
                        <span>Liquidity</span>
                        <b>
            ...
            <div className="fx-builder-backend">
              ...
              <div className="fx-backend-summary">
                <div>
                  <span>INPUT</span>
                  <b>{FUNDING_DETAILS[state.fiatPlane]}</b>
                </div>
                <i>→</i>
                <div>
                  <span>FX</span>
                  <b>
                    {corridor.fromToken} → {corridor.toToken}
                  </b>
                </div>
                <i>→</i>
                <div>
                  <span>OUTPUT</span>
                  <b>{DELIVERY_DETAILS[state.fiatPlane]}</b>
                </div>
              </div>
              ...
              <div className="fx-backend-flow">
                ...
                <div className="fx-backend-node">
                  ...
                  <div>
                    <span>POLICY</span>
                    <b>
                      Customer, account, corridor, ticket size and source
                      eligibility are checked before pricing.
                    </b>
                  </div>
                </div>
                <div className="fx-backend-connector" />
                <div className="fx-backend-node route-node">
                  ...
                  <div>
                    ...
                    <b>
                      {sources.length
                        ? `${sources.length} eligible sources cover the quote`
                        : "No complete source allocation"}
                    </b>
                    <small>{LIQUIDITY_OPTIONS[state.liquidityPlane]}</small>
                    {sources.length > 0 && (
                      <div className="fx-backend-sources">
                        {sources.map((source) => (
                          <em key={source.id}>{source.label}</em>
                        ))}
                      </div>
                    )}
                  </div>
                ...
                <div className="fx-backend-node">
                  ...
                  <div>
                    ...
                    <b>{EXECUTION_OPTIONS[state.executionPlane]}</b>
                  </div>
                </div>
                <div className="fx-backend-connector" />
                <div className="fx-backend-node">
              ...
              <div className="fx-backend-result">
                ...
                <div>
                  ...
                  <b>{status.complete ? finalityLabel : status.label}</b>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="fx-builder-connections">
            <div className="fx-builder-connections-head">
              <div>
                <span>FOLLOW THIS SAME EXCHANGE</span>
                <b>Open any layer below without changing context.</b>
              </div>
              <small>
                Every section is reading the configuration above, not starting a
                new example.
              </small>
            </div>
