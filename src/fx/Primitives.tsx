...
export function Label({ children }: { children: ReactNode }) { return <div className="fxp-label">{children}</div>; }
export function Pill({ children, tone = "blue" }: { children: ReactNode; tone?: "blue" | "green" | "red" | "grey" }) { return <span className={`fxp-pill ${tone}`}>{children}</span>; }
export function LiveBadge({ connected, loading, mode }: { connected: boolean; loading: boolean; mode: "node" | "demo"; }) {
