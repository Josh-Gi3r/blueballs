...
const entries = [
  {
    date: "31/07",
    name: "Payroll · outgoing",
    acct: "DR 6000 / CR 1001",
    amount: "− €4,120.00",
    color: "#07144F",
  },
  {
    date: "28/07",
    name: "Client invoice 204",
    acct: "DR 1001 / CR 4000",
    amount: "+ €6,800.00",
    color: "#0647E8",
  },
  {
...
function ExchangeScreen({
  ...
  useEffect(() => {
    ...
    const tick = () =>
      setSecondsLeft(
        Math.max(
          0,
          Math.round((Date.parse(fxQuote.expires_at) - Date.now()) / 1000),
        ),
      );
    tick();
  ...
  const spreadEur = fxQuote
    ? (FX_SAMPLE_EUR * fxQuote.spread_bps) / 10000
    : null;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div style={{ ...white, borderRadius: 16, padding: 16 }}>
          ...
          >
            <span>TO</span>
            <span>
              {fxQuote ? `MID + ${fxQuote.spread_bps} BPS` : "MID + BPS"}
            </span>
          </div>
          <div
            style={{
      ...
      >
        {fxQuote ? (
          <>
            ...
            >
              <span>Rate</span>
              <span style={{ color: "#07144F" }}>
                1 EUR = {fxQuote.rate} GBP
              </span>
            </div>
            <div
              style={{
            ...
            >
              <span>Spread ({fxQuote.spread_bps} bps)</span>
              <span style={{ color: "#07144F" }}>€{spreadEur!.toFixed(2)}</span>
            </div>
            <div
              style={{
                display: "flex",
            ...
            >
              ...
              <span style={{ color: expired ? "#B4453C" : "#07144F" }}>
                {expired ? "expired" : `${secondsLeft}s`}
              </span>
            </div>
          </>
        ) : fxErr ? (
          <div style={{ fontSize: 12.5, color: "#B4453C" }}>
            Could not reach the reference API. Is it running on :5290?
          </div>
        ) : (
          <div style={{ fontSize: 12.5, color: "#7A8296" }}>
            Fetching a reference quote from POST /v2/quotes…
          </div>
        )}
      </div>
      <div
      ...
      >
        This card uses the hosted sandbox response from{" "}
        <span style={{ fontFamily: MONO }}>POST /v2/quotes</span> for €
        {fmt(FX_SAMPLE_EUR)} → GBP.
      </div>
      <div style={{ marginTop: "auto", padding: "14px 20px 22px" }}>
        <div
          style={{
            background: expired ? "#DDE1E8" : "#0868FF",
            color: expired ? "#7A8296" : "#FFFFFF",
            borderRadius: 12,
            padding: 15,
            textAlign: "center",
            fontSize: 14.5,
        ...
        >
          {fxQuote
            ? expired
              ...
              : `Convert · rate holds ${secondsLeft}s`
            : "Convert"}
        </div>
      </div>
    </div>
...
function FxTradeScreen({
  fxTrade,
  fxTradeErr,
}: {
  fxTrade?: FxTrade | null;
  fxTradeErr?: boolean;
}) {
  const [secondsLeft, setSecondsLeft] = useState<number | null>(null);
  useEffect(() => {
    ...
    const expiryMs =
      fxTrade.expiresAt > 1e12 ? fxTrade.expiresAt : fxTrade.expiresAt * 1000;
    const tick = () =>
    ...
    >
      ...
      >
        <div style={{ ...white, borderRadius: 16, padding: 16 }}>
          ...
          >
            <span>FROM</span>
            <span>{fxTrade ? fxTrade.from.symbol : ""}</span>
          </div>
          <div
            style={{
              display: "flex",
          ...
          >
            ...
            >
              {fxTrade
                ? money(fxTrade.from.symbol, sent)
                : fxTradeErr
                  ? "—"
                  : "…"}
            </div>
        ...
        <div style={{ ...white, borderRadius: 16, padding: 16 }}>
          ...
          >
            ...
            <span>
              {fxTrade
                ? `${fxTrade.sources.length} SOURCE${fxTrade.sources.length === 1 ? "" : "S"}`
                : ""}
            </span>
          </div>
          <div
          ...
          >
            ...
            >
              {fxTrade
                ? money(fxTrade.to.symbol, fxTrade.to.amount)
                : fxTradeErr
                  ? "—"
                  : "…"}
            </div>
        ...
        {fxTrade ? (
          <>
            ...
            >
              <span>Rate</span>
              <span style={{ color: "#07144F" }}>
                1 {fxTrade.to.symbol} = {Number(fxTrade.rate).toFixed(4)}{" "}
                {fxTrade.from.symbol}
              </span>
            </div>
            <div
            ...
            >
              <span>Filled from</span>
              <span style={{ color: "#07144F" }}>
                {fxTrade.sources.length} source
                {fxTrade.sources.length === 1 ? "" : "s"} in your market
              </span>
            </div>
            <div
              style={{
            ...
            >
              ...
              <span style={{ color: "#07144F" }}>One on-chain transaction</span>
            </div>
            <div
              style={{
                display: "flex",
          ...
          <div style={{ fontSize: 12.5, color: "#B4453C" }}>
            Could not reach the FX runtime.
          </div>
        ) : (
          <div style={{ fontSize: 12.5, color: "#7A8296" }}>
            Fetching a live preview from the FX runtime…
          </div>
        )}
      </div>
      <div
      ...
      <div style={{ marginTop: "auto", padding: "14px 20px 22px" }}>
        ...
        >
          {fxTrade
            ? expired
              ? "Quote expired · re-quote"
              : `Review exchange · rate holds ${secondsLeft}s`
            : "Review exchange"}
        </div>
      </div>
    </div>
...
export default function PhoneScreen({
  ...
  const isLive =
    (screen === "exchange" && !!fxQuote) || (screen === "fx" && !!fxTrade);
  return (
    <div
      style={{
        width: "100%",
        ...
        >
          ...
          <div
            style={{
              ...
              padding: "3px 8px",
              borderRadius: 999,
              background: isLive ? "#E7F3EC" : "#F0F1F5",
              color: isLive ? "#2E7D53" : "#7A8296",
          ...
          {screen === "accounts" && (
            ...
            >
              ...
              <div style={{ padding: "20px 20px 14px" }}>
                ...
                >
                  €24,183<span style={{ color: "#8A91A2" }}>.40</span>
                </div>
                <div style={{ fontSize: 12.5, color: "#0647E8", marginTop: 4 }}>
                  + €412.10 this month
                </div>
              </div>
              <div
              ...
              >
                ...
                >
                  <div style={{ fontSize: 14, fontWeight: 600 }}>Accounts</div>
                  <div style={{ fontSize: 12.5, color: "#0868FF" }}>Manage</div>
                </div>
                {accounts.map((ac) => (
                  <div
                    key={ac.name}
          ...
          {screen === "cards" && (
            ...
            >
              ...
              >
                ...
                <div
                  style={{ fontSize: 13, color: "#0868FF", fontWeight: 500 }}
                >
                  + New
                </div>
              </div>
            ...
            >
                ...
                {rails.map((r) => (
                  <div
                    ...
                    style={{
                      ...
                      padding: "12px 15px",
                      borderRadius: 14,
                      border: `1px solid ${r.sel ? "#0868FF" : "#E3E6EE"}`,
                      background: r.sel ? "#EAF4FF" : "#FFFFFF",
                    }}
                  >
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 13.5, fontWeight: 500 }}>
                      ...
                      <div style={{ fontSize: 11.5, color: "#7A8296" }}>
                        {r.sub}
                      </div>
                    </div>
                    <div
              ...
              >
                ...
                >
                  ...
                  <span style={{ color: "#07144F" }}>SEPA Instant</span>
                </div>
                <div
                  style={{
                    background: "#07144F",
            ...
            >
              ...
              <div style={{ padding: "0 20px 14px" }}>
                ...
                >
                  ...
                  >
                    €12,480<span style={{ color: "#8B93A6" }}>.00</span>
                  </div>
                  <div
                    style={{
                      height: 7,
                  ...
                  >
                    <span>62% of €20,000</span>
                    <span>Daily accrual rule</span>
                  </div>
                </div>
              </div>
              <div
            ...
            >
              ...
              <div style={{ padding: "0 20px 14px" }}>
                <div style={{ ...white, borderRadius: 18, padding: 20 }}>
                  ...
                  >
                    €13,000<span style={{ color: "#8A91A2" }}>.00</span>
                  </div>
                  <div
                    style={{
                      height: 7,
              ...
              >
                Liquidation at 78% LTV. You are notified 24 hours before any
                margin call.
              </div>
              <div
                style={{
          ...
          {screen === "ledger" && (
            ...
            >
              ...
              <div
                style={{ padding: "4px 20px 12px", display: "flex", gap: 7 }}
              >
                {[
                  { label: "July", sel: true },
                  { label: "June", sel: false },
                  { label: "Q2", sel: false },
                ].map((p) => (
                  <div
                    key={p.label}
                    style={{
                      ...
                      background: p.sel ? "#EAF4FF" : "#FFFFFF",
                      color: p.sel ? "#0647E8" : "#454B5C",
                    }}
                  >
                    {p.label}
                  </div>
              ...
              >
                ...
                {entries.map((e) => (
                  ...
                  >
                    ...
                    <div
                      style={{ fontSize: 13, fontWeight: 600, color: e.color }}
                    >
                      {e.amount}
                    </div>
                  </div>
