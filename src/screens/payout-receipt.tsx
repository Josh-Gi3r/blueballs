...
export default function PayoutReceipt() {
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", alignItems: "center" }}>
      <div style={{
        width: 60, height: 60, borderRadius: 999, background: "#E7F3EC", border: "1px solid #BEE3CE",
        display: "flex", alignItems: "center", justifyContent: "center", margin: "26px 0 14px",
        fontSize: 24, color: "#2E7D53", fontWeight: 600,
      }}>✓</div>

      <div style={label}>SENT</div>
      <div style={{ fontSize: 34, fontWeight: 600, letterSpacing: "-0.04em", marginTop: 4 }}>€1,250<span style={{ color: "#8A91A2" }}>.00</span></div>

      <div style={{ padding: "18px 20px 0", width: "100%" }}>
        <div style={{ ...white, borderRadius: 16, padding: "4px 16px" }}>
          {rows.map((r) => (
            <div key={r.k} style={{ display: "flex", justifyContent: "space-between", gap: 12, ...rowB }}>
              <div style={{ fontSize: 12.5, color: "#7A8296" }}>{r.k}</div>
              <div style={{ fontSize: 12.5, fontWeight: 500, textAlign: "right", fontFamily: r.k === "Reference" ? MONO : undefined }}>{r.v}</div>
            </div>
          ))}
        </div>
      </div>
