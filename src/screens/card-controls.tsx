  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
      <div style={{ padding: "14px 20px 10px", display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <div style={{ fontSize: 19, fontWeight: 600, letterSpacing: "-0.03em" }}>Spend controls</div>
      </div>
      ...
      <div style={{ flex: 1, background: "#FFFFFF", borderTop: "1px solid #E3E6EE", padding: "15px 20px 0" }}>
        ...
        {categories.map((c) => (
          <div key={c.mcc} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, ...rowB }}>
            <div>
              <div style={{ fontSize: 13.5, fontWeight: 500 }}>{c.name}</div>
              <div style={{ fontFamily: MONO, fontSize: 10.5, color: "#7A8296" }}>MCC {c.mcc}</div>
            </div>
            <div style={toggle(c.on)}><div style={knob(c.on)} /></div>
          </div>
        ))}
      </div>
    </div>
