...
function moduleOn(row: number, col: number): boolean {
  const inCorner = (r0: number, c0: number) => row >= r0 && row < r0 + 5 && col >= c0 && col < c0 + 5;
...
function QrBlock() {
  ...
  for (let row = 0; row < GRID; row++) for (let col = 0; col < GRID; col++) if (moduleOn(row, col)) cells.push({ row, col });
  return (
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      <div style={{ padding: "14px 20px 0" }}>
        <div style={{ ...white, borderRadius: 14, padding: "12px 15px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
          <div>
            <div style={label}>WALLET ADDRESS · BASE</div>
            <div style={{ fontFamily: MONO, fontSize: 13.5, fontWeight: 500, marginTop: 4 }}>{ADDRESS_SHORT}</div>
          </div>
          <div style={{ borderRadius: 10, border: "1px solid #DDE1E8", padding: "7px 12px", fontSize: 12, fontWeight: 500, color: "#454B5C" }}>Copy</div>
        </div>
      </div>
