  ...
  return (
    ...
    >
      ...
      <div style={{ padding: "6px 20px 14px", textAlign: "center" }}>
        ...
        >
          + $12,000<span style={{ color: "#8A91A2" }}>.00</span>
        </div>
        <div
          style={{
            display: "inline-flex",
          ...
          >
            ...
            <div
              style={{
                fontSize: 12.5,
                fontWeight: 500,
                color: s.done ? "#07144F" : "#B4B9C6",
              }}
            >
              {s.name}
            </div>
            {i < steps.length - 1 && (
              <div style={{ flex: 1, height: 1, background: "#F0F1F5" }} />
            )}
          </div>
        ))}
      </div>
      ...
      >
        <span style={{ fontFamily: MONO, color: "#07144F" }}>
          trf_9k2ncQ81pR
        </span>{" "}
        is a sample transfer reference, not proof of settlement. The example
        balance changes only after the sandbox rail confirms.
      </div>
    </div>
  );
}
...
export const meta: ScreenMeta = {
  ...
  code: 'fetch("/v2/transfers/trf_9k2ncQ81pR", {\n  headers: { "x-api-key": key }\n}).then((response) => response.json());',
};
